...
import {
  gridColumnsAlignClassMap as alignClassMap,
  gridColumnsBorderWidthValueMap as borderWidthValueMap,
  gridColumnsDesktopSpanClassMap as desktopSpanClassMap,
  gridColumnsGapXClassMap as gapXClassMap,
  gridColumnsGapYClassMap as gapYClassMap,
  gridColumnsMinHeightClassMap as minHeightClassMap,
  gridColumnsMobileReverseOrderClassMap as mobileReverseOrderClassMap,
  gridColumnsPaddingClassMap as paddingClassMap,
  gridColumnsRadiusClassMap as radiusClassMap,
  gridColumnsSelfAlignClassMap as selfAlignClassMap,
  gridColumnsSpanClassMap as spanClassMap,
  gridColumnsTabletMinHeightClassMap as tabletMinHeightClassMap,
  gridColumnsTabletSpanClassMap as tabletSpanClassMap,
  gridColumnsTwoXlSpanClassMap as twoXlSpanClassMap,
  gridColumnsXlSpanClassMap as xlSpanClassMap,
  joinGridColumnsClasses as joinClasses,
} from "./gridColumnsClassNames";
import {
  gridColumnsColumnMax,
  gridColumnsColumnMin,
  gridColumnsDefaults,
  gridColumnsEditorContract,
  gridColumnsGapTokens,
  gridColumnsMinHeightTokens,
  gridColumnsSchema,
  gridColumnsSlot,
  gridColumnsSpanTokens,
  type GridColumnsAlign,
  type GridColumnsAsymmetricVariantState,
  type GridColumnsBorderWidth,
  type GridColumnsColorValue,
  ...
  type GridColumnsSpanTotals,
  type GridColumnsVariantId,
} from "./gridColumnsContract";

export * from "./gridColumnsContract";
...
function orderLiveGridColumnsTargets(
  columns: GridColumnsColumn[],
  slotMap: Record<string, WidgetBlock[]>
) {
  const liveTargets = resolveWidgetSlotTargets([gridColumnsSlot], slotMap).filter(
    (target) => target.definitionId === gridColumnsSlot.id
  );
  const liveTargetsByInstanceId = new Map(
    liveTargets.flatMap((target) => {
      const instanceId = parseRepeatableSlotId(target.slotId)?.instanceId;
      return instanceId ? [[instanceId, target] as const] : [];
    })
  );
  const usedSlotIds = new Set<string>();
  const configuredTargets = columns.flatMap((column, index) => {
    const instanceId = column.id?.trim() || String(index + 1);
    const target = liveTargetsByInstanceId.get(instanceId);
    if (!target || usedSlotIds.has(target.slotId)) return [];
    usedSlotIds.add(target.slotId);
    return [target];
  });
  const orphanTargets = liveTargets
    .filter((target) => !usedSlotIds.has(target.slotId))
    .sort((left, right) => {
      if (left.slotId === right.slotId) return 0;
      return left.slotId < right.slotId ? -1 : 1;
    });

  return [...configuredTargets, ...orphanTargets];
}
...
const resolveGridColumnsForSlots = ({
  ...
  const slotTargets = hasLiveColumnSlots
    ? orderLiveGridColumnsTargets(columns, slotMap)
