/**
 * MenuDesignEditorBlockFields - per-block field renderers of the Menu Design
 * editor (TASK-542-03-L03 line-gate split). Owns the block-type branches
 * (nav-items / brand / cta-button / divider / spacer) plus the shared
 * nav-appearance and nav-chrome control helpers they consume. The parent
 * `MenuBlockPanel` keeps the header chrome (title, move/remove) and the
 * visibility override, then delegates the authored fields here.
 */
...
export function MenuBlockFields({
  block,
  doc,
  device,
  palette,
