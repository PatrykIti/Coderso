...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        // TASK-479-06-L01: rounded-2xl + soft shadow. Internal flex/gap/py rhythm
        // preserved so existing (pre-479-07) pages keep their vertical spacing.
        "bg-card text-card-foreground flex flex-col gap-6 rounded-2xl border border-border py-6 shadow-soft",
        className
      )}
      {...props}
    />
