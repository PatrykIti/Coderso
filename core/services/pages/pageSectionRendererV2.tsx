...
import { toPageSectionBleedStyle, toPageSectionRenderProps } from "./pageSectionRenderStyles";
import {
...
const wrapSectionTemplateBlock = (
  ...
  const templateGridItem =
    spanTarget === "section-template-wrapper" ? { [PAGE_BLOCK_GRID_ITEM_ATTRIBUTE]: block.id } : {};
...
const renderTemplateSectionChildren = (
  ...
  if (template.template === "media-split" && template.variant !== "default") {
    return renderMediaSplitSectionChildren(section, template, blocks, (block, index) =>
...
export function PageSectionRenderImpl({
  ...
  const isRevealSection = scrollEffect === "reveal-fade" || scrollEffect === "reveal-up";
