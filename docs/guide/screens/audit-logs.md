...
7. Use `Next` and `Previous` when the current filter scope has more rows than
   the loaded page. Changing search, date range, event type, or severity starts
   the slice again from the first page.
8. Open a row when you need more than the summary table gives you.
9. In `Event Details`, review:
   ...
   - event type,
   - JSON payload.
10. Use `Copy JSON` when the selected event payload is the important artifact.
   The copied JSON is redacted and includes the stable event timestamp.
11. `Share Log` and `Report` are disabled until a supported compliance workflow
    is available.
12. Use `Export` when the current filtered view needs to leave the page.
13. In the export dialog, review:
   - file format,
   - included fields,
   - export filename,
   - reminder that export uses the current filters.
14. Export only after confirming the filter scope is correct.
...
- Export should be treated as a filtered evidence snapshot, not as a substitute
  for narrowing the view properly first. CSV and JSON exports are generated by
  `/admin/api/audit/export`, use the current server-side filters, and serialize
  only selected allowlisted columns.
- Export scope follows the current filters, not only the currently visible page
  cursor.
- Payload exports are redacted recursively before download. CSV output escapes
  commas, quotes, newlines, and formula prefixes.
...
- Count copy describes loaded rows and cursor availability. It should not be
  ...
  metadata.
- `Next` and `Previous` are driven by server cursor metadata. If a cursor
  expires, the UI reloads the first page instead of keeping a stale page state.
