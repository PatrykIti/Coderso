---
title: "Playbook: Choosing the Right Solution Kit"
audience: "admin"
productArea: "playbooks"
language: "en"
keywords:
  - solution kit
  - selection
  - recommendation
  - planning
---

# What Is It

This playbook helps teams choose between the available Solution Kits and decide
when a manual setup is the better option.

# When To Use

Use it before running the guided site builder or when the assistant recommends a
kit and you want to understand why.

# Step By Step

1. Define the real business outcome: leads, booking, directory discovery,
   commerce, or mixed goals.
2. Compare the recommended modules and workflows, not just the kit label.
3. Check whether the kit will save setup time or create cleanup work.
4. If no kit is a close fit, choose the manual playbook instead.

# Examples

- A workshop decides between lead-generation-first and booking-first rollout.
- A directory business confirms that listings and filters matter more than
  commerce.
- A hybrid business chooses manual setup because no kit matches its exact blend
  of needs.

# Common Mistakes

- Choosing the most familiar label instead of the best workflow fit.
- Assuming every site needs a kit.
- Treating the assistant recommendation as mandatory instead of explainable
  guidance.
