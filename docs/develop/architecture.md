...
- **Two connection targets.** `DATABASE_URL` is the default client and should point at the provider's connection pooler where one exists (Render: same host, port `6432`, `pool_mode = transaction`). It serves everything transactional, and skipping the backend fork saves roughly 250 ms on every new connection — which dominates for short-lived processes such as test and smoke subprocesses. A transaction pooler cannot carry SESSION state, so the paths that take `pg_advisory_lock` open a separate direct connection via `withSessionDatabaseClient` (`core/db/sessionClient.ts`) against `DATABASE_DIRECT_URL`; the resolution and its fail-closed guard live in `core/db/connectionTargets.ts`. See [Runtime model → Database connection targets](./runtime-model.md#database-connection-targets).
