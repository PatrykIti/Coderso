...
export const meta = Object.freeze({
  ...
  description:
    "Implement TASK-554 sequentially with fail-closed ownership, executable gates, shared smoke, and an owner-review handoff.",
...
const TASK_554_WORKFLOW_PATHS = Object.freeze([
...
export const OWNERS = Object.freeze([
  ...
  owner("security-gate-repair", TASK_554_SECURITY_GATE_REPAIR_PATHS),
]);
...
export const FULL_GATE_COMMANDS = Object.freeze([
  ...
  command("task_554_frozen_install_test", "bun", [
    "test",
    "tests/unit/workflows/task554IsolatedFrozenInstall.test.ts",
  ]),
  command("task_554_vitest", "bunx", [
    "vitest",
    "run",
    "--config",
    "vitest.config.ts",
  ...
  command("task_554_route_and_rbac", "bun", [
    "test",
    "tests/integration/routes/postsRoutes.test.ts",
    "tests/integration/routes/postMetadataRbac.test.ts",
    "tests/unit/auth/rbac.test.ts",
  ]),
  command("task_554_runtime_harness", "bun", [
    "test",
    "tests/unit/runtime-smoke/cli-registry.test.ts",
    "tests/unit/runtime-smoke/supervised-server.test.ts",
    "tests/unit/runtime-smoke/repository-report.test.ts",
  ...
  command("task_554_workflow_contracts", "bun", [
    "test",
    "tests/unit/workflows/task554AuthorAudit.test.ts",
    "tests/unit/workflows/task554WorkflowContracts.test.ts",
  ]),
  command("task_554_types", "bun", ["--cwd", "core", "lint:types"]),
  command("task_554_lint", "bun", ["--cwd", "core", "lint"]),
  command("task_554_admin_boundary", "bun", ["run", "check:admin-boundary"]),
  command("task_540_boundary", "bun", [
    "test",
    "tests/unit/runtime-smoke/task540-native-suite-boundary.test.ts",
  ]),
  command("task_554_security_scan", "bun", ["run", "scan:security:strict"]),
  command("task_554_coderso_release_gates", "bun", ["run", "gates:coderso"]),
  command("task_554_precommit", "bun", ["run", "precommit:check"]),
  command("task_554_author_syntax", "node", [
    "--check",
    "_docs/_workflows/task-554-author-audit.mjs",
  ]),
  command("task_554_implement_syntax", "node", [
    "--check",
    "_docs/_workflows/task-554-implement.mjs",
  ]),
  command("task_554_fix_syntax", "node", ["--check", "_docs/_workflows/task-554-fix.mjs"]),
  command("task_554_closeout_syntax", "node", [
    "--check",
    "_docs/_workflows/task-554-closeout.mjs",
  ]),
]);
export const OWNER_GATE_COMMANDS = Object.freeze({
  ...
  "contract-schema-route": Object.freeze([
    command("task_554_contract_vitest", "bunx", [
      "vitest",
      "run",
      "--config",
      "vitest.config.ts",
    ...
    command("task_554_contract_bun", "bun", [
      "test",
      "tests/integration/routes/postsRoutes.test.ts",
      "tests/integration/routes/postMetadataRbac.test.ts",
      "tests/unit/auth/rbac.test.ts",
  ...
  "admin-client": Object.freeze([
    command("task_554_client_vitest", "bunx", [
      "vitest",
      "run",
      "--config",
      "vitest.config.ts",
  ...
  "classic-metadata-ui": Object.freeze([
    command("task_554_ui_vitest", "bunx", [
      "vitest",
      "run",
      "--config",
      "vitest.config.ts",
...
export function verifyTask554Bootstrap(root = ROOT) {
  ...
  if (
    ...
    parsed.paths.length !== TASK_554_WORKFLOW_PATHS.length ||
    parsed.paths.some((entry, index) => entry !== TASK_554_WORKFLOW_PATHS[index])
  )
...
function verifyBeforeDispatch(phaseName, root = ROOT) {
  try {
    ...
    throw new Error(
      `task_554_bootstrap_before_${phaseName.replaceAll(" ", "_")}:${error instanceof Error ? error.message : String(error)}`
    );
  }
}
function verifyTask554AuthorAuditReceipt(root = ROOT) {
  ...
  if (receipt?.task !== TASK || receipt?.fingerprint === undefined)
    throw new Error("task_554_author_audit_receipt_invalid");
  return receipt;
}
...
export const TASK_554_RESUME_ALLOWED_DIRTY_PATHS = Object.freeze([
  ...new Set([
    ...INITIAL_DIRTY_PATHS,
...
function assertImplementationPreflight(root = ROOT) {
  ...
  if (unexpected.length > 0 || forbidden.length > 0)
    throw new Error(`task_554_start_state_invalid:${JSON.stringify({ forbidden, unexpected })}`);
  return Object.freeze({
    receipt: verifyTask554AuthorAuditReceipt(root),
...
function assertResumePreflight(root = ROOT) {
  ...
  const unexpected = dirty.filter(
    (relativePath) => !TASK_554_RESUME_ALLOWED_DIRTY_PATHS.includes(relativePath)
  );
  if (unexpected.length > 0)
    throw new Error(`task_554_resume_state_invalid:${JSON.stringify({ unexpected })}`);
...
function workflowTreePaths(root) {
  ...
  const visit = (absolutePath, depth) => {
    if (depth > MAX_WORKFLOW_TREE_DEPTH || entries.length >= MAX_WORKFLOW_TREE_ENTRIES)
      throw new Error("task_554_workflow_tree_limit");
    let stats;
    try {
...
function readStableTask554TmpFile(absolute, label) {
  ...
  try {
    ...
    const node = task554TmpNode(before);
    if (
      !sameTask554TmpNode(task554TmpNode(initial), node) ||
      !sameTask554TmpNode(node, task554TmpNode(after)) ||
      !sameTask554TmpNode(node, task554TmpNode(final)) ||
...
function captureTask554TmpSnapshot(root) {
  ...
  if (!initial.isDirectory() || initial.isSymbolicLink())
    throw new Error("task_554_tmp_root_invalid");
  const entries = [[".tmp", `directory:${initial.dev}:${initial.ino}:${initial.mode}`]];
  const files = new Map();
  const visit = (absolute, relative, depth) => {
    if (depth > MAX_WORKFLOW_TREE_DEPTH || entries.length >= MAX_WORKFLOW_TREE_ENTRIES)
      throw new Error("task_554_tmp_tree_limit");
    const stats = lstatSync(absolute);
    if (stats.isSymbolicLink()) throw new Error("task_554_tmp_entry_invalid");
    if (stats.isDirectory()) {
      const node = task554TmpNode(stats);
      ...
      if (
        ...
        throw new Error("task_554_tmp_ancestor_changed");
      return;
    }
    if (!stats.isFile()) throw new Error("task_554_tmp_entry_invalid");
    const file = readStableTask554TmpFile(absolute, "task_554_tmp_entry");
  ...
  {
    const finalNode = task554TmpNode(lstatSync(directory));
    const initialNode = task554TmpNode(initial);
    if (
      initialNode.dev !== finalNode.dev ||
      initialNode.ino !== finalNode.ino ||
      initialNode.mode !== finalNode.mode
    ) {
      console.error(
        "DIAG ancestor_changed root:",
        JSON.stringify({ root, initial: initialNode, final: finalNode, entries })
      );
      console.error("DIAG ancestor stack:", new Error().stack.split("\n").slice(2, 8).join("\n"));
      throw new Error("task_554_tmp_ancestor_changed");
    }
  }
  return Object.freeze({
...
export function countPhysicalLines(filePath) {
  ...
  if (result.error || result.status !== 0 || result.signal) {
    throw new Error(
      `task_554_line_count_failed:${filePath}:${result.error?.message ?? result.status ?? result.signal}`
    );
  }
  const count = Number.parseInt(result.stdout.trim(), 10);
  if (!Number.isSafeInteger(count) || count < 0)
    throw new Error(`task_554_line_count_invalid:${filePath}`);
  return count;
}
export function assertTask554LineLimit(root = ROOT, baseline = TASK_554_BASELINE_SHA) {
  ...
  for (const relativePath of listTask554LineCountCandidates(root, baseline)) {
    ...
    if (!stats.isFile() || stats.isSymbolicLink())
      throw new Error(`task_554_line_count_not_regular:${relativePath}`);
...
function releaseGateSnapshot(root) {
  const temporary = captureTask554TmpSnapshot(root);
  return Object.freeze({
    temporary,
...
function restoreReleaseGateReport(reportPath, expected) {
  const current = readStableTask554TmpFile(reportPath, "task_554_release_gate_report");
  if (!sameTask554TmpNode(current.node, expected.node)) {
    console.error("DIAG report_identity expected:", JSON.stringify(expected.node));
    console.error("DIAG report_identity current:", JSON.stringify(current.node));
    throw new Error("task_554_release_gate_report_identity_changed");
  }
  let descriptor;
  try {
    descriptor = openSync(
    ...
    const before = fstatSync(descriptor);
    if (!sameTask554TmpNode(task554TmpNode(before), expected.node))
      throw new Error("task_554_release_gate_report_identity_changed");
    ...
    if (
      !sameTask554TmpNode(expected.node, task554TmpNode(after)) ||
      !sameTask554TmpNode(expected.node, task554TmpNode(final))
    )
      throw new Error("task_554_release_gate_report_identity_changed");
  } finally {
    if (descriptor !== undefined) closeSync(descriptor);
...
function restoreReleaseGateSnapshot(root, expected) {
  ...
  if (
    ...
    throw new Error("task_554_release_gate_tmp_identity_changed");
  ...
  if (!expected.temporary.directory && existsSync(directory)) {
    ...
    if (!stats.isDirectory() || stats.isSymbolicLink() || readdirSync(directory).length !== 0)
      throw new Error("task_554_release_gate_tmp_not_empty");
    rmdirSync(directory);
  }
  {
    const finalActual = releaseGateSnapshot(root);
    if (!sameReleaseGateSnapshot(expected, finalActual)) {
      console.error("DIAG restore_failed expected:", JSON.stringify(expected.temporary.entries));
      console.error("DIAG restore_failed actual:", JSON.stringify(finalActual.temporary.entries));
      throw new Error("task_554_release_gate_report_restore_failed");
    }
  }
}
  ...
  try {
    ...
    if (primary)
      throw new AggregateError(
        [primary, restoration],
        "TASK-554 release gate and restoration failures"
      );
    throw restoration;
  }
...
function runRequiredCommand(root, entry) {
  if (entry.label === "task_554_frozen_install")
    throw new Error("task_554_frozen_install_must_run_first");
  if (entry.label === "task_554_coderso_release_gates")
    return runTask554ReleaseGate(root, () => runRequiredCommandDirect(root, entry));
  return runRequiredCommandDirect(root, entry);
}
function runIsolatedFrozenInstallFirst(root, entries) {
  const installs = entries.filter((entry) => entry.label === "task_554_frozen_install");
  if (installs.length > 1) throw new Error("task_554_frozen_install_duplicate");
  if (installs.length)
    runReadOnlyGate("task_554_isolated_frozen_install_mutated", root, () =>
      runTask554IsolatedFrozenInstall(root)
    );
  return entries.filter((entry) => entry.label !== "task_554_frozen_install");
}
function runOwnerGateCommands(root, ownerId) {
  ...
  return runReadOnlyGate(`task_554_owner_gate_mutated:${ownerId}`, root, () => {
    ...
    runRequiredCommand(
      root,
      Object.freeze({
        label: `task_554_owner_diff_${ownerId}`,
        command: "git",
...
const TASK_554_SMOKE_SESSIONS = Object.freeze({
  fast: "task-554-fast",
  certification: "task-554-certification",
});
function assertTask554SmokePair(profile, session) {
  if (profile !== "fast" && profile !== "certification")
    throw new Error("task_554_smoke_profile_invalid");
  if (session !== TASK_554_SMOKE_SESSIONS[profile])
    throw new Error(`task_554_smoke_profile_session_mismatch:${profile}:${session}`);
}
...
function task554SmokeDirectoryNode(stats) {
  return Object.freeze({ dev: stats.dev, ino: stats.ino, uid: stats.uid, mode: stats.mode });
}
function sameTask554SmokeDirectoryNode(left, right) {
  return (
    left.dev === right.dev &&
    left.ino === right.ino &&
    left.uid === right.uid &&
...
function assertNofollowTask554SmokeRoot(root, create = false, createdDirectories = null) {
  ...
  for (const component of ["_docs", "_workflows", "_smoke", "task-554"]) {
    ...
    if (!stats.isDirectory() || stats.isSymbolicLink())
      throw new Error("task_554_smoke_ancestor_invalid");
    if (created && createdDirectories !== null)
      createdDirectories.push(
        Object.freeze({ path: directory, node: task554SmokeDirectoryNode(stats) })
      );
  }
  return directory;
}
...
export function assertExactTask554Manifest(root, profile, session, manifest) {
  ...
  if (
    manifest.entries.length !== TASK_554_SMOKE_SCENARIO_IDS.length ||
    manifest.paths.length !== TASK_554_SMOKE_SCENARIO_IDS.length
  ) {
    throw new Error("task_554_smoke_manifest_count");
  }
  const sessionRelative = ensureInsideRoot(
    root,
    task554SessionDirectory(root, session),
    "smoke_session"
  );
  ...
  for (const [index, entry] of manifest.entries.entries()) {
    if (
      !entry ||
      entry.scenarioId !== TASK_554_SMOKE_SCENARIO_IDS[index] ||
  ...
  if (new Set(normalizedPaths).size !== normalizedPaths.length)
    throw new Error("task_554_smoke_manifest_duplicate");
  return Object.freeze({
...
function loadTask554Manifest(root, profile, session) {
  const manifestModule = pathToFileURL(
    path.join(root, "scripts/runtime-smoke/adapters/task-554/output-manifest.ts")
  ).href;
...
function collectSessionFiles(root, session) {
  ...
  if (!directoryStats.isDirectory() || directoryStats.isSymbolicLink())
    throw new Error("task_554_smoke_session_not_directory");
  const files = [];
  const walk = (absolute) => {
...
function readStableSmokeFile(root, relativePath, maximumBytes, label) {
  const absolute = path.resolve(root, relativePath);
  const smokeRoot = assertNofollowTask554SmokeRoot(root);
  const assertSession = () => {
  ...
  try {
    if (!Number.isInteger(constants.O_NOFOLLOW))
      throw new Error("task_554_smoke_nofollow_unsupported");
    descriptor = openSync(absolute, constants.O_RDONLY | constants.O_NOFOLLOW);
    const before = fstatSync(descriptor);
...
function assertBoundedPng(root, relativePath) {
  ...
  if (
    ...
    throw new Error(`task_554_smoke_png_invalid:${relativePath}`);
  ...
  if (
    ...
    throw new Error(`task_554_smoke_png_dimensions:${relativePath}`);
  return bytes;
}
function assertDecodedTask554Pngs(root, pngBytes) {
  ...
  const source = [
    ...
    "process.stdout.write(JSON.stringify(decodeTask554Png(bytes)));",
  ].join("\n");
...
const TASK_554_SUCCESS_TIMINGS = Object.freeze([
  ["cleanup", "all"],
  ["snapshot", "task554-after"],
  ["snapshot", "task554-before"],
  ["suite", "task-554"],
]);
const TASK_554_REMOVAL_KEYS = Object.freeze([
  "postChildrenRemoved",
  "accessLogsRemoved",
...
const TASK_554_CLEANUP_COUNT_KEYS = Object.freeze([
  ...TASK_554_REMOVAL_KEYS,
  "workerStarts",
  "workerRequests",
  "databaseBatches",
  "statements",
...
const TASK_554_CLEANUP_KEYS = Object.freeze([
  ...TASK_554_CLEANUP_COUNT_KEYS,
  "settingsRestored",
  "fixturesAbsent",
  "identitiesAbsent",
]);
...
function assertExactReport(report, profile, session, manifest, root) {
  ...
  if (
    ...
    report.timings.length !== TASK_554_SUCCESS_TIMINGS.length ||
    report.timings.some(
      (entry, index) =>
        !hasExactKeys(entry, ["kind", "name", "count", "failed", "elapsedMs"]) ||
        entry.kind !== TASK_554_SUCCESS_TIMINGS[index][0] ||
        entry.name !== TASK_554_SUCCESS_TIMINGS[index][1] ||
        entry.count !== 1 ||
        entry.failed !== 0 ||
        !isSafeCount(entry.elapsedMs)
    )
  )
    throw new Error("task_554_smoke_report_timings");
  const expectedProcesses = {
    git: 2,
    "playwright-close": 1,
    "playwright-open": 1,
    "playwright-run-code": fixtureCount,
    "playwright-state-load": 2,
    "task554-dev-host": 1,
    "task554-worker-db": 1,
  };
  if (
    ...
    throw new Error("task_554_smoke_report_processes");
  if (!Array.isArray(report.scenarios)) throw new Error("task_554_smoke_report:scenario_count");
  assertExactScenarioIds(
    report.scenarios.map((scenario) => scenario?.id),
    "task_554_smoke_report"
  );
  if (
    ...
    throw new Error("task_554_smoke_report_scenarios");
  const cleanup = report.suiteCleanup;
  const removalRows = hasExactKeys(cleanup, TASK_554_CLEANUP_KEYS)
    ? TASK_554_REMOVAL_KEYS.reduce((sum, key) => sum + cleanup[key], 0)
    : -1;
  if (
    !hasExactKeys(cleanup, TASK_554_CLEANUP_KEYS) ||
    TASK_554_CLEANUP_COUNT_KEYS.some((key) => !isSafeCount(cleanup[key])) ||
    cleanup.postsRemoved !== fixtureCount ||
    ...
    throw new Error("task_554_smoke_report_suite_cleanup");
  if (
    ...
    throw new Error("task_554_smoke_report_cleanup");
...
const TASK_554_SAFE_SMOKE_FAILURE_CODES = new Set([
...
function task554SmokeFailureCode(report, profile, session) {
  ...
  return Array.isArray(report.failures) &&
    ...
    TASK_554_SAFE_SMOKE_FAILURE_CODES.has(failure.code)
    ? failure.code
    : null;
}
function readTask554SmokeFailureCode(root, reportPath, profile, session) {
  try {
    const bytes = readStableSmokeFile(root, reportPath, 1_048_576, "task_554_smoke_report");
    ...
    return task554SmokeFailureCode(JSON.parse(text), profile, session);
  } catch {
    return null;
  }
}
export function assertExactTask554SmokeEvidence(root, profile, session, manifest, reportBytes) {
  ...
  if (!Buffer.from(reportText, "utf8").equals(bytes))
    throw new Error("task_554_smoke_report_utf8_invalid");
  if (!readStableSmokeFile(root, reportPath, 1_048_576, "task_554_smoke_report").equals(bytes))
    throw new Error("task_554_smoke_report_file_mismatch");
  let report;
  try {
...
function captureSmokeEvidenceSnapshot(root, evidence) {
  assertExactTask554SmokeEvidence(
    root,
  ...
  const smokeRoot = ensureInsideRoot(root, assertNofollowTask554SmokeRoot(root), "smoke_ancestor");
  const sessionPath = ensureInsideRoot(
    root,
    task554SessionDirectory(root, evidence.report.session),
    "smoke_session"
  );
  const reportPath = ensureInsideRoot(
    root,
    path.join(task554SessionDirectory(root, evidence.report.session), "report.json"),
    "smoke_report"
  );
  return new Map(
...
function assertSmokeEvidenceSnapshot(snapshot, root) {
  for (const [relativePath, value] of snapshot)
    if (fingerprintPath(root, relativePath) !== value)
      throw new Error(`task_554_smoke_evidence_changed:${relativePath}`);
}
...
function createEmptySmokeSession(root, session, createdDirectories = null) {
  ...
  const stats = lstatSync(directory);
  if (!stats.isDirectory() || stats.isSymbolicLink())
    throw new Error(`task_554_smoke_session_create_failed:${session}`);
  return directory;
}
function createOwnedTask554SmokeSession(root, session) {
  ...
  return Object.freeze({
    directory,
    node: task554SmokeDirectoryNode(stats),
...
function removeOwnedTask554SmokeDirectory(root, expected) {
  ...
  if (
    ...
    !sameTask554SmokeDirectoryNode(expected.node, task554SmokeDirectoryNode(stats))
  )
    throw new Error(`task_554_smoke_ancestor_changed:${directory}`);
...
function removeOwnedTask554FailedSmokeSession(root, session, owned) {
  ...
  if (owned.directory !== directory || path.dirname(directory) !== smokeRoot)
    throw new Error(`task_554_smoke_session_ownership_invalid:${session}`);
  let stats;
  try {
    stats = lstatSync(directory);
  } catch (error) {
  ...
  if (stats !== null) {
    if (
      !stats.isDirectory() ||
      stats.isSymbolicLink() ||
      !sameTask554SmokeDirectoryNode(owned.node, task554SmokeDirectoryNode(stats))
    )
      throw new Error(`task_554_smoke_session_changed:${session}`);
  ...
  for (const expected of [...owned.createdDirectories].reverse())
    removeOwnedTask554SmokeDirectory(root, expected);
}
function assertByteIdenticalReport(expectedBytes, reportPath) {
  ...
  if (!Buffer.from(expectedBytes).equals(actualBytes))
    throw new Error("task_554_smoke_report_not_stdout_identical");
}
function preserveSmokePrimaryFailure(primary, restoration) {
  ...
  return new Error(primary instanceof Error ? primary.message : String(primary), {
    cause: new AggregateError(
      [primary, restoration],
      "TASK-554 smoke primary and restoration failures"
    ),
  });
}
function restoreFailedTask554SmokeRun(root, session, ownedFailedSession, before, primary) {
  ...
  try {
    if (ownedFailedSession !== null)
      removeOwnedTask554FailedSmokeSession(root, session, ownedFailedSession);
  } catch (restoration) {
    failure = preserveSmokePrimaryFailure(failure, restoration);
  }
  try {
    assertNoRepositoryMutation(
      "task_554_smoke_repository_restoration",
      before,
...
function finalizeTask554SmokeProfile(
  root,
  session,
  ownedFailedSession,
  before,
  ...
  try {
    if (evidenceSnapshot === null || evidence === null)
      throw new Error("task_554_smoke_evidence_missing");
    assertExactTask554SmokeEvidence(
      root,
      evidence.report.profile,
      evidence.report.session,
  ...
  if (failure !== null)
    return restoreFailedTask554SmokeRun(root, session, ownedFailedSession, before, failure);
  try {
    assertNoRepositoryMutation(
      "task_554_smoke_repository_restoration",
      before,
      captureRepositoryFingerprint(root, smokeEvidencePaths(evidenceSnapshot)),
      root
    );
  } catch (restoration) {
    failure = preserveSmokePrimaryFailure(failure, restoration);
  ...
  return failure === null && evidenceRevalidated
    ? null
    : restoreFailedTask554SmokeRun(root, session, ownedFailedSession, before, failure);
}
export function runTask554SmokeProfile(root, profile, session) {
  ...
  try {
    ...
    if (execution?.error || execution?.status !== 0 || execution?.signal)
      throw new Error(
        `task_554_smoke_runner_failed:${readTask554SmokeFailureCode(root, ensureInsideRoot(root, reportPath, "smoke_report"), profile, session) ?? "report_invalid"}`
      );
    assertNofollowTask554SmokeRoot(root);
    ...
    primary = finalizeTask554SmokeProfile(
      root,
      session,
      ownedFailedSession,
      before,
...
function removeFastSmokeEvidence(root) {
  ...
  if (directory !== expected || !existsSync(directory))
    throw new Error("task_554_fast_evidence_missing_before_cleanup");
  const stats = lstatSync(directory);
  if (!stats.isDirectory() || stats.isSymbolicLink())
    throw new Error("task_554_fast_evidence_not_owned_directory");
  rmSync(directory, { recursive: true, force: false });
  try {
    lstatSync(directory);
  } catch (error) {
  ...
  throw new Error("task_554_fast_evidence_cleanup_failed");
}
export function runTask554SmokeSequence(
  root = ROOT,
  operations = Object.freeze({
    runProfile: runTask554SmokeProfile,
...
async function dispatchAudit(phaseName, identity, prompt) {
  ...
  try {
    ...
    assertNoRepositoryMutation(
      `task_554_read_only_audit_mutated:${identity}`,
      before,
      captureRepositoryFingerprint()
    );
  }
...
cross-stream collision risks. Return no findings only when the lens is actually clean.`
  ...
  if (checks.length !== POST_AUDIT_LENSES.length)
    throw new Error("task_554_post_audit_missing_results");
...
function makeSelfTestManifest(root, session) {
  const directory = task554SessionDirectory(root, session);
  const entries = TASK_554_SMOKE_SCENARIO_IDS.map((scenarioId, index) =>
    Object.freeze({
      scenarioId,
...
function workflowSelfTest() {
  ...
  try {
    ...
    if (!sameReleaseGateSnapshot(releaseAbsent, releaseGateSnapshot(tempRoot)))
      throw new Error("task_554_self_test_release_gate_created_report");
    ...
    if (!sameReleaseGateSnapshot(releasePresent, releaseGateSnapshot(tempRoot)))
      throw new Error("task_554_self_test_release_gate_existing_report");
    ...
    expectFailure(
      () => runTask554ReleaseGate(tempRoot, () => writeTinyFile(unexpectedSibling, "unexpected\n")),
      "task_554_release_gate_report_restore_failed"
    );
    unlinkSync(unexpectedSibling);
    {
      const afterUnlink = releaseGateSnapshot(tempRoot);
      if (!sameReleaseGateSnapshot(releaseWithSibling, afterUnlink)) {
        console.error(
          "DIAG sibling expected:",
          JSON.stringify(releaseWithSibling.temporary.entries)
        );
        console.error("DIAG sibling afterUnlink:", JSON.stringify(afterUnlink.temporary.entries));
        throw new Error("task_554_self_test_release_gate_sibling");
      }
    }
    ...
    expectFailure(
      () =>
        assertNoRepositoryMutation(
          "task_554_self_test_tmp",
          tmpMutationBefore,
          captureRepositoryFingerprint(tempRoot),
          tempRoot
        ),
      "task_554_self_test_tmp:scope_violation:"
    );
    ...
    expectFailure(() => captureTask554TmpSnapshot(tempRoot), "task_554_tmp_entry_invalid");
    rmSync(path.join(tempRoot, ".tmp"), { recursive: true, force: true });
    ...
    expectFailure(
      ...
      "task_554_release_gate_tmp_identity_changed"
    );
    rmSync(path.join(tempRoot, ".tmp"), { recursive: true, force: true });
    ...
    expectFailure(
      () =>
        runTask554ReleaseGate(tempRoot, () => {
          renameSync(releaseReport, `${releaseReport}.moved`);
          writeTinyFile(releaseReport, '{"replaced":true}\n');
        }),
      "task_554_release_gate_report_identity_changed"
    );
    if (!reportIdentity.report) throw new Error("task_554_self_test_release_gate_report_identity");
    rmSync(path.join(tempRoot, ".tmp"), { recursive: true, force: true });
    ...
    expectFailure(
      ...
      "task_554_line_limit:scripts/too-long.ts:1001"
    );
    rmSync(path.join(tempRoot, "scripts/too-long.ts"));
    writeTinyFile(path.join(tempRoot, "scripts/exempt.generated.ts"), `${"x\n".repeat(1001)}`);
    if (
      listTask554LineCountCandidates(tempRoot, baseline).includes("scripts/exempt.generated.ts")
    ) {
    ...
    const certificationInvocation = task554SmokeInvocation(
      "certification",
      "task-554-certification"
    );
    if (
      fastInvocation.profile !== "fast" ||
      fastInvocation.session !== "task-554-fast" ||
      certificationInvocation.profile !== "certification" ||
      certificationInvocation.session !== "task-554-certification"
    )
      throw new Error("task_554_self_test_manifest_input_binding");
    expectFailure(
      () => task554SmokeInvocation("fast", "task-554-certification"),
      "task_554_smoke_profile_session_mismatch:fast:task-554-certification"
    );
    ...
    expectFailure(
      () =>
        requirePass("task_554_self_test_result", validResultIdentity, {
          ...validResult,
          identity: validResultIdentity,
        }),
      "task_554_self_test_result:invalid_result:"
    );
    ...
    expectFailure(
      () =>
        requireCleanAudit("task_554_self_test_final_drift", validAuditIdentity, {
          ...validAudit,
          findings: [
            {
              severity: "LOW",
      ...
      "task_554_self_test_final_drift:invalid_finding:"
    );
    ...
    expectFailure(
      () =>
        runReadOnlyGate("task_554_self_test_gate", tempRoot, () =>
          writeTinyFile(
    ...
    expectFailure(
      () =>
        assertNoRepositoryMutation(
          "task_554_self_test_ignored",
          ignoredBefore,
          captureRepositoryFingerprint(tempRoot),
          tempRoot
        ),
      "task_554_self_test_ignored:scope_violation:"
    );
    ...
    expectFailure(
      () =>
        assertNoRepositoryMutation(
          "task_554_self_test_mode",
          modeBefore,
          captureRepositoryFingerprint(tempRoot),
          tempRoot
        ),
      "task_554_self_test_mode:scope_violation:"
    );
    ...
    expectFailure(
      () =>
        assertNoRepositoryMutation(
          "task_554_self_test_symlink",
          symlinkBefore,
          captureRepositoryFingerprint(tempRoot),
          tempRoot
        ),
      "task_554_self_test_symlink:scope_violation:"
    );
    unlinkSync(linkPath);
    ...
    for (const action of [
      () => createEmptySmokeSession(tempRoot, "task-554-fast"),
      () => collectSessionFiles(tempRoot, "task-554-fast"),
    ...
    if (readdirSync(externalSmokeRoot).length !== 0)
      throw new Error("task_554_self_test_smoke_ancestor_symlink");
    ...
    const failedSmokeSession = Object.freeze({
      ...
      node: task554SmokeDirectoryNode(failedSmokeStats),
      createdDirectories: Object.freeze(failedSmokeDirectories),
    });
    expectFailure(
      () =>
        assertNoRepositoryMutation(
          "task_554_self_test_failed_empty_smoke",
          failedSmokeBefore,
          captureRepositoryFingerprint(tempRoot),
          tempRoot
        ),
      "task_554_self_test_failed_empty_smoke:scope_violation:"
    );
    removeOwnedTask554FailedSmokeSession(tempRoot, "task-554-certification", failedSmokeSession);
    assertNoRepositoryMutation(
      "task_554_self_test_failed_smoke_restored",
      failedSmokeBefore,
      captureRepositoryFingerprint(tempRoot),
      tempRoot
    );
    ...
    writeTinyFile(
      path.join(tempRoot, "scripts/runtime-smoke/adapters/task-554/output-manifest.ts"),
      `export function decodeTask554Png(bytes: Uint8Array) { if (bytes.byteLength !== ${pngBytes.byteLength} || bytes[12] !== 73 || bytes[13] !== 72 || bytes[14] !== 68 || bytes[15] !== 82) throw new Error("invalid_png"); const read = (offset: number) => (((bytes[offset] << 24) | (bytes[offset + 1] << 16) | (bytes[offset + 2] << 8) | bytes[offset + 3]) >>> 0); return { width: read(16), height: read(20) }; }\n`
    );
    const smokeBefore = captureRepositoryFingerprint(tempRoot);
    const ownedEvidenceSession = createOwnedTask554SmokeSession(tempRoot, session);
    ...
    const reportValue = {
      ...
      serverUp: true,
      snapshots: 2,
      timings: TASK_554_SUCCESS_TIMINGS.map(([kind, name]) => ({
        kind,
        name,
        count: 1,
        failed: 0,
    ...
    const evidence = assertExactTask554SmokeEvidence(
      tempRoot,
      "fast",
      session,
      manifest,
    ...
    assertNoRepositoryMutation(
      "task_554_self_test_validated_smoke",
      smokeBefore,
    ...
    const reportMutations = [
      ...
      (value) => {
        value.suiteCleanup.postsRemoved = 6;
      },
      (value) => {
    ...
    Object.assign(certificationReport, {
      ...
      session: "task-554-certification",
      processes: { ...certificationReport.processes, "playwright-run-code": 28 },
      suiteCleanup: {
    ...
    const fakeBunDirectory = path.join(tempRoot, ".task-554-fake-bun");
    const fakeManifest = makeSelfTestManifest(tempRoot, "task-554-certification");
    writeTinyFile(
      path.join(tempRoot, AUTHOR_AUDIT_PATH),
      `if (process.argv[2] === "--task-554-bootstrap-verify") process.stdout.write(${JSON.stringify(JSON.stringify({ baseline: TASK_554_BASELINE_SHA, paths: TASK_554_WORKFLOW_PATHS }))});\n`
    );
    const failureReport = (code) =>
      `${JSON.stringify({ schemaVersion: 1, suiteId: "task-554", profile: "certification", session: "task-554-certification", pass: false, serverUp: false, timings: [], processes: {}, snapshots: 0, scenarios: [], screenshots: [], consoleErrors: [], suiteCleanup: {}, cleanup: { pass: true, failures: [] }, failures: [{ code }] })}\n`;
    ...
    const previousReport = process.env.TASK_554_SELF_TEST_REPORT;
    process.env.PATH = `${fakeBunDirectory}:${previousPath ?? ""}`;
    try {
      process.env.TASK_554_SELF_TEST_REPORT = failureReport("smoke_process_failed");
      expectFailure(
        () => runTask554SmokeProfile(tempRoot, "certification", "task-554-certification"),
        "task_554_smoke_runner_failed:smoke_process_failed"
      );
      process.env.TASK_554_SELF_TEST_REPORT = "{}\n";
      expectFailure(
        () => runTask554SmokeProfile(tempRoot, "certification", "task-554-certification"),
        "task_554_smoke_runner_failed:report_invalid"
      );
      process.env.TASK_554_SELF_TEST_REPORT = failureReport("smoke_unknown");
      expectFailure(
        () => runTask554SmokeProfile(tempRoot, "certification", "task-554-certification"),
        "task_554_smoke_runner_failed:report_invalid"
      );
      ...
      if (previousReport === undefined) delete process.env.TASK_554_SELF_TEST_REPORT;
      else process.env.TASK_554_SELF_TEST_REPORT = previousReport;
    }
    if (existsSync(task554SessionDirectory(tempRoot, "task-554-certification")))
      throw new Error("task_554_self_test_failed_runner_session_residue");
    assertNoRepositoryMutation(
      "task_554_self_test_failed_runner_restored",
      runnerBefore,
      captureRepositoryFingerprint(tempRoot),
      tempRoot
    );
    ...
    expectFailure(
      () =>
        assertExactTask554SmokeEvidence(tempRoot, "fast", session, manifest, evidence.reportBytes),
      "task_554_smoke_report_file_mismatch"
    );
    ...
    expectFailure(
      ...
      "task_554_smoke_png_decode_invalid"
    );
    expectFailure(
      () => assertSmokeEvidenceSnapshot(evidenceSnapshot, tempRoot),
      "task_554_smoke_evidence_changed:"
    );
    ...
    expectFailure(
      () =>
        assertNoRepositoryMutation(
          "task_554_self_test_smoke_sibling",
          smokeBefore,
          captureRepositoryFingerprint(tempRoot, [...evidenceSnapshot.keys()]),
          tempRoot
        ),
      "task_554_self_test_smoke_sibling:scope_violation:"
    );
    ...
    expectFailure(
      ...
      "task_554_smoke_report_identity"
    );
    ...
    expectFailure(
      ...
      "task_554_smoke_output_nested_directory:"
    );
    ...
    expectFailure(
      ...
      "task_554_smoke_report_not_stdout_identical"
    );
    chmodSync(path.join(tempRoot, manifest.paths[0]), 0o600);
    const revalidationFailure = finalizeTask554SmokeProfile(
      tempRoot,
      session,
    ...
    if (existsSync(sessionDirectory))
      throw new Error("task_554_self_test_failed_evidence_revalidation_session_residue");
    const boardBefore = [
      "- **To Do:** 1 tasks",
      "- **In Progress:** 2 tasks",
      "- **Done:** 3 tasks",
      "## In Progress",
      "| ID |",
      "| TASK-554 | title | priority | effort | In progress 2026-08-11. details |",
      "## Done",
      "| ID |",
      "| TASK-999 | retained |",
    ].join("\n");
    const boardAfter = [
      ...
      "| ID |",
      "| TASK-999 | retained |",
      "| TASK-554 | title | priority | effort | ✅ Done (2026-08-11): details |",
    ].join("\n");
    assertTask554BoardClosureDelta(boardBefore, boardAfter);
    expectFailure(
      () => assertTask554BoardClosureDelta(boardBefore, boardAfter.replace("TASK-999", "TASK-998")),
      "task_554_closure_board_scope_invalid"
    );
    expectFailure(
      () =>
        assertTask554BoardClosureDelta(
      ...
      "task_554_closure_board_statistics_duplicate"
    );
    ...
    expectFailure(
      () =>
        assertTask554ChangelogClosureDelta(
          ...
          indexAfter,
          null,
          Buffer.concat([CHANGELOG_1267_ENTRY_BYTES, Buffer.from("unrelated\n")])
        ),
    ...
    if (
      ...
      throw new Error("task_554_self_test_smoke_primary_preserved");
    assertTask554TerminalStatusDelta(
      "**Status:** 🚧 In Progress\n**Started:** 2026-08-11",
    ...
    expectFailure(
      ...
      "task_554_closure_terminal_status_invalid"
    );
    return Object.freeze({
      pass: true,
...
if (importedForVerification && isDirectInvocation())
  throw new Error("task_554_workflow_import_direct_invocation");
