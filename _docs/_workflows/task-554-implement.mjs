...
export const OWNER_GATE_COMMANDS = Object.freeze({
  ...
  "admin-client": Object.freeze([command("task_554_client_vitest", "bunx", ["vitest", "run", "--config", "vitest.config.ts", "tests/vitest/admin/postsClient.test.ts", "tests/vitest/admin/postsClientCacheAuthority.test.ts"]), command("task_554_types", "bun", ["--cwd", "core", "lint:types"]), command("task_554_lint", "bun", ["--cwd", "core", "lint"])]), "classic-metadata-ui": Object.freeze([command("task_554_ui_vitest", "bunx", ["vitest", "run", "--config", "vitest.config.ts", "tests/vitest/ui/post-metadata-mutation-payload.test.ts", "tests/vitest/ui/post-external-update-authority.test.ts", "tests/vitest/ui/post-classic-editor-shell-wave.test.tsx", "tests/vitest/ui/post-classic-metadata-hydration.test.tsx", "tests/vitest/ui/post-editor-state-metadata-boundary.test.ts"]), command("task_554_types", "bun", ["--cwd", "core", "lint:types"]), command("task_554_lint", "bun", ["--cwd", "core", "lint"])]),
...
function assertExactReport(report, profile, session, manifest, root) {
  ...
  const expectedProcesses = { git: 2, "playwright-close": 1, "playwright-open": 1, "playwright-run-code": fixtureCount + 1, "playwright-state-load": 3, "task554-dev-host": 1, "task554-worker-db": 1 };
...
function finalizeTask554SmokeProfile(root, session, ownedFailedSession, before, evidenceSnapshot, evidence, primary) {
  ...
  try { const allowed = smokeEvidencePaths(evidenceSnapshot, root, session); const after614 = captureRepositoryFingerprint(root, allowed); assertScopedRepositoryMutation("task_554_smoke_repository_restoration", before, after614, allowed, root); } catch (restoration) { failure = preserveSmokePrimaryFailure(failure, restoration); }
...
function workflowSelfTest() {
  ...
  try {
    ...
    const reportValue = { schemaVersion: 1, suiteId: "task-554", profile: "fast", session, pass: true, serverUp: true, snapshots: 2,
      timings: TASK_554_SUCCESS_TIMINGS.map(([kind, name]) => ({ kind, name, count: 1, failed: 0, elapsedMs: 1 })), processes: { git: 2, "playwright-close": 1, "playwright-open": 1, "playwright-run-code": 8, "playwright-state-load": 3, "task554-dev-host": 1, "task554-worker-db": 1 },
    ...
    const fakeBun = path.join(fakeBunDirectory, "bun"); writeTinyFile(fakeBun, `#!/usr/bin/env node\nconst fs = require("fs");\nif (process.argv[2] === "--eval") process.stdout.write(${JSON.stringify(JSON.stringify(fakeManifest))}); else { const session = process.argv[process.argv.indexOf("--session") + 1]; const reportPath = \`_docs/_workflows/_smoke/task-554/\${session}/report.json\`; fs.writeFileSync(reportPath, process.env.TASK_554_SELF_TEST_REPORT ?? ""); process.exit(1); }\n`); chmodSync(fakeBun, 0o755);
