...
function finalizeTask554SmokeProfile(root, session, ownedFailedSession, before, evidenceSnapshot, evidence, primary) {
  ...
  try { const excluded = smokeEvidencePaths(evidenceSnapshot, root, session); const after614 = captureRepositoryFingerprint(root, excluded); const changed614 = [...after614.keys()].filter((p) => before.get(p) !== after614.get(p)); console.error(`[614] changed=${JSON.stringify(changed614)} excluded=${JSON.stringify(excluded)}`); assertNoRepositoryMutation("task_554_smoke_repository_restoration", before, after614, root); } catch (restoration) { failure = preserveSmokePrimaryFailure(failure, restoration); }
