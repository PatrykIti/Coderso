export const meta = Object.freeze({
  name: "task-486-implement",
  description:
    "Implement TASK-486 leaves sequentially with per-leaf gates, post-audit, runtime smoke and closure",
...
TASK-486 implementation is allowed only after TASK-414, TASK-489, TASK-547 and TASK-555 are terminal
...
export const result = await (async () => {
  ...
  Do not edit.`,
      { label: "task-486:start-gate", phase: "Start gate", schema: RESULT_SCHEMA }
    )
  );
  ...
  for (const leaf of PRODUCT_LEAVES) {
    ...
    Return pass only when the owned diff and all required receipts are green.`,
        { label: `task-486:implement:${leaf}`, phase: "Sequential leaves", schema: RESULT_SCHEMA }
      )
    );
    ...
    skipped command. Do not edit.`,
        { label: `task-486:gate:${leaf}`, phase: "Sequential leaves", schema: RESULT_SCHEMA }
      )
    );
  ...
  const closureHarness = requirePass(
    "implement:TASK-486-04-L02:harness",
    await agent(
      `${COMMON}\nImplement only TASK-486-04-L02's runtime-smoke adapter, worker/database/browser/cleanup
  ...
  task statuses yet. Run the focused harness tests, lint/types, line counts and git diff --check.`,
      {
        label: "task-486:implement:TASK-486-04-L02:harness",
        phase: "Sequential leaves",
  ...
  const closureHarnessGate = requirePass(
    "gate:TASK-486-04-L02:harness",
    await agent(
      `${COMMON}\nRead-only review of the TASK-486-04-L02 harness diff. Verify exact five IDs, shared lifecycle
  reuse, strict visible evidence, owner-scoped cleanup, no product source and green focused gates.`,
      {
        label: "task-486:gate:TASK-486-04-L02:harness",
        phase: "Sequential leaves",
        schema: RESULT_SCHEMA,
      }
    )
      ...
      auditPrompts.map(
        (prompt, index) => () =>
          agent(
            ...
            { label: `task-486:post-audit:${index + 1}`, phase: "Post-audit", schema: AUDIT_SCHEMA }
          )
      )
    )
  );
  ...
  const smoke = requireSmoke(
    await agent(
      `${COMMON}\nRun the registered task-486 fast and certification profiles from a restarted healthy host.
   ...
   request errors, reviewed screenshots and complete owner-scoped cleanup. Do not repair product source.`,
      { label: "task-486:runtime-smoke", phase: "Runtime smoke", schema: SMOKE_SCHEMA }
    )
  );
  ...
  const closure = requirePass(
    "closure",
    await agent(
      `${COMMON}\nImplement and execute TASK-486-04-L02 only after all receipts above are current: run full mandatory
  gates, update only required docs, changelog 1255/index, TASK-486 board/statistics and all 13 statuses.
  Re-read shared indexes immediately before edits. Never edit product source, stage or commit.`,
      { label: "task-486:closure", phase: "Closure", schema: RESULT_SCHEMA }
    )
  );
  ...
  terminal statuses, changelog 1255, board statistics, docs, validated code, security invariants,
  smoke evidence and cleanup. Report every finding.`,
    { label: "task-486:final-drift", phase: "Final drift", schema: AUDIT_SCHEMA }
  );
  requireAuditResults("task-486-final-drift", 1, [finalDrift]);
  ...
  return Object.freeze({
    pass: true,
    summary: "TASK-486 implementation workflow complete.",
    start,
    leafResults,
    closureHarness,
    closureHarnessGate,
