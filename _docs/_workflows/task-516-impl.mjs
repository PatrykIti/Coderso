...
const WT = (typeof args === "string" ? JSON.parse(args) : args)?.wt || "/home/coder/project/Coderso-task-516";
...
const CLOSURE_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["done", "changelogFile", "docsUpdated", "boardUpdated", "gates", "committed", "commitSha", "notes"],
  properties: {
    done: { type: "boolean" },
...
const SUBTASKS = [
  { id: "516-01", file: "TASK-516-01-Form-Theme-Model-And-Resolver.md", owns: "core/services/forms/formSettings.ts, NEW core/services/forms/formTheme.ts (theme model + enum unions + clamp sets + resolveFormTheme — the ONE source of truth), core/admin/services/formsClient.ts (FormSettings type mirror)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:bun + test:vitest for the form settings/theme tests" },
  { id: "516-02", file: "TASK-516-02-Form-Design-Inspector-Panel.md", owns: "NEW core/admin/ui/forms/FormDesignPanel.tsx (design inspector — imports theme vocab read-only from 516-01)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:vitest for the design panel tests" },
  { id: "516-04", file: "TASK-516-04-Canvas-Fidelity-And-Preview-Fixes.md", owns: "core/admin/ui/forms/FormCanvas.tsx (canvas fidelity + field-preview fixes + theme apply; ADD optional deviceWidth?/theme? props to FormCanvasProps so 516-03 can wire them — no-op until supplied)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:vitest for the form canvas tests" },
  { id: "516-03", file: "TASK-516-03-Builder-Chrome-And-Rail-Fidelity.md", owns: "core/admin/ui/forms/FormBuilderPage.tsx, core/admin/ui/forms/FieldLibrary.tsx (builder chrome + rail fidelity + mount the 516-02 FormDesignPanel + pass deviceWidth/theme to FormCanvas; STOP rendering FieldListPanel but leave FieldListPanel.tsx file unchanged)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:vitest for the form builder page tests" },
  { id: "516-05", file: "TASK-516-05-Field-Settings-Control-Fixes.md", owns: "core/admin/ui/forms/FieldSettingsPanel.tsx, core/services/forms/fieldSettings.ts (field settings control fixes B1 wiring/B4/B5/B6)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:vitest for the field settings tests" },
  { id: "516-06", file: "TASK-516-06-Runtime-Theme-Application.md", owns: "core/admin/ui/forms/FormRuntimePreviewDialog.tsx, core/widgets/core/formEmbed.tsx, AND the per-region mapFormBindingToEmbedData block in core/services/pages/pageRendererV2.tsx (~:1329-1361 — present-only `theme` passthrough ONLY; the projection currently drops theme so the public embed renders un-themed)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:vitest + test:bun for the runtime theme / page renderer form-embed tests" },
  { id: "516-07", file: "TASK-516-07-File-Field-Type.md", owns: "core/services/forms/validation.ts, core/services/forms/submissionService.ts; NEW core/services/forms/formAttachment.ts + core/services/forms/mimeMatchesAccept.ts; core/server/routes/formsRoutes.ts (submission branch + NEW public route POST /forms/:id/uploads handleFormAttachmentUploadRoute + additive mapFormError media cases); NEW formAttachmentUploadSchema in core/server/validation/formSchemas.ts; additive edits to core/services/media/mediaService.ts (uploadMedia `constraints?` param) + core/services/media/mediaUsageService.ts (\"submission\" MediaUsageTargetType + form_submissions scan); PLUS the documented additive file-case-ONLY seam edits to FieldLibrary.tsx/FormBuilderPage.tsx (rail item), FormCanvas.tsx (preview), formEmbed.tsx (control), FormRuntimePreviewDialog.tsx (control)", gates: "bun --cwd core lint; bun --cwd core lint:types; ./node_modules/.bin/tsc -p tsconfig.json --noEmit; bun run test:bun (upload route: mime/size constraints enforced, reject-unknown, unauthorized/oversized/wrong-mime rejected, media usage tracked as submission) + test:vitest for the file-field UI" },
];
...
Return the structured result. In notes, include anything the NEXT sequential subtask needs (theme enum/type names + resolveFormTheme signature, new component names + props, FormCanvas prop additions, route path/schema names).`,
  ...
  if (!r?.done) {
    log(`STOP: ${st.id} did not land green — halting (later subtasks consume it). Resume with resumeFromRunId after diagnosis.`);
...
log(`Closure: done=${closure?.done} changelog=${closure?.changelogFile} committed=${closure?.committed} sha=${closure?.commitSha}`);
