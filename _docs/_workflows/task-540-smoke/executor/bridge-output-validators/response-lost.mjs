...
export function createResponseLostBridgeOutputValidators({
  RESPONSE_LOST_QUERY_OPERATION_BINDINGS,
}) {
  // The query-operation binding table is owned by the response-lost registry, which
  // only exists once its Bun bridge authority is constructed, so the facade injects
  // the frozen table here and every validator below closes over that one value.
  invariant(
    typeof RESPONSE_LOST_QUERY_OPERATION_BINDINGS === "object" &&
      RESPONSE_LOST_QUERY_OPERATION_BINDINGS !== null,
    "response-lost query operation bindings are absent"
  );
  assertRecursivelyFrozen(RESPONSE_LOST_QUERY_OPERATION_BINDINGS);
