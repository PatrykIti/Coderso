...
} from "./response-lost.mjs";

// The API-session and bootstrap-login observation validators belong to runtime
// authorities that only exist once the facade has constructed them, so they cannot
// be imported statically. The facade binds them into this module exactly once,
// before any bridge output validator can run.
let validateApiSessionObservation = null;
let validateBootstrapLoginObservation = null;

function bindResourceBridgeObservationValidators(validators) {
  invariant(
    validateApiSessionObservation === null && validateBootstrapLoginObservation === null,
    "resource bridge observation validators are already bound"
  );
  exactOwnKeys(
    validators,
    ["validateApiSessionObservation", "validateBootstrapLoginObservation"],
    "resource bridge observation validators",
    { plain: true }
  );
  invariant(
    typeof validators.validateApiSessionObservation === "function" &&
      typeof validators.validateBootstrapLoginObservation === "function",
    "resource bridge observation validators are not callable"
  );
  validateApiSessionObservation = validators.validateApiSessionObservation;
  validateBootstrapLoginObservation = validators.validateBootstrapLoginObservation;
}
