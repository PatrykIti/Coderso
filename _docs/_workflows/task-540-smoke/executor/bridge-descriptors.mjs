...
export function createBunBridgeDescriptors(dependencies) {
  ...
  // injects the frozen registry here and every descriptor built or validated below
  // closes over that one value, so this module keeps no mutable state.
  exactOwnKeys(dependencies, ["BUN_BRIDGE_OUTPUT_VALIDATORS"], "Bun bridge descriptor registry", {
    plain: true,
  });
  invariant(
    Object.isFrozen(dependencies.BUN_BRIDGE_OUTPUT_VALIDATORS) &&
      Object.values(dependencies.BUN_BRIDGE_OUTPUT_VALIDATORS).every(
        (validator) => typeof validator === "function"
      ),
    "Bun bridge output validator registry binding drift"
  );
  const { BUN_BRIDGE_OUTPUT_VALIDATORS } = dependencies;
