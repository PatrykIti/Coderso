export const meta = {
  name: "task-483-author-audit",
  description:
    "TASK-483 drift-audit loop: 5 sequential rounds of per-subtask audits + cross-subtask reconcile + fixers, then residual protocol",
  phases: [
    { title: "Round 1" },
    { title: "Round 2" },
    { title: "Round 3" },
...
const GROUPS = [
  { id: "parent", files: ["TASK-483_Real_Web_Analytics_Pipeline.md"] },
  {
    id: "483-01",
    files: [
  ...
  },
  {
    ...
    files: [
      ...
      "TASK-483-06-L02-Test-Matrix-And-Documentation-Closure.md",
    ],
  },
];

const PINS = `
PINNED COORDINATION FACTS for the TASK-483 stream (task files must state these; a missing or contradicting pin is a HIGH finding):
- Pinned changelog number: 1221 (closure creates _docs/_CHANGELOG/1221-*.md). Numbers 1219 (TASK-510, in flight in the shared main tree — may be absent from this worktree's checkout, do NOT reallocate it), 1220 (TASK-482) and 1222 (TASK-484) are RESERVED by parallel streams.
- Pinned migration index: 0064. The current max migration in this worktree is 0063_yummy_glorian.sql. TASK-483 owns 0064 and its stream merges BEFORE TASK-484 (which is pinned to 0065). Full artifacts required per AGENTS.md: SQL migration file + meta/0064_snapshot.json + meta/_journal.json entry. Any 483 file claiming a different index, or claiming "renumber at merge", must be corrected to a firm 0064.
- Parallel streams: TASK-482 (setup wizard, worktree /home/coder/project/Coderso-task-482) and TASK-484 (backups, /home/coder/project/Coderso-task-484) run concurrently on sibling branches. FORBIDDEN PATHS for TASK-483: core/services/backups/**, backup route modules, core/admin/ui/setup/**, auth/install route surfaces, usersService first-admin logic.
- Shared surfaces that all three streams touch ADDITIVELY (contracts must scope edits to their own sections/lines and must not restructure): the server route registration module (verify its real filename in this worktree), the security-gate test expectations under tests/security/, _docs/CMS_API.md, _docs/SECURITY_SPEC.md, _docs/DATA_MODEL.md (483 and 484 both add sections — own section only), core/db/schema.ts (483 adds ONLY its analytics tables; 484 will separately touch backup tables — 483 files must not reserve or restructure anything beyond its own additions).
- Shared REMOTE test database: all three streams and the owner share ONE Postgres (render.com, DATABASE_URL in .env). All DB-backed test contracts must use uniquely scoped fixtures and clean up only rows they created; truncating/deleting whole shared tables is a HIGH finding. Ingestion/aggregation tests must not depend on global table emptiness.
- Board/changelog discipline: ONLY the closure leaf (TASK-483-06-L02) edits _docs/_TASKS/README.md and _docs/_CHANGELOG/*; it touches only TASK-483 rows and its own statistics deltas. Implementation subtasks never touch them.
- Land order: 01 -> 02 -> 03 -> 04 -> 05 -> 06, strictly sequential, single writer per source file.
- Public ingestion is a PUBLIC WRITE endpoint: the Security Contract must follow the shared access-evaluator + nonce/HMAC anti-abuse patterns that already exist in forms/booking (verify the real helpers in source); no weaker one-off flow. IP/PII redaction and bot/DNT classification are part of the contract, not optional.
`;
...
const COMMON = `
You work EXCLUSIVELY inside the git worktree ${WT} (branch feature/task-483, HEAD fbe93dae). Never read from or write to /home/coder/project/Coderso or the sibling task-482/task-484 worktrees.
...
Also read the parent file TASK-483_Real_Web_Analytics_Pipeline.md for context (report contradictions with it against YOUR files).
...
function crossFixerPrompt(findings, round) {
  return `You are the single CROSS-FILE drift fixer (round ${round}) for the ${TREE} tree. You may edit any TASK-483* file in ${TASKS_DIR} (and nothing else — no source, no board README, no changelog).
${COMMON}
...
${COMMON}
Read ALL TASK-483* files in ${TASKS_DIR} as one set. Name the RESIDUAL contradictions precisely (which file must change, to what exact value, and why that direction is correct given the owning subtask). Ignore anything already consistent. Return only findings that still hold on the current file state.`;
}
