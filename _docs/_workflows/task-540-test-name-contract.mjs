...
const EXPECTED_MODULE_BASENAME = "task-540-test-name-contract.mjs";
const ROOT_DERIVATION_DEPENDENCY_KEYS = Object.freeze([
  "execFile",
  "fileURLToPath",
  "lstat",
  "readFile",
...
const ROOT_RUNTIME_DEPENDENCY_KEYS = Object.freeze(["lstat", "realpath"]);
const ROOT_GIT_ENVIRONMENT = Object.freeze(
  Object.assign(Object.create(null), {
    GIT_CONFIG_GLOBAL: "/dev/null",
    GIT_CONFIG_NOSYSTEM: "1",
    GIT_OPTIONAL_LOCKS: "0",
    GIT_TERMINAL_PROMPT: "0",
    HOME: "/nonexistent/task540-test-name-root-git-home",
    LANG: "C.UTF-8",
    LC_ALL: "C.UTF-8",
    PATH: "/usr/bin:/bin",
  })
);
const ROOT_LIVE_DEPS = Object.freeze({
...
const ROOT_RUNTIME_LIVE_DEPS = Object.freeze({ lstat, realpath });

function rootInvariant(condition, message) {
  if (!condition) throw new Error(message);
}

function requireExactRootFunctionDependencies(deps, expectedKeys, label) {
  rootInvariant(
    deps !== null &&
      typeof deps === "object" &&
      !Array.isArray(deps) &&
      Object.keys(deps).sort().join(",") === [...expectedKeys].sort().join(","),
    label + ": dependency keys drift"
  );
  for (const key of expectedKeys) {
    ...
    rootInvariant(
      ...
      label + ": dependency is not an own function"
    );
  }
  return deps;
}

function requireSingleLineRootGitObservation(result, label) {
  rootInvariant(
    result !== null &&
      typeof result === "object" &&
      !Array.isArray(result) &&
      Object.keys(result).sort().join(",") === "stderr,stdout" &&
      typeof result.stdout === "string" &&
      typeof result.stderr === "string" &&
      result.stderr.length === 0 &&
      Buffer.byteLength(result.stdout) <= 4096 &&
      /^[^\0\r\n]+(?:\r?\n)?$/u.test(result.stdout),
    label + ": Git observation is malformed"
  );
  return result.stdout.trim();
}

function requireTask540WorktreeRootProjection(projection, label) {
  if (
    projection === null ||
    typeof projection !== "object" ||
    Array.isArray(projection) ||
    Object.keys(projection).sort().join(",") !==
      "branch,gitCommonDir,gitDir,modulePath,root" ||
    dirname(projection.root) !== PROJECT_PARENT ||
    !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(basename(projection.root)) ||
    projection.modulePath !==
      projection.root + "/_docs/_workflows/" + EXPECTED_MODULE_BASENAME ||
    projection.branch !== EXPECTED_BRANCH ||
    projection.gitCommonDir !== EXPECTED_GIT_COMMON_DIR ||
    basename(projection.gitCommonDir) !== ".git" ||
    dirname(dirname(projection.gitCommonDir)) !== PROJECT_PARENT ||
    !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(
      basename(dirname(projection.gitCommonDir))
    ) ||
    projection.gitDir === projection.gitCommonDir ||
    dirname(projection.gitDir) !== projection.gitCommonDir + "/worktrees" ||
    basename(projection.gitDir) !== basename(projection.root)
  ) {
    throw new Error(label + ": dedicated worktree relationship drift");
  }
  return Object.freeze({ ...projection });
}

async function deriveTask540WorktreeRoot(moduleUrl, deps = ROOT_LIVE_DEPS) {
  const authorityDeps = requireExactRootFunctionDependencies(
    deps,
    ROOT_DERIVATION_DEPENDENCY_KEYS,
    "TASK-540 test-name root"
  );
  rootInvariant(typeof moduleUrl === "string", "TASK-540 test-name module URL is invalid");
  const modulePath = authorityDeps.fileURLToPath(moduleUrl);
  rootInvariant(
    typeof modulePath === "string" &&
      modulePath.startsWith(PROJECT_PARENT + "/") &&
      resolve(modulePath) === modulePath,
    "TASK-540 test-name module path is invalid"
  );
  const candidate = resolve(dirname(modulePath), "../..");
  if (modulePath !== candidate + "/_docs/_workflows/" + EXPECTED_MODULE_BASENAME) {
    throw new Error("TASK-540 test-name module path drift");
  }
  const paths = [
    PROJECT_PARENT,
    candidate,
    candidate + "/_docs",
    candidate + "/_docs/_workflows",
    modulePath,
  ];
  for (const [index, target] of paths.entries()) {
    const info = await authorityDeps.lstat(target);
    if (
      info.isSymbolicLink() ||
      !(index === paths.length - 1 ? info.isFile() : info.isDirectory()) ||
      (await authorityDeps.realpath(target)) !== target
    ) {
      throw new Error("TASK-540 test-name root path is not canonical");
    }
  }
  const gitFilePath = candidate + "/.git";
  const gitFileInfo = await authorityDeps.lstat(gitFilePath);
  rootInvariant(
    !gitFileInfo.isSymbolicLink() &&
      gitFileInfo.isFile() &&
      (await authorityDeps.realpath(gitFilePath)) === gitFilePath,
    "TASK-540 test-name worktree Git file is not canonical"
  );
  const gitFileSource = await authorityDeps.readFile(gitFilePath, "utf8");
  rootInvariant(
    typeof gitFileSource === "string" &&
      Buffer.byteLength(gitFileSource) <= 4096 &&
      /^gitdir: \/[^\0\r\n]+\r?\n$/u.test(gitFileSource),
    "TASK-540 test-name worktree Git file is malformed"
  );
  const gitFileTarget = await authorityDeps.realpath(
    gitFileSource.slice("gitdir: ".length).trim()
  );
  const options = {
    cwd: candidate,
    encoding: "utf8",
  ...
  const [topLevelResult, gitDirResult, gitCommonDirResult, branchResult] =
    await Promise.all([
      authorityDeps.execFile(GIT, ["rev-parse", "--show-toplevel"], options),
      authorityDeps.execFile(GIT, ["rev-parse", "--path-format=absolute", "--git-dir"], options),
      authorityDeps.execFile(
        GIT,
        ["rev-parse", "--path-format=absolute", "--git-common-dir"],
        options
      ),
      authorityDeps.execFile(GIT, ["branch", "--show-current"], options),
    ]);
  const topLevel = await authorityDeps.realpath(
    requireSingleLineRootGitObservation(topLevelResult, "TASK-540 test-name Git top-level")
  );
  const gitDir = await authorityDeps.realpath(
    requireSingleLineRootGitObservation(gitDirResult, "TASK-540 test-name Git directory")
  );
  const gitCommonDir = await authorityDeps.realpath(
    requireSingleLineRootGitObservation(
      gitCommonDirResult,
      "TASK-540 test-name Git common directory"
    )
  );
  const branch = requireSingleLineRootGitObservation(
    branchResult,
    "TASK-540 test-name Git branch"
  );
  if (topLevel !== candidate || gitDir !== gitFileTarget) {
    throw new Error("TASK-540 test-name Git top-level or worktree Git-file relation drift");
  }
  return requireTask540WorktreeRootProjection(
    {
      branch,
      gitCommonDir,
      gitDir,
...
async function requireTask540LocalRuntimeAuthority(
  rootAuthority,
  deps = ROOT_RUNTIME_LIVE_DEPS
) {
  const authorityDeps = requireExactRootFunctionDependencies(
    deps,
    ROOT_RUNTIME_DEPENDENCY_KEYS,
    "TASK-540 test-name runtime"
  );
  const verifiedRoot = requireTask540WorktreeRootProjection(
    { ...rootAuthority },
    "TASK-540 test-name runtime root"
  ).root;
  const resources = Object.freeze([
    Object.freeze({ kind: "file", path: verifiedRoot + "/.env" }),
    Object.freeze({ kind: "directory", path: verifiedRoot + "/node_modules" }),
  ]);
  for (const resource of resources) {
    const info = await authorityDeps.lstat(resource.path);
    const linkDescriptor = Object.getOwnPropertyDescriptor(info, "nlink");
    const safeLinkCount =
      linkDescriptor &&
      Object.hasOwn(linkDescriptor, "value") &&
      Number.isSafeInteger(linkDescriptor.value) &&
      linkDescriptor.value >= 1;
    rootInvariant(
      !info.isSymbolicLink() &&
        (resource.kind === "file" ? info.isFile() : info.isDirectory()) &&
        safeLinkCount &&
        (resource.kind !== "file" || linkDescriptor.value === 1) &&
        (await authorityDeps.realpath(resource.path)) === resource.path,
      "TASK-540 test-name runtime resource is not worktree-local"
    );
  }
  return Object.freeze({
    envPath: resources[0].path,
    nodeModulesPath: resources[1].path,
    root: verifiedRoot,
  });
}

function createTask540RootAuthorityFixture(overrides = {}) {
  const root = overrides.root ?? PROJECT_PARENT + "/Coderso-task540-exec";
  const modulePath =
    overrides.modulePath ?? root + "/_docs/_workflows/" + EXPECTED_MODULE_BASENAME;
  const gitCommonDir = overrides.gitCommonDir ?? EXPECTED_GIT_COMMON_DIR;
  const gitDir = overrides.gitDir ?? gitCommonDir + "/worktrees/" + basename(root);
  const gitFileTarget =
    overrides.gitFileTarget ??
    PROJECT_PARENT + "/Coderso/.git/worktrees/" + basename(root);
  const symlinks = new Set(overrides.symlinks ?? []);
  const pathKinds = new Map(
    [
      [PROJECT_PARENT, "directory"],
      [root, "directory"],
      [root + "/_docs", "directory"],
      [root + "/_docs/_workflows", "directory"],
      [root + "/.git", "file"],
      [modulePath, "file"],
      ...(overrides.pathKinds ?? []),
    ].map(([path, kind]) => [path, kind])
  );
  const realpaths = new Map(overrides.realpaths ?? []);
  const gitRows = new Map([
    ["rev-parse\0--show-toplevel", overrides.topLevel ?? root],
    ["rev-parse\0--path-format=absolute\0--git-dir", gitDir],
    ["rev-parse\0--path-format=absolute\0--git-common-dir", gitCommonDir],
    ["branch\0--show-current", overrides.branch ?? EXPECTED_BRANCH],
  ]);
    ...
    execFile: async (executable, args, options) => {
      rootInvariant(
        executable === GIT &&
          ...
          options.env === ROOT_GIT_ENVIRONMENT &&
          options.maxBuffer === 64 * 1024,
        "TASK-540 test-name fake Git invocation drift"
      );
      const key = args.join("\0");
      rootInvariant(gitRows.has(key), "TASK-540 test-name fake Git argv drift");
      return overrides.gitResults?.[key] ?? {
        stderr: "",
        stdout: gitRows.get(key) + "\n",
      };
    },
    fileURLToPath: (url) => {
      rootInvariant(url === "task540:fixture", "TASK-540 test-name fake module URL drift");
      return modulePath;
    },
    lstat: async (target) => {
      const kind = pathKinds.get(target);
      rootInvariant(kind !== undefined, "TASK-540 test-name fake lstat path drift");
      return Object.freeze({
        isDirectory: () => kind === "directory",
        isFile: () => kind === "file",
        isSymbolicLink: () => symlinks.has(target),
      });
    },
    readFile: async (target, encoding) => {
      rootInvariant(
        target === root + "/.git" && encoding === "utf8",
        "TASK-540 test-name fake Git-file read drift"
      );
      return overrides.gitFileSource ?? "gitdir: " + gitFileTarget + "\n";
    },
    realpath: async (target) => realpaths.get(target) ?? target,
  });
  return Object.freeze({ deps, moduleUrl: "task540:fixture", root });
}

async function assertTask540RootAuthorityContract() {
  const baselineFixture = createTask540RootAuthorityFixture();
  const baseline = await deriveTask540WorktreeRoot(
    baselineFixture.moduleUrl,
    baselineFixture.deps
  );
  rootInvariant(
    baseline.root === baselineFixture.root &&
      baseline.modulePath.endsWith("/" + EXPECTED_MODULE_BASENAME),
    "TASK-540 test-name root fixture baseline drift"
  );
  const rejectionFixtures = [
    createTask540RootAuthorityFixture({
      symlinks: [
        PROJECT_PARENT +
          "/Coderso-task540-exec/_docs/_workflows/" +
          EXPECTED_MODULE_BASENAME,
      ],
    }),
    createTask540RootAuthorityFixture({
      symlinks: [PROJECT_PARENT + "/Coderso-task540-exec"],
    }),
    createTask540RootAuthorityFixture({
      symlinks: [PROJECT_PARENT + "/Coderso-task540-exec/_docs/_workflows"],
    }),
    createTask540RootAuthorityFixture({
      symlinks: [PROJECT_PARENT + "/Coderso-task540-exec/.git"],
    }),
    createTask540RootAuthorityFixture({
      pathKinds: [
        [
    ...
    createTask540RootAuthorityFixture({
      realpaths: [
        [
          PROJECT_PARENT + "/Coderso-task540-exec",
          PROJECT_PARENT + "/Coderso-sibling",
        ],
      ],
    }),
    createTask540RootAuthorityFixture({ root: "/tmp/Coderso-task540-exec" }),
    createTask540RootAuthorityFixture({ root: PROJECT_PARENT + "/-unsafe" }),
    createTask540RootAuthorityFixture({
      modulePath:
        PROJECT_PARENT +
        "/Coderso-task540-exec/_docs/_workflows/task-540-copy.mjs",
    }),
    createTask540RootAuthorityFixture({
      root: PROJECT_PARENT + "/Coderso",
      gitCommonDir: PROJECT_PARENT + "/Coderso/.git",
      gitDir: PROJECT_PARENT + "/Coderso/.git",
    }),
    createTask540RootAuthorityFixture({
      root: PROJECT_PARENT + "/Coderso-sibling",
      branch: "feat/implementations",
    }),
    createTask540RootAuthorityFixture({ branch: "feat/implementations" }),
    createTask540RootAuthorityFixture({ topLevel: PROJECT_PARENT + "/Coderso" }),
    createTask540RootAuthorityFixture({
      gitDir: PROJECT_PARENT + "/Coderso/.git/other/Coderso-task540-exec",
    }),
    createTask540RootAuthorityFixture({
      gitCommonDir: PROJECT_PARENT + "/Elsewhere/.git",
      gitDir: PROJECT_PARENT + "/Elsewhere/.git/worktrees/Coderso-task540-exec",
      gitFileTarget: PROJECT_PARENT + "/Elsewhere/.git/worktrees/Coderso-task540-exec",
    }),
    createTask540RootAuthorityFixture({
      gitFileSource: "gitdir: relative/worktree\n",
    }),
    createTask540RootAuthorityFixture({
      gitResults: {
        ["branch\0--show-current"]: {
          stderr: "",
          stdout: EXPECTED_BRANCH + "\nsecond-line\n",
        },
      },
    }),
  ];
  ...
  for (const fixture of rejectionFixtures) {
    try {
      await deriveTask540WorktreeRoot(fixture.moduleUrl, fixture.deps);
    } catch {
      rejected += 1;
    }
  }
  const spoofDependencyKeys = ["argv", "cwd", "env", "global"];
  for (const key of spoofDependencyKeys) {
    try {
      await deriveTask540WorktreeRoot(
        baselineFixture.moduleUrl,
        Object.freeze({ ...baselineFixture.deps, [key]: "/tmp/spoof" })
      );
    } catch {
      rejected += 1;
    }
  ...
  const originalEnvDescriptor = Object.getOwnPropertyDescriptor(process.env, "TASK540_ROOT");
  const originalGlobalDescriptor = Object.getOwnPropertyDescriptor(globalThis, "TASK540_ROOT");
  let ambientSpoofPass = false;
  try {
    process.chdir("/tmp");
    process.argv = [...process.argv, "--root=/tmp/spoof"];
    process.env.TASK540_ROOT = "/tmp/spoof";
    Object.defineProperty(globalThis, "TASK540_ROOT", {
      configurable: true,
      value: "/tmp/spoof",
      writable: true,
    });
    ...
    if (originalEnvDescriptor) {
      Object.defineProperty(process.env, "TASK540_ROOT", originalEnvDescriptor);
    } else {
      delete process.env.TASK540_ROOT;
    }
    if (originalGlobalDescriptor) {
      Object.defineProperty(globalThis, "TASK540_ROOT", originalGlobalDescriptor);
    } else {
      delete globalThis.TASK540_ROOT;
    }
  }
  rootInvariant(
    rejected === rejectionFixtures.length + spoofDependencyKeys.length && ambientSpoofPass,
    "TASK-540 test-name root authority mutants failed"
  );
  const runtimePaths = new Map([
    [baseline.root + "/.env", "file"],
    [baseline.root + "/node_modules", "directory"],
  ]);
  const runtimeDeps = Object.freeze({
    lstat: async (target) => {
      const kind = runtimePaths.get(target);
      rootInvariant(kind !== undefined, "TASK-540 test-name fake runtime path drift");
      return Object.freeze({
        isDirectory: () => kind === "directory",
        isFile: () => kind === "file",
  ...
  await requireTask540LocalRuntimeAuthority(baseline, runtimeDeps);
  let runtimeRejected = 0;
  for (const deps of [
    Object.freeze({
      ...runtimeDeps,
      lstat: async (target) => {
        const info = await runtimeDeps.lstat(target);
        return target.endsWith("/.env")
          ? Object.freeze({ ...info, isSymbolicLink: () => true })
          : info;
      },
    }),
    Object.freeze({
    ...
    Object.freeze({
      ...
      lstat: async (target) => {
        const info = await runtimeDeps.lstat(target);
        return target.endsWith("/node_modules")
          ? Object.freeze({
              ...info,
              isDirectory: () => false,
              isFile: () => true,
    ...
    Object.freeze({
      ...
      realpath: async (target) =>
        target.endsWith("/.env") ? PROJECT_PARENT + "/Coderso/.env" : target,
    }),
    Object.freeze({
      ...runtimeDeps,
      realpath: async (target) =>
        target.endsWith("/node_modules")
          ? PROJECT_PARENT + "/Coderso/node_modules"
          : target,
    }),
    Object.freeze({
      ...runtimeDeps,
      lstat: async (target) => {
        const info = await runtimeDeps.lstat(target);
        return target.endsWith("/.env") ? Object.freeze({ ...info, nlink: 2 }) : info;
      },
    }),
    Object.freeze({
      ...runtimeDeps,
      lstat: async (target) => {
        const info = await runtimeDeps.lstat(target);
        return target.endsWith("/node_modules")
          ? Object.freeze({ ...info, nlink: 0 })
          : info;
      },
    }),
    Object.freeze({ ...runtimeDeps, env: "/tmp/spoof" }),
  ]) {
    try {
      await requireTask540LocalRuntimeAuthority(baseline, deps);
    } catch {
      runtimeRejected += 1;
    }
  }
  rootInvariant(runtimeRejected === 9, "TASK-540 test-name runtime mutants failed");
  return Object.freeze({
    ambientSpoofCases: 4,
    derivationCases: rejectionFixtures.length + spoofDependencyKeys.length + 1,
    runtimeCases: runtimeRejected + 1,
  });
}
...
const TYPESCRIPT_MODULE_PATH = ROOT + "/node_modules/typescript/lib/typescript.js";
const FAST_XML_PARSER_MODULE_PATH =
  ROOT + "/node_modules/fast-xml-parser/lib/fxp.cjs";
for (const modulePath of [TYPESCRIPT_MODULE_PATH, FAST_XML_PARSER_MODULE_PATH]) {
  const info = await lstat(modulePath);
  rootInvariant(
    !info.isSymbolicLink() &&
      info.isFile() &&
      (await realpath(modulePath)) === modulePath,
    "TASK-540 test-name bootstrap module is not canonical"
  );
}
const [typescriptModule, fastXmlParserModule] = await Promise.all([
  import(pathToFileURL(TYPESCRIPT_MODULE_PATH).href),
  import(pathToFileURL(FAST_XML_PARSER_MODULE_PATH).href),
]);
const ts = typescriptModule.default;
const XMLParser = fastXmlParserModule.default?.XMLParser;
rootInvariant(
  ROOT_RUNTIME_AUTHORITY.root === ROOT &&
    typeof ts?.createSourceFile === "function" &&
    typeof XMLParser === "function",
  "TASK-540 test-name bootstrap module contract drift"
);
...
async function assertScannerSelfTest() {
  const rootAuthorityEvidence = await assertTask540RootAuthorityContract();
  const rootProjection = requireTask540WorktreeRootProjection(
    { ...ROOT_AUTHORITY },
    "TASK-540 test-name self-test root"
  );
  let rootProjectionRejections = 0;
  for (const mutation of [
    { ...rootProjection, root: PROJECT_PARENT },
    { ...rootProjection, branch: "feat/implementations" },
    {
      ...rootProjection,
      gitCommonDir: PROJECT_PARENT + "/Elsewhere/.git",
      gitDir: PROJECT_PARENT + "/Elsewhere/.git/worktrees/" + basename(rootProjection.gitDir),
    },
    { ...rootProjection, gitDir: rootProjection.gitCommonDir },
    {
      ...rootProjection,
      gitDir: rootProjection.gitCommonDir + "/other/" + basename(rootProjection.gitDir),
    },
    { ...rootProjection, modulePath: ROOT + "/_docs/_workflows/other.mjs" },
  ]) {
    try {
      requireTask540WorktreeRootProjection(mutation, "TASK-540 test-name root mutant");
    } catch {
      rootProjectionRejections += 1;
    }
  }
  if (rootProjectionRejections !== 6) {
    throw new Error("TASK-540 test-name dynamic-root self-test failed");
  }
  const before = `
  ...
  return Object.freeze({
    cases:
      37 +
      auxiliaryMutationCases +
      rootAuthorityEvidence.derivationCases +
      rootAuthorityEvidence.runtimeCases +
      rootAuthorityEvidence.ambientSpoofCases,
    registrations: 2,
  });
}
...
if (mode === "self-test") {
  process.stdout.write(
    JSON.stringify({ pass: true, mode, evidence: await assertScannerSelfTest() })
  );
  process.exit(0);
}
