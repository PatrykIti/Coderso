...
**Renderowanie:** `<section class="mx-auto w-full max-w-6xl px-4 py-8">` z opcjonalnym wyśrodkowanym `<header>` (`<h3>` + `<p>`) i siatką `figure`-kafelków. Po TASK-343-23 sekcja ma `aria-labelledby` wskazujące nagłówek albo fallback `aria-label="Gallery"`. Każdy kafelek: **video > image > placeholder**. Caption jako `<figcaption>`: `inside`/`hover` = overlay absolutny na dole (z tłem `overlay`), `below` = `<figcaption class="mt-2 …">` pod kafelkiem. Interakcja per-kafelek: `href` → `<a>` (link wygrywa zawsze), inaczej `lightbox` (gdy `mode=lightbox` i są media) → `<button data-gallery-lightbox-trigger>` + `<div role="dialog">`, inaczej `none`. Lightbox sterowany **wstrzykiwanym skryptem runtime** (`dangerouslySetInnerHTML`).
...
| # | Obszar | Obserwacja |
|---|--------|-----------|
| **N1 — Lightbox nieaktywny w podglądzie admina** | Renderer / runtime | **Zamknięte przez TASK-343-23 przez jawne copy, nie przez CSR-binding.** Visual w trybie lightbox informuje, że admin preview pokazuje statyczny markup lightboxa, a opublikowane strony wiążą skrypt runtime. Sam runtime lightbox pozostaje bez zmian. |
| **N2 — Agramatyzm liczby pojedynczej w komunikatach o linkach** | Visual ostrzeżenie + Advanced summary | **Zamknięte przez TASK-343-23.** Visual używa teraz `uses` dla pojedynczego linkowanego kafelka, a Advanced summary używa `keeps`; liczba mnoga nadal używa `use`/`keep`. |
| **N3 — Picker overlay ukrywa kanał alfa** | Visual / overlay | **Odroczone jako produktowe UX polish.** Swatch dalej zachowuje istniejącą alfę i nie pokazuje raw `rgba(...)`, zgodnie z beginner-safe kontraktem swatch-only. Widoczny edytor/preview kanału alfa wymaga osobnej decyzji produktu. |
| **N4 — `<section>` bez `aria-label`/`aria-labelledby`** | Renderer / a11y | **Zamknięte przez TASK-343-23.** Sekcja ma `aria-labelledby` wskazujące widoczny nagłówek, a przy braku nagłówka fallback `aria-label="Gallery"` w adminie i publicznym SSR. |
...
| **N6 — Duplikacja klas siatki (feature-left)** | Renderer | **Zamknięte przez TASK-343-23.** `feature-left` używa teraz klasy kontenera z mapy density bez doklejania drugiej bazy `grid grid-cols-1`. |
| **N7 — Redukcja liczby kart jest destrukcyjna i nieodwracalna przez ponowne zwiększenie** | Visual / Wizard count | **Zamknięte przez TASK-343-23 jako jawna destrukcyjna akcja.** Wizard i Visual pokazują `ConfirmActionDialog` przed redukcją authored tiles, wymieniają usuwane kafelki i wyjaśniają, że ponowne zwiększenie liczby tworzy placeholdery zamiast przywracać media, captiony, alt, poster i destination. |
| **N8 — Wizard ukryty za „Run setup again"** | UX nawigacji | **Odroczone jako decyzja produktowa.** Gallery Mosaic pozostaje w setup-only wzorcu TASK-336; ewentualne mocniejsze ujawnienie Wizard wymaga spójnej decyzji dla całej rodziny widgetów. |
| **N9 — Strona-fixture jest wielowidgetowa** | UX audytu (nie bug) | **Odnotowane jako fixture caveat, nie bug.** `/admin/pages/5b42d115…` zawiera wiele widgetów; zaznaczenie łatwo zmienić klikając canvas, co podmienia edytor po prawej. Dla audytu trzeba pilnować, by zaznaczony pozostał Gallery Mosaic. |

**Nie wykryto** błędów konsoli na froncie, twardego buga renderowania, ani rozjazdu render admin↔front dla wspólnie testowanych opcji (poza celową izolacją niezapisanych zmian i N5). Wszystkie przetestowane kontrolki Visual/Wizard aktualizują podgląd na żywo; Advanced wiernie podsumowuje stan roboczy; front jest responsywny, bez błędów konsoli i po TASK-343-23 ma nazwaną sekcję. Admin lightbox preview pozostaje statyczny, ale Visual mówi o tym wprost.
...
- **Realne auto-wykonanie lightboxa na SSR froncie** — opublikowany fixture ma `interaction=none` (brak markupu lightboxa, `scriptInjected=false`). Nie istnieje więc opublikowana strona z lightboxem, na której można by zaobserwować **automatyczne** związanie skryptu w przeglądarce. *Mitygacja:* logikę runtime potwierdziłem wykonując skrypt ręcznie (open/Escape/backdrop — §4.3), a TASK-343-23 dodał admin-copy ostrzegające, że preview jest statyczny.
...
| Nazwa dostępna `<section>` | `aria-labelledby` lub `aria-label="Gallery"` po TASK-343-23 | `aria-labelledby` lub `aria-label="Gallery"` po TASK-343-23 | ✓ |
| Hover-caption a11y (tabindex/aria) | obecne (gdy interaction none) | obecne (kafelki 2–5: `tabindex=0`) | ✓ |
| **Lightbox — interaktywność** | markup TAK; preview statyczny i opisany w Visual; **logika potwierdzona ręcznie** | brak (fixture `interaction=none`) | ✓ truthfulness / ⚠ brak opublikowanego fixture lightbox |
...
- **Najmocniejszy nowy wynik:** runtime lightboxa **działa** — open/Escape/backdrop potwierdzone przez ręczne wykonanie wstrzykniętego skryptu na żywym DOM. Po TASK-343-23 admin copy mówi wprost, że preview jest statyczny, a opublikowane strony wiążą skrypt.
...
- **Niespójności / kosmetyka:** po TASK-343-23 zamknięto agramatyzm „1 linked item use/keep" (N2), brak nazwy `<section>` (N4), duplikację klas feature-left (N6) oraz cichą destrukcyjność count reduction (N7). N3 pozostaje odroczonym polish dla widoczności kanału alfa, N8 jest decyzją produktową setup-only, a N9 pozostaje caveatem fixture.
