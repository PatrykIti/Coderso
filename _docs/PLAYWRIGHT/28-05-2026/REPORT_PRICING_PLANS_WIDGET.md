...
- **Billing bar:** `role="status"`, `aria-live="polite"`, `data-pricing-billing-toggle="static"`, `data-pricing-billing-display="static-cycle"`. ✓ Po TASK-343-25 renderuje jako nieinteraktywny status cyklu, a nie wizualny przełącznik.
...
| **N1 — Wariant deklaruje więcej planów niż realnie renderuje** | Visual / Wizard | **Zamknięte przez TASK-343-25.** `FixedPlanCountNotice` pokazuje teraz pojemność wariantu osobno od liczby zapisanych planów, które faktycznie renderują się w podglądzie. Dla brakujących kart wskazuje dodanie planu w Visual zamiast obiecywać nieistniejący render. |
| **N2 — Wizard odsyła do kontrolki, której w Wizardzie nie ma** | Wizard | **Zamknięte przez TASK-343-25.** Wizard nie wspomina już o przełączniku wariantu w swoim trybie; przy brakujących planach odsyła do Visual jako właściciela edycji. |
| **N3 — Niespójna mechanika potwierdzeń destrukcyjnych** | Visual vs Advanced | **Zamknięte przez TASK-343-25.** Usuwanie planów, usuwanie feature oraz akcje naprawcze Advanced używają `ConfirmActionDialog`; Visual nie używa już natywnego `window.confirm`, a feature nie usuwa się natychmiast bez potwierdzenia. |
| **N4 — „Clear" surface/border = przezroczystość, nie kolor motywu** | Visual / colors | **Wyjaśnione przez TASK-343-25 + TASK-343-30.** Kontrolki używają wspólnego opisu stanu koloru: surface/border po Clear są opisane jako stan dziedziczony/bez inline override, a nie jako fałszywy powrót do motywu. |
| **N5 — „Billing toggle" nie jest klikalnym togglem** | Renderer / frontend | **Zamknięte przez TASK-343-25.** Publiczny pasek billingowy nadal jest statyczny (`role="status"`), ale renderuje pojedynczy status „Billing cycle: … pricing shown" zamiast dwóch pigułek wyglądających jak przełącznik. |
...
| **N8 — „Clear" highlight ring ≠ „Clear" surface/border** | Visual / colors | **Wyjaśnione przez TASK-343-25 + TASK-343-30.** Highlight ring po Clear wraca do domyślnego `var(--color-primary)`, a copy kontrolki mówi o przywróceniu domyślnego highlightu; surface/border mówią o usunięciu override. |
| **N9 — Badge planu wyróżnionego bywa ukrywany** | Renderer | **Zamknięte przez TASK-343-25.** `renderPlanBadge` renderuje badge także wtedy, gdy tekst jest taki sam jak highlight label, więc `data-pricing-badge-tone` pozostaje widoczne i testowalne. |
