...
| Row | Final status | Owner | Evidence |
|---|---|---|---|
| BF-05 | `deferred` | `TASK-328` landed; Spacer still needs widget-local product scope | Spacer remains vertical-only. The shared `row-flow-item` shell now exists, but a truthful horizontal Spacer still needs its own bounded width/orientation contract instead of silently reopening BF-05 through infrastructure alone. |
