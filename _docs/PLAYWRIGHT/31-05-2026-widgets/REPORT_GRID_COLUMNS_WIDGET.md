...
| Asymmetric reapply | Klik `Reapply asymmetric desktop widths` | W UI `7/12, 3/12, 2/12` zmienia sie na `6/12, 3/12, 3/12`, copy przechodzi na active. | Nie dotyczy bez zapisu; helper logic zgodny z presetem. | Dziala funkcjonalnie, metadata nie | Button nie jest opakowany `data-widget-control`, mimo ze mutuje `columns.desktopSpan`. | Patrz `GC-31-05-01`. |
...
| Shared Structure | Visual Structure section | `Add Column`, Move up/down, Remove widoczne; live slots utrzymuja content/data order. | Public slot ids sa stable: `column:lead`, `column:proof`, `column:action`. | Dziala funkcjonalnie, metadata niepelna | `Add Column` jest unwrapped, Move/Remove maja action metadata bez path. | Patrz `GC-31-05-01`. |
...
### GC-31-05-01 - Action controls dzialaja, ale czesc nie ma stabilnego `data-widget-control` metadata

**Objaw:** Visual inspection pokazal dwa unwrapped controls:
...
1. Owinac `Reapply asymmetric desktop widths` w Grid Columns metadata, np.
   `data-widget-control="grid-columns.visual.asymmetric-reapply"`,
   `data-widget-control-path="columns.desktopSpan"`,
   `data-widget-control-ownership="writable"` albo `"action"` z path.
2. W shared Structure rendererze nadac `Add Column` wrapper z action metadata
   i path `slots.column` albo stabilnym `slotControls.addActions[*].path`.
3. Dla Move/Remove rozwazyc path `slots.column` + action id z instance id, zeby
   testy i automatyzacja widzialy ktory repeatable slot jest mutowany.
4. Dodac DOM test dla Grid Columns Visual: brak unwrapped `Reapply` / `Add
   Column`, reapply path to `columns.desktopSpan`, Structure add path to
   `slots.column`.
