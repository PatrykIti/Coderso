...
| Missing live preview inside the widget editors | fixed | `TASK-317` | the shared builder `BlockSettings` surface now renders the selected widget through `WidgetRenderer` in `editor-preview` mode using transient `previewState.dataPatch` data |
| Sticky works in local editor contexts but frontend sticky can be blocked by `Section` / page-shell overflow | fixed | `TASK-318` | shared Section/layout containment no longer blocks sticky after TASK-318; Navigation still owns only its local sticky/collapse contract |
