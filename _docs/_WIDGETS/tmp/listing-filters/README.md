...
| daisyUI filter controls | https://daisyui.com/components/filter/ | open-source | MIT package; verify. | Filter buttons/chips for mutually exclusive or additive choices. | `layout.mode: chips`, `selection.mode`, `activeStyle`. | Adapt | Summarize only. |
