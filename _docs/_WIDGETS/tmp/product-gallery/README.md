...
|---|---|---|---|---|---|---|---|
| Shopify Hydrogen product gallery | https://github.com/Shopify/hydrogen-demo-store | open-source | MIT repo; verify current license. | Product image gallery with thumbnails and product media selection. | `source.limit`, `source.search`, `source.collectionIds`, `source.status`, `source.sortField`, `source.sortDir`, `media.mode`, `media.thumbnailPosition`; product-id picker remains conditional owner-extension scope. | Keep | Summarize only. |
