...
|---|---|---|---|---|---|---|---|
| Shopify Hydrogen product gallery | https://github.com/Shopify/hydrogen-demo-store | open-source | MIT repo; verify current license. | Product image carousel with thumbnails and selected variant media. | `source.productId`, `media.mode`, `media.thumbnailPosition`, `variantBinding`. | Keep | Summarize only. |
