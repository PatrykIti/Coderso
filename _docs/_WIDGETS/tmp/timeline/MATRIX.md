...
| Icon/status indicators | Chakra | Keep | Own `items[].icon` and `items[].status` as validated enums/tokens; Origin UI remains Adapt reference material. |
...
| Per-item CTA | Flowbite | Keep | Support optional safe-link CTA per item with label/href validation; ReUI remains Adapt reference material. |
