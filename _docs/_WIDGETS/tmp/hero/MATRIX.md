...
|---|---|---|---|
| Centered copy with dual CTA | HyperUI | Keep | Preserve `heading`, `body`, `primaryCta`, `secondaryCta`, alignment, and width controls; Tailwind UI Plus polish stays Adapt. |
| Split media hero | Preline | Keep | Add/confirm `layout`, `media`, `mediaPosition`, and mobile stacking controls; Flowbite form/search composition stays Adapt. |
| Badge or announcement line | Origin UI | Keep | Own a `badge` object with label, optional href, tone, and visibility. |
