...
|---|---|---|
| Keep | Binary/two-state content swap from daisyUI Swap and pricing toggles. | Model `states[]` with labels, default state, and panel content/slots. |
...
| Keep | Accessible pressed/toggle semantics from React Aria and Radix. | Renderer must expose state changes through proper button/switch semantics. |
