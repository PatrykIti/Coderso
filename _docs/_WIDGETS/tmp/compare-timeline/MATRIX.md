...
| Track labels | ReUI | Keep | Add `tracks` or `trackLabel` only if comparison requires named lanes; Tailwind UI Plus remains Adapt reference material. |
| Status/current highlight | Chakra | Keep | Add `state`/`highlightCurrent` enums for segments; Origin UI remains Adapt reference material. |
