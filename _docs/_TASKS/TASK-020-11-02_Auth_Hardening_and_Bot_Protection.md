...
Strengthen login/reset endpoints with stricter rate limits, rereCAPTCHA v3 bot protection, and stronger validation. Keep UX reasonable while blocking abuse.
...
2. rereCAPTCHA v3 scoring gate for login/reset.
...
| `core/services/settings/securitySettings.ts` | Add reCAPTCHA v3 config (siteKey, secretKey, score thresholds) |
...
2. **rereCAPTCHA scope**: login only or login + password reset?
3. **Pepper**: Do we want a server-side pepper for passwords?
