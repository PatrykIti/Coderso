# TASK-394-03: AF-31-05-03 - Mixed public/internal service catalog must scope captcha/nonce per selected service
# FileName: TASK-394-03_AF_31_05_03_Mixed_Public_Internal_Service_Catalog_Must_Scope_Captcha.md
...
Execution-ready leaf task for AF-31-05-03 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_APPOINTMENT_FORM_WIDGET.md` and parent `TASK-394`.

Status log:

- 2026-06-02: Moved to In Progress with TASK-394 family implementation.
- 2026-06-02: Done; captcha/nonce behavior is scoped to the selected service access mode.

Global public-service presence can force captcha on internal flow.

## Sub-Tasks

- [x] Reproduce AF-31-05-03 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Render `submissionAccess` into service options, include it in selection events, and decide captcha/nonce per selected service.
...
**Regression-test shape:** Mixed catalog runtime/API test: internal service not blocked by missing captcha, public service still requires nonce/captcha.
...
- `git diff --check`
- Focused regression: Mixed catalog runtime/API test: internal service not blocked by missing captcha, public service still requires nonce/captcha.
...
- AF-31-05-03 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Booking Calendar service options now expose `data-submission-access`, runtime
  selections carry it, and Appointment Form only resolves CAPTCHA for public
  selections.
- Runtime regression covers internal selection in a mixed catalog with captcha
  configured on the form but not required for the selected internal service.
