# TASK-373-01: CTA-31-05-01 - Advanced must report active background gradients
# FileName: TASK-373-01_CTA_31_05_01_Advanced_Must_Report_Active_Background_Gradients.md
...
Execution-ready leaf task for CTA-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_CTA_BANNER_WIDGET.md` and parent `TASK-373`.

Runtime preview renders `background-image`, but Advanced `Style diagnostics` says `Background Theme default` and omits the gradient.
...
**Helper/function shape:** Add `Background gradient` row or make `Background` summarize `Gradient configured` when `normalized.background.gradient` is active.
...
**Regression-test shape:** UI regression: gradient state appears in Advanced diagnostics and default background copy remains truthful.
...
- `git diff --check`
- Focused regression: UI regression: gradient state appears in Advanced diagnostics and default background copy remains truthful.
...
- `_docs/_TASKS/TASK-373_CTA_Banner_Widget_31_05_UI_Audit_Remediation_Family.md` parent status/checklist when this leaf starts or closes.
...
- CTA-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
