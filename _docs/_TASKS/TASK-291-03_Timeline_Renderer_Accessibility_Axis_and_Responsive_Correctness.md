...
- [ ] Add section and ordered-list accessible names from header fields,
  fallback labels, or static Timeline labels.
...
- [ ] Add bounded minimum height/density behavior for short timelines.

## Files to Change

| File | Required change |
|---|---|
| `core/widgets/core/timeline.tsx` | Update milestone, cards, chronology, alternating, compact renderers for accessibility, responsive layout, connectors, line semantics, and min-height. |
| `tests/vitest/widgets/timeline.test.tsx` | Add SSR assertions for ARIA, mobile-visible date output, connector style, card border semantics, chronology classes/styles, and min-height. |
...
function getTimelineLabel(data: TimelineData) {
  return data.header?.title?.trim() || "Timeline";
}
...
2. Resolve static labels and style tokens before entering layout-specific
   renderer functions.
3. Pass label, `aria-current`, connector, and min-height decisions into every
   layout that renders an ordered list.
4. Keep legacy data attributes used by Playwright/report evidence.

Error handling:

- Missing header/title data falls back to a static accessible label.
...
- Timeline renderer has section/list names and current-step semantics.
