# TASK-396-08: CONTACT-31-05-08 - Complete Visual path metadata beyond style rows
# FileName: TASK-396-08_CONTACT_31_05_08_Complete_Visual_Path_Metadata_Beyond_Style_Rows.md
...
Execution-ready leaf task for CONTACT-31-05-08 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_CONTACT_WIDGET.md` and parent `TASK-396`.

Only recently fixed style rows have complete path metadata.

Status log:

- 2026-06-02: Moved to In Progress with TASK-396 Contact remediation.
- 2026-06-02: Done. CONTACT-31-05-08 / CT-31-05-08 is closed with stable
  Visual path/action metadata across real Contact controls.

## Sub-Tasks

- [x] Reproduce CONTACT-31-05-08 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Wrap remaining Contact controls with stable `data-widget-control-path` and align editor contract sections.
...
**Regression-test shape:** Editor metadata scan for Contact has no unwrapped mutating controls.
...
- `git diff --check`
- Focused regression: Editor metadata scan for Contact has no unwrapped mutating controls.
...
- CONTACT-31-05-08 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Contact Visual exposes stable metadata for variant cards, title/description,
  fields/required/order, field settings, submission mode/form/mapping/copy,
  details/social, map, and layout controls.
- Regression coverage is in `tests/vitest/ui/contact-editor-wave.test.tsx`.
