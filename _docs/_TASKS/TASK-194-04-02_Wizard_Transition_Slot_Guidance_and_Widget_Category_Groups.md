...
- `core/admin/ui/pages/builder/BlockList.tsx:252-276`
  - replace bare `Empty slot.` placeholder with actionable copy and CTA.
...
- Copy-only empty states are not enough for this leaf. Preferred path:
  - clicking the slot CTA opens the existing insert surface scoped to widgets
    that fit the slot contract,
  - if the slot click cannot ship in this wave, keep the CTA text and make the
    follow-up dependency explicit in the doc/changelog.
...
- `tests/vitest/pageBuilder/blockList.test.tsx`
  - empty-slot placeholder is actionable and exposes CTA text or button.
...
2. Empty slots explain what the user can do there and expose a visible CTA.
