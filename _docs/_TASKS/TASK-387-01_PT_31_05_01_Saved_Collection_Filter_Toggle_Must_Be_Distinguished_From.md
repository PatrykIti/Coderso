# TASK-387-01: PT-31-05-01 - Saved collection filter toggle must be distinguished from active public control
# FileName: TASK-387-01_PT_31_05_01_Saved_Collection_Filter_Toggle_Must_Be_Distinguished_From.md

**Priority:** Medium
**Category:** Widgets + Product Table + Commerce UI + QA + Docs + Leaf Remediation
...
Execution-ready leaf task for PT-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_PRODUCT_TABLE_WIDGET.md` and parent `TASK-387`.

Renderer hides collection fieldsets unless more than one collection resolves, but Advanced reports `Collection filters` as active based only on saved toggle.

## Sub-Tasks

- [x] Reproduce PT-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Summarize runtime availability from `resolved.runtime.availableCollections/statuses`; label saved-but-inactive filters as inactive until enough options resolve.
...
**Regression-test shape:** UI regression: empty available collections + showCollectionFilter true does not report active visitor collection filter without inactive copy.
...
- `git diff --check`
- Focused regression: UI regression: empty available collections + showCollectionFilter true does not report active visitor collection filter without inactive copy.
...
- PT-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Advanced `Visitor controls` now marks saved collection filters inactive until at least two collections resolve.
- The same runtime-availability distinction now applies to saved status filters.
- Runtime/public renderer behavior was not changed.
