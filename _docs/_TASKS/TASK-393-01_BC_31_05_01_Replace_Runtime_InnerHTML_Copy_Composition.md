# TASK-393-01: BC-31-05-01 - Replace runtime `innerHTML` copy composition
# FileName: TASK-393-01_BC_31_05_01_Replace_Runtime_InnerHTML_Copy_Composition.md
...
Execution-ready leaf task for BC-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_BOOKING_CALENDAR_WIDGET.md` and parent `TASK-393`.

Runtime builds service context and messages through `innerHTML` from decoded text/dataset values.

## Sub-Tasks

- [x] Reproduce BC-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
- Replaced runtime `innerHTML` copy composition in service context, empty/missing/error slot copy, and week button labels with explicit DOM nodes and `textContent`.
- Added happy-dom regression coverage with `<img onerror>` payloads in service name, service description, empty copy, missing copy, and error copy; no image nodes are created and the payload renders as literal text.
- Validation covered by TASK-393 parent changelog 1083.

## Implementation Pseudocode

**Helper/function shape:** Create DOM nodes and assign `textContent`, or escape all HTML before assignment. Cover service names/descriptions and empty/missing/error copy.
...
**Regression-test shape:** Happy-dom runtime test with `<img onerror>` payloads creates no image and displays literal text.
...
- `git diff --check`
- Focused regression: Happy-dom runtime test with `<img onerror>` payloads creates no image and displays literal text.
...
- BC-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
