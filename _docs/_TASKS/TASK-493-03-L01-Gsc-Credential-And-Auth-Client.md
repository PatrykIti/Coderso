...
- **Owning module(s) to create-or-extend:**
  - `core/services/integrations/registry.ts` (**extend** — append a
    ...
    `scopes: ["seo:read", "search-console:read"]`, and `secret`-typed fields).
...
- **Validation:** `ensureKnownKeys` in `integrationsService.ts:151` already
...
export async function getGscClient(scope = "webmasters.readonly") {
  ...
  return { request, siteUrl: cfg.siteUrl };
}
```
...
`request`/`siteUrl` to callers (L02 sync, 02-L02 submit).
...
- Client: `getGscClient` throws `gsc_not_configured` with no config; mints a
  ...
  surfaces `gsc_request_failed:<status>` on non-2xx.
