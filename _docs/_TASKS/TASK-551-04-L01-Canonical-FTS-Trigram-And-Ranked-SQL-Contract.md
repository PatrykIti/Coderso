...
const SEARCH_SOURCE_CONTRACT = strictReadonly({
  pages: { vector: pages.searchVector, updatedAt: pages.updatedAt, id: pages.id },
  entries: { vector: contentEntries.searchVector, updatedAt: contentEntries.updatedAt, id: contentEntries.id },
  posts: { vector: posts.searchVector, updatedAt: posts.updatedAt, id: posts.id },
  media: { vector: media.searchVector, updatedAt: media.createdAt, id: media.id },
  users: { vector: users.searchVector, updatedAt: users.updatedAt, id: users.id },
});
...
async function searchAll(request: StrictSearchRequest, deps: SearchDeps): Promise<SearchItem[]> {
  ...
  // and must use the matching 551-05 normalized pg_trgm index.
}
```

TASK-551-05 installs the required extensions, five CMS generated vectors, and
matching GIN plus evidence-backed normalized trigram indexes. This leaf verifies
and consumes those objects without recreating their SQL expressions. Trigram
fallback is bounded and parameterized; exact email lookup continues via
...
No shared docs. Pass consumed vector fields, limits, ranked-query plan evidence,
and fallback policy to TASK-551-10-L02.
