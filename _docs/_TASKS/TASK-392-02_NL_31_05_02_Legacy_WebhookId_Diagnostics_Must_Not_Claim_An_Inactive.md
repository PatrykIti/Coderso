# TASK-392-02: NL-31-05-02 - Legacy `webhookId` diagnostics must not claim an inactive submit path
# FileName: TASK-392-02_NL_31_05_02_Legacy_WebhookId_Diagnostics_Must_Not_Claim_An_Inactive.md
...
Execution-ready leaf task for NL-31-05-02 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NEWSLETTER_WIDGET.md` and parent `TASK-392`.

Advanced reports legacy webhook as a destination even though runtime blocks that path.
...
**Helper/function shape:** Mark legacy webhook as inactive/unsupported, or migrate it to the supported Forms/action destination before reporting active.
...
**Regression-test shape:** UI regression: saved `webhookId` does not appear as active destination without inactive copy.
...
- `git diff --check`
- Focused regression: UI regression: saved `webhookId` does not appear as active destination without inactive copy.
...
- NL-31-05-02 is fixed or reclassified with fresh evidence in the report and parent task.
