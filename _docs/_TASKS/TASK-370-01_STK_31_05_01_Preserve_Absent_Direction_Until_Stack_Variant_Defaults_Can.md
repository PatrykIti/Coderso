...
- [x] Reproduce STK-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
- Reproduced STK-31-05-01 against the validator/render path: non-empty Stack payloads without `direction` were filled with vertical default direction before variant defaults could apply.
- Added `preserveAbsentDefaultKeys: ["direction"]` to the Stack widget definition, reusing the existing generic validator opt-in rather than adding route/editor fallbacks.
- Preserved explicit saved `direction` values and empty new-block default behavior; only non-empty saved/imported payloads that omit `direction` defer to Stack's variant-aware defaults.
- Added focused regressions for validator output and public `WidgetRenderer` markers: responsive defaults to `row/row/column`, horizontal defaults to `row/row/row`, and validator output does not inject `direction` for non-empty omitted payloads.
- Claude staged review reported no blockers for the Stack normalization contract, tests, docs, and changelog closure.
