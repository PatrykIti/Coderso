# TASK-396-06: CONTACT-31-05-06 - Configured/effective runtime metadata must not claim forms-runtime when fallback is static/internal
# FileName: TASK-396-06_CONTACT_31_05_06_Configured_Effective_Runtime_Metadata_Must_Not_Claim_Forms.md
...
Execution-ready leaf task for CONTACT-31-05-06 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_CONTACT_WIDGET.md` and parent `TASK-396`.

Static/internal fallback has misleading `data-contact-form-mode=forms-runtime`.

Status log:

- 2026-06-02: Moved to In Progress with TASK-396 Contact remediation.
- 2026-06-02: Done. CONTACT-31-05-06 / CT-31-05-06 is closed with separate
  configured/effective form mode metadata.

## Sub-Tasks

- [x] Reproduce CONTACT-31-05-06 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Separate configured mode from effective render mode in data attrs and Advanced summaries.
...
**Regression-test shape:** Renderer regression for configured forms-runtime but effective static/internal fallback.
...
- `git diff --check`
- Focused regression: Renderer regression for configured forms-runtime but effective static/internal fallback.
...
- CONTACT-31-05-06 is fixed or reclassified with fresh evidence in the report and parent task.
...
- `data-contact-form-mode` now reports the effective render mode.
- `data-contact-form-configured-mode` preserves the saved editor mode.
- `data-contact-runtime-boundary` explains static/internal/nonce/field
  mismatch fallback states.
