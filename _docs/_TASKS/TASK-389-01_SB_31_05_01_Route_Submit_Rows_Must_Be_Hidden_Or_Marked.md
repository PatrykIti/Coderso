# TASK-389-01: SB-31-05-01 - Route-submit rows must be hidden or marked inactive outside route-submit mode
# FileName: TASK-389-01_SB_31_05_01_Route_Submit_Rows_Must_Be_Hidden_Or_Marked.md

**Priority:** Medium
**Category:** Widgets + Search Box + Admin UI + Runtime Diagnostics + QA + Docs + Leaf Remediation
...
Execution-ready leaf task for SB-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_SEARCH_BOX_WIDGET.md` and parent `TASK-389`.

Advanced always shows `Results page` and `Search term routing`, but listing/global modes use different runtime paths.

## Sub-Tasks

- [x] Reproduce SB-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Gate route-target/query-param rows by `mode === "route-submit"`; for listing/global hide them or label as only used in route-submit.
...
**Regression-test shape:** UI regression: Advanced for listing and global modes does not suggest active route-submit routing.
...
- `git diff --check`
- Focused regression: UI regression: Advanced for listing and global modes does not suggest active route-submit routing.
...
- `_docs/_TASKS/TASK-389_Search_Box_Widget_31_05_UI_Audit_Remediation_Family.md` parent status/checklist when this leaf starts or closes.
...
- SB-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
...
Closed on 2026-06-02. Advanced diagnostics now hide route-submit-only rows in `listing` and `global` modes while preserving those rows for `route-submit`; a read-only `Active routing` row names the effective branch.
