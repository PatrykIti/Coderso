...
| TASK-256-07 | Dynamic/operational widget issues: content-list columns visible for non-card variants and textColor clear gap (`REPORT_CONTENT_LIST_WIDGET.md:66-87,268-280`); navigation logo href/editor validation/a11y/sticky wrapper drift (`REPORT_NAVIGATION_WIDGET.md:74-84,142-151,207-214,327-332,380-391,397-427,450`) | `ContentListEditors.tsx`; `contentList.tsx:145-156,256-267`; `NavigationEditors.tsx`; `navigation.tsx:25-32,391-405` and logo/dropdown/mobile render region around `navigation.tsx:440-516`; section/layout wrapper owners for sticky overflow | Hide/disable controls that have no runtime effect; align menu href validation with renderer hash support; render logo/external links safely; assign dropdown/mobile accessibility findings; decide sticky ownership at wrapper level and document if layout constraints remain | Update `tests/vitest/ui/content-list-editor-wave.test.tsx`, `tests/unit/widgets/contentList.test.tsx`, `tests/vitest/ui/navigation-editor-wave.test.tsx`, `tests/vitest/widgets/navigation.test.tsx`, `tests/vitest/widgets/widgetSafeHref.test.ts`, and `tests/vitest/ui/menu-editor-validation.test.ts` |
