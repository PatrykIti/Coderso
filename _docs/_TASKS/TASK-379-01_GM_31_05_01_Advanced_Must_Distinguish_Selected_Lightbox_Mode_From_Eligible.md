# TASK-379-01: GM-31-05-01 - Advanced must distinguish selected lightbox mode from eligible lightbox tiles
# FileName: TASK-379-01_GM_31_05_01_Advanced_Must_Distinguish_Selected_Lightbox_Mode_From_Eligible.md
...
Execution-ready leaf task for GM-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_GALLERY_MOSAIC_WIDGET.md` and parent `TASK-379`.

When lightbox mode is selected but all media are cleared, runtime has no triggers while Advanced still reports `Interaction Lightbox, fill zoom` as active.
...
**Helper/function shape:** Compute eligible lightbox item count and summarize `Lightbox selected; no media tiles currently open` when count is zero.
...
**Regression-test shape:** Editor/renderer regression for lightbox mode with zero eligible media.
...
- `git diff --check`
- Focused regression: Editor/renderer regression for lightbox mode with zero eligible media.
...
- GM-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
