# TASK-397-06: NV-31-05-06 - Bound persisted/imported style colors
# FileName: TASK-397-06_NV_31_05_06_Bound_Persisted_Imported_Style_Colors.md
...
Execution-ready leaf task for NV-31-05-06 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NAVIGATION_WIDGET.md` and parent `TASK-397`.

Imported color strings are not schema-bounded.

## Sub-Tasks

- [x] Reproduce NV-31-05-06 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Apply shared safe color normalizer/schema pattern to Navigation style fields before public render.
...
**Regression-test shape:** Renderer regression blocks unsafe color strings and keeps safe swatches/tokens.
...
- `git diff --check`
- Focused regression: Renderer regression blocks unsafe color strings and keeps safe swatches/tokens.
...
2026-06-02: Closed in Navigation scope. Navigation color fields are
schema-bounded and normalized before render with `resolveClearableCssColorValue`.
Focused regressions: `navigation schema rejects unsafe imported style color
strings` and `navigation normalizes unsafe imported color values before render`.

Validation is recorded in parent TASK-397 closure notes.

## Acceptance Criteria

- NV-31-05-06 is fixed or reclassified with fresh evidence in the report and parent task.
