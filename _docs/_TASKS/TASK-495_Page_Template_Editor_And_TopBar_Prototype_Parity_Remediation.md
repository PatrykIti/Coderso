...
Live comparison of the redesign **prototype** (design source-of-truth, `_docs/_PROTOTYPE/`, runs `:5180`) against the **shipped DEV admin** (`core/admin/**`, runs `:5173`) after TASK-479 closed (2026-06-29, HEAD `abb64b6e`; closure `018501ab`) surfaced **two prototype-parity gaps** the owner wants remediated, plus a
follow-up visual-parity pass after TASK-495-02 shipped. This is the board task; the
gaps are delivered as three children:
...
- **TASK-495-03** — Page/Page-Template editor *visual* parity follow-up (GAP B polish): after 495-02 restructured the builder, the owner flagged three remaining mismatches vs the prototype — dark-mode canvas stays a white slab, the rail is 340px (proto 280px), and the builder is full-bleed/flat instead of a separated rounded card.
...
| **TASK-495-03** | Page-Template Editor Visual Parity | Visual follow-up after 495-02. P1 (SHARED, light-pixel-safe): canvas frame `bg-white`→`bg-card` (`PageEditor.tsx:2974`) + canvas CTA/ghost/add-beside literals→tokens (`controlChrome.ts:65-89`) — fixes the dark-mode white slab for builder AND menu. P2 (builder-gated): wrap the sub-toolbar + `BuilderCanvasRegion` in one `rounded-2xl border bg-card shadow-card` card with `mx-6 mb-6` + float the `PageHeader` above it. P3 (builder-gated): rail `340`→`280` (`:3134`) + `paddingRight 360`→`300` (`:2968`). P4 (builder-gated): demote the context strip, realign the rail inside the dots, `p-6`→`p-6 lg:p-8`. Menu path byte-identical except the two shared P1 token swaps; the optional `max-w-2xl` doc-column is deferred to the owner. |
...
- **TASK-495-03:** the page-editor + menu-design suites stay green (do **not** weaken `data-*` hook assertions); update only the P1b CONSTANT value-shape checks (`editorCanvasCtaButtonClass` `bg-white`→`bg-card`, `hover:bg-slate-100`→`hover:bg-muted` at `page-editor-v2-flow.test.tsx:3146-3147`) and add guards for the `bg-card` frame (P1a), the `w-[min(280px,…)]` rail (P3a), and the `bg-card`/`shadow-card` builder wrapper present on the `page` host / absent on the bare `mode:"menu"` host (P2a). Visual acceptance (playwright real input): **dark-mode** canvas frame + CTAs + card recolor (no white slab) vs `/tmp/task495-parity/proto-dark.png`; **card separation** (rounded `bg-card shadow-card` wrapper, `PageHeader` floating above, `mx-6 mb-6`); **panel width** 280px; menu path visually unchanged in light + dark-correct. The optional `max-w-2xl` doc-column is **deferred to the owner**.
