...
6. **DB backfill:** migration `0080_detail_page_v2_backfill.sql` (pattern
...
| L03 | [Detail Page V2 DB Backfill Migration](TASK-580-03-L03-Detail-Page-V2-DB-Backfill-Migration.md) | SQL backfill `0080` (renumbered from `0079` on merge) + snapshot + journal + DB tests | Large | ✅ Done |
...
`core/db/migrations/0080*` (renumbered from `0079` on merge with TASK-493) +
`core/db/migrations/meta/*` for that idx, and the
...
3. Migration `0080` (renumbered from `0079` on merge with TASK-493) ships
   complete artifacts (SQL + snapshot + journal),
