...
# FileName: TASK-310-04_Shared_Color_Picker_Team_and_Testimonials_Editor_Adoption.md
...
Adopt the landed shared color-field seam in the people/social-proof editors
...
This leaf must stay narrow: it only replaces the duplicated color helper path
and preserves the current Team/Testimonial product surface.

## Scope Boundary

This leaf owns:

- removing local `resolvePickerColor()` / `ColorField` clones in
  `TeamEditors.tsx` and `TestimonialsEditors.tsx`;
...
- Team/Testimonial product changes such as spotlight behavior, slider controls,
  avatar/media authoring, or rating semantics;
...
|---|---|
| `core/admin/ui/widgets/editors/TeamEditors.tsx` | Replace the local color-field helper with the landed shared seam. |
| `core/admin/ui/widgets/editors/TestimonialsEditors.tsx` | Replace the local color-field helper with the landed shared seam. |
...
| `tests/vitest/ui/team-editor-wave.test.tsx` | Update shared color-field expectations when the editor behavior changes. |
| `tests/vitest/ui/testimonials-editor-wave.test.tsx` | Update shared color-field expectations when the editor behavior changes. |
| `tests/vitest/ui/clearable-fields.test.tsx` | Update only if the shared helper surface changes. |

## Implementation Pseudocode

```tsx
function SharedPeopleColorField(props: {
  label: string;
...
- Clear must delete the configured style key, not replace it with an empty
  string sentinel.
- Do not widen Team/Testimonial product scope in this leaf.
...
- `TeamEditors.tsx` and `TestimonialsEditors.tsx` no longer duplicate local
  `resolvePickerColor()` / `ColorField` helpers.
- CSS-variable/custom token text remains truthful after swatch interaction in
  both adopters.
- Clear behavior is consistent with the landed shared seam across both editors.
