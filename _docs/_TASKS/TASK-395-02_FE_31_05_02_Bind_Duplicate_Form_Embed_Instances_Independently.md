# TASK-395-02: FE-31-05-02 - Bind duplicate Form Embed instances independently
# FileName: TASK-395-02_FE_31_05_02_Bind_Duplicate_Form_Embed_Instances_Independently.md
...
Execution-ready leaf task for FE-31-05-02 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_FORM_EMBED_WIDGET.md` and parent `TASK-395`.

Second Form Embed on the same page does not bind runtime.

Status log:

- 2026-06-02: Moved to In Progress with TASK-395 family implementation.
- 2026-06-02: Done. Form runtime now exposes an idempotent shared binder,
  rebinds on repeated script execution, microtask, and DOMContentLoaded, and
  independently binds later duplicate Form Embed instances.

## Sub-Tasks

- [ ] Reproduce FE-31-05-02 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Make runtime boot idempotent and bind all roots by instance id without global single-root exit.
...
**Regression-test shape:** Runtime test with two embeds and independent submit state.
...
- `git diff --check`
- Focused regression: Runtime test with two embeds and independent submit state.
...
- FE-31-05-02 is fixed or reclassified with fresh evidence in the report and parent task.
