# TASK-397-04: NV-31-05-04 - Complete Visual path metadata
# FileName: TASK-397-04_NV_31_05_04_Complete_Visual_Path_Metadata.md
...
Execution-ready leaf task for NV-31-05-04 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NAVIGATION_WIDGET.md` and parent `TASK-397`.

Visual controls work but `data-widget-control-path` coverage is incomplete.

## Sub-Tasks

- [x] Reproduce NV-31-05-04 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Wrap all mutating Navigation Visual controls and align editor contract.
...
**Regression-test shape:** Editor metadata scan has no unwrapped Navigation controls.
...
- `git diff --check`
- Focused regression: Editor metadata scan has no unwrapped Navigation controls.
...
2026-06-02: Closed in Navigation scope. Navigation Visual mutating controls now
emit stable `data-widget-control-path` metadata via shared editor controls.
Focused regression:
`NavigationVisualEditor renders path metadata for every declared Visual
writable path`.
...
- NV-31-05-04 is fixed or reclassified with fresh evidence in the report and parent task.
