...
```text
1. Exercise the Quote block through the final targeted validation set.
2. Run the lane-owned tests plus lint/type checks for the quote contract.
3. Replay the browser audit steps and confirm the published front still renders a real `quote` block with no unresolved dedicated-control drift.
