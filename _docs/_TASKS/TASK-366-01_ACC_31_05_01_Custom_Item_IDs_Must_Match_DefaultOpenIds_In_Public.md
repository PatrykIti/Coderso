# TASK-366-01: ACC-31-05-01 - Custom item IDs must match `defaultOpenIds` in public runtime
# FileName: TASK-366-01_ACC_31_05_01_Custom_Item_IDs_Must_Match_DefaultOpenIds_In_Public.md
...
Execution-ready leaf task for ACC-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_ACCORDION_WIDGET.md` and parent `TASK-366`.

Normalizer validates against custom item IDs, but renderer opens by slot instance IDs only.
...
**Helper/function shape:** Resolve each item with both `instanceId` and `selectionId`; match default-open settings against custom item IDs and legacy instance IDs while keeping DOM/slot IDs stable.
...
**Regression-test shape:** Renderer regression: slots `item:1/2`, item ids `alpha/beta`, default `beta` opens second details with `aria-expanded=true`.
...
- `git diff --check`
- Focused regression: Renderer regression: slots `item:1/2`, item ids `alpha/beta`, default `beta` opens second details with `aria-expanded=true`.
...
- ACC-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
