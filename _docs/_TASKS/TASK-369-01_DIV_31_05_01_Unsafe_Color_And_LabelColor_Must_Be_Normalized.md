# TASK-369-01: DIV-31-05-01 - Unsafe `color` and `labelColor` must be normalized
# FileName: TASK-369-01_DIV_31_05_01_Unsafe_Color_And_LabelColor_Must_Be_Normalized.md
...
Execution-ready leaf task for DIV-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_DIVIDER_WIDGET.md` and parent `TASK-369`.

Divider accepts plain strings for `color` and `labelColor`, so imported `url(javascript:)` and `expression(...)` appear in inline styles.
...
**Helper/function shape:** Add bounded color sanitizer in `normalizeDividerData`; preserve UI hex/token values and omit unsafe import values.
...
**Regression-test shape:** Renderer regression: unsafe color strings absent from SSR, dashed/dotted safe colors still compute correctly.
...
- `git diff --check`
- Focused regression: Renderer regression: unsafe color strings absent from SSR, dashed/dotted safe colors still compute correctly.
...
- `_docs/_TASKS/TASK-369_Divider_Widget_31_05_UI_Audit_Remediation_Family.md` parent status/checklist when this leaf starts or closes.
...
- DIV-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
