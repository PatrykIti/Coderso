# TASK-383-01: PF-31-05-01 - Inactive category filter must not be reported active in Latest mode
# FileName: TASK-383-01_PF_31_05_01_Inactive_Category_Filter_Must_Not_Be_Reported_Active.md
...
Execution-ready leaf task for PF-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_POSTS_FEED_WIDGET.md` and parent `TASK-383`.

Category/tag filter value survives after switching back to Latest, while runtime only applies category in category mode.
...
**Helper/function shape:** Advanced should include category only when `source.mode === "category"` or mark saved category as inactive outside category mode.
...
**Regression-test shape:** UI regression: category set -> Latest -> Advanced does not present category as active.
...
No new public write. Seed data must use existing internal/admin paths; public renderer stays read-only and safe-link bounded.
...
- `git diff --check`
- Focused regression: UI regression: category set -> Latest -> Advanced does not present category as active.
...
- `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_POSTS_FEED_WIDGET.md`
- `_docs/_TASKS/TASK-383_Posts_Feed_Widget_31_05_UI_Audit_Remediation_Family.md` parent status/checklist when this leaf starts or closes.
...
- PF-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
