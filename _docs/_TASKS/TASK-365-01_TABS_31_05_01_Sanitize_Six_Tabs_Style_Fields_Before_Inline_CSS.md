# TASK-365-01: TABS-31-05-01 - Sanitize six Tabs style fields before inline CSS
# FileName: TASK-365-01_TABS_31_05_01_Sanitize_Six_Tabs_Style_Fields_Before_Inline_CSS.md
...
Execution-ready leaf task for TABS-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_TABS_WIDGET.md` and parent `TASK-365`.

Tabs schema accepts style strings and normalizer forwards them, allowing imported `javascript:`/`expression(` values into public inline style.

## Sub-Tasks

- [x] Reproduce TABS-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Add Tabs-owned safe color normalizer or shared helper and use it for background, border, active/inactive, panel, and text style fields; unsafe values normalize to undefined.
...
**Regression-test shape:** Renderer regression: unsafe style strings are absent from SSR; safe hex/token values remain.
...
- `git diff --check`
- Focused regression: Renderer regression: unsafe style strings are absent from SSR; safe hex/token values remain.
...
- TABS-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Reproduced the report drift in code: Tabs used the generic clearable style trimmer for six color-only fields.
- `normalizeTabsData()` now resolves `surfaceColor`, `borderColor`, `activeBackgroundColor`, `activeTextColor`, `inactiveTextColor`, and `panelBackgroundColor` through `resolveClearableCssColorValue()`.
- `TabsBlock` revalidates normalized style values before assigning inline CSS, preserving safe hex, `rgb/rgba`, `hsl/hsla`, `transparent`, `currentColor`, and `var(--color-*)` values while dropping unsafe strings.
- Added renderer regressions proving unsafe strings are absent from SSR and bounded imported colors remain.
