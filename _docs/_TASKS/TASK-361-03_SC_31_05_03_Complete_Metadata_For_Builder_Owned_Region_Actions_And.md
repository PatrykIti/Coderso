# TASK-361-03: SC-31-05-03 - Complete metadata for builder-owned Region actions and labels
# FileName: TASK-361-03_SC_31_05_03_Complete_Metadata_For_Builder_Owned_Region_Actions_And.md
...
Execution-ready leaf task for SC-31-05-03 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_SECTION_WIDGET.md` and parent `TASK-361`.

Region add action and label input work but lack stable `data-widget-control-path`, so automation cannot map them to `regions`/slot instance ownership.

## Sub-Tasks

- [x] Reproduce SC-31-05-03 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Wrap `Add Region` and region label inputs with action/write metadata; use stable paths such as `regions` or `regions.<instanceId>.label` and keep editor-only labels out of public runtime.
...
**Regression-test shape:** Page-builder DOM test: no unwrapped Region action and label input exposes a stable path.
...
- `git diff --check`
- Focused regression: Page-builder DOM test: no unwrapped Region action and label input exposes a stable path.
...
- SC-31-05-03 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Report fixture evidence showed `Add Region` as an unwrapped button and the
  Region label input without a stable path despite mutating Section
  `regions[]` editor metadata.
- Extended builder slot controls with path/ownership metadata and rendered Add
  Region through `WidgetControlRow`.
- Added Section-specific paths for region row and label controls:
  `regions.<instanceId>` and `regions.<instanceId>.label`, with label ownership
  marked writable.

## Validation Executed (2026-06-01)

- `NODE_ENV=test ./node_modules/.bin/vitest run --config vitest.config.ts tests/vitest/ui/section-editor-wave.test.tsx`
- `bun --cwd core lint`
- `bun --cwd core lint:types`
