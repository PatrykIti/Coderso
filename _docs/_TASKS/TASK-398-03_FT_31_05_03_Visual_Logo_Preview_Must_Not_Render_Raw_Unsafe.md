# TASK-398-03: FT-31-05-03 - Visual logo preview must not render raw unsafe `brand.logoUrl`
# FileName: TASK-398-03_FT_31_05_03_Visual_Logo_Preview_Must_Not_Render_Raw_Unsafe.md
...
Execution-ready leaf task for FT-31-05-03 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_FOOTER_WIDGET.md` and parent `TASK-398`.

Visual preview renders unsafe raw logo URL.

Status log:

- 2026-06-02: Moved to In Progress with TASK-398 Footer remediation.
- 2026-06-02: Done. Visual logo preview now uses the Footer safe image URL
  normalizer and shows unsafe saved URLs as replace-or-clear state.

## Sub-Tasks

- [x] Reproduce FT-31-05-03 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Use shared safe media URL normalizer before preview and public render; show placeholder for unsafe values.
...
**Regression-test shape:** UI/renderer regression for unsafe logo URL.
...
- `git diff --check`
- Focused regression: UI/renderer regression for unsafe logo URL.
...
- FT-31-05-03 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Observed stale report state: public runtime omitted unsafe logo images, but
  Visual preview rendered raw `brand.logoUrl`.
- Exported and reused `normalizeFooterImageSrc()` in `BrandLogoField`; unsafe
  saved logo URLs now render warning copy instead of an image.
