...
4. Render the normalized payload snapshot after diagnostics, not as the only
   Advanced value.
...
- Raw payload view must include only normalized Split Layout data, not page,
...
- Anti-abuse: diagnostics are derived from bounded normalized widget data and
  must not expose arbitrary classes beyond owner-resolved token labels.
...
- Advanced mode gives Split Layout users useful breakpoint diagnostics or a
