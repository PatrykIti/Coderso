...
```text
1. Exercise the Group block through the final targeted validation set.
2. Run the lane-owned tests plus lint/type checks for the group contract.
3. Replay the browser audit steps and confirm the published front still renders a real `group` block with no unresolved dedicated-control drift.
