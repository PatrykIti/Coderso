# TASK-396-02: CONTACT-31-05-02 - Reset submit button label after failure
# FileName: TASK-396-02_CONTACT_31_05_02_Reset_Submit_Button_Label_After_Failure.md
...
Execution-ready leaf task for CONTACT-31-05-02 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_CONTACT_WIDGET.md` and parent `TASK-396`.

After submit error, button remains `Sending...`.

Status log:

- 2026-06-02: Moved to In Progress with TASK-396 Contact remediation.
- 2026-06-02: Done. CONTACT-31-05-02 / CT-31-05-02 is closed with explicit
  submit-label markers and shared runtime label snapshotting.

## Sub-Tasks

- [x] Reproduce CONTACT-31-05-02 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Use finally/error branch to restore label/disabled state and expose accessible error text.
...
**Regression-test shape:** Runtime test for failed submit restores button state.
...
- `git diff --check`
- Focused regression: Runtime test for failed submit restores button state.
...
- CONTACT-31-05-02 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Contact now emits `data-form-submit-label`.
- Shared Forms runtime stores the original button label during bind so failure
  and `finally` restore the user-facing label instead of `Sending...`.
- Regression coverage lives in `tests/vitest/widgets/formRuntimeScript.test.ts`.
