# TASK-395-04: FE-31-05-04 - Normalize checkbox payload to backend schema
# FileName: TASK-395-04_FE_31_05_04_Normalize_Checkbox_Payload_To_Backend_Schema.md
...
Execution-ready leaf task for FE-31-05-04 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_FORM_EMBED_WIDGET.md` and parent `TASK-395`.

Browser sends `on` while backend accepts boolean/true/1/false/0.

Status log:

- 2026-06-02: Moved to In Progress with TASK-395 family implementation.
- 2026-06-02: Done. Checkbox controls render `value="true"` and runtime payload
  serialization emits checked checkbox values as booleans accepted by the
  backend schema instead of browser-default `on`.

## Sub-Tasks

- [ ] Reproduce FE-31-05-04 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Serialize checkbox values as accepted boolean tokens or widen schema intentionally with normalizer.
...
**Regression-test shape:** Submit test for checked/unchecked checkbox payload.
...
- `git diff --check`
- Focused regression: Submit test for checked/unchecked checkbox payload.
...
- FE-31-05-04 is fixed or reclassified with fresh evidence in the report and parent task.
