...
The section inserts and renders, but `centered` and `full-width` variants remain
data-only no-ops on the published front, the audit also calls out an empty
Responsive tab, and the entire inspector still lacks the dedicated control
widgets expected by the redesign.
