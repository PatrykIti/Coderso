# TASK-390-01: TL-31-05-01 - CTA saved in process/compact mode must render or be called out as hidden
# FileName: TASK-390-01_TL_31_05_01_CTA_Saved_In_Process_Compact_Mode_Must_Render.md
...
Execution-ready leaf task for TL-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_TIMELINE_WIDGET.md` and parent `TASK-390`.

Visual allows step CTA, Advanced reports safe CTA, but process/compact runtime renders no link.

## Sub-Tasks

- [x] Reproduce TL-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Prefer rendering compact CTA as inline/secondary row without nested anchors; otherwise show Visual/Advanced hidden-by-mode warnings.
...
**Regression-test shape:** Renderer and UI regressions for `mode=process` + step CTA.
...
- `git diff --check`
- Focused regression: Renderer and UI regressions for `mode=process` + step CTA.
...
- TL-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
...
Closed on 2026-06-02. Compact/process Timeline layouts now render configured step CTAs as compact inline links while the existing whole-step suppression continues to prevent nested anchors.
