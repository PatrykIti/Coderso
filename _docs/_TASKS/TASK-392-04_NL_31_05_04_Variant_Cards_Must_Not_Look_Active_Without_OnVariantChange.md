# TASK-392-04: NL-31-05-04 - Variant cards must not look active without `onVariantChange`
# FileName: TASK-392-04_NL_31_05_04_Variant_Cards_Must_Not_Look_Active_Without_OnVariantChange.md
...
Execution-ready leaf task for NL-31-05-04 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NEWSLETTER_WIDGET.md` and parent `TASK-392`.

Cards appear selected/mutable even when variant change handler is absent.
...
**Helper/function shape:** Disable cards or render read-only state when `onVariantChange` is unavailable; keep admin copy truthful.
...
**Regression-test shape:** UI regression for missing `onVariantChange` path.
...
- `git diff --check`
- Focused regression: UI regression for missing `onVariantChange` path.
...
- NL-31-05-04 is fixed or reclassified with fresh evidence in the report and parent task.
