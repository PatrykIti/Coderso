...
testing webhooks, removing IP allowlist entries, and saving risky security
policy changes.
...
| Relevant clients/routes/services | Ensure strict schemas, CSRF, most-specific RBAC, lockout guards, and audit events. |
...
- RBAC: most-specific high-risk permission required; define missing
  session/security/API key/webhook/IP allowlist permissions before widening
  `settings:write`.
...
- Bun route/service tests: CSRF, strict body validation, most-specific RBAC,
  lockout guard, one-time API key secret handling, mapped errors, and audit.
