...
- Exact `entryBytes = UTF8(key).byteLength + value.byteLength`; track aggregate
  bytes and entry count on insert, replacement, expiry, delete and eviction.
- Reject/skip values above normalized per-entry/aggregate limits before state
...
- `get` lazily evicts expired entries using an injected monotonic clock. Each
  operation performs at most the exported bounded sweep batch; no unbounded
...
- The backend-neutral circuit counts store/transport failures, never loader or
  ...
  canonical family/schema/input digest. That map retains promises only and is
...
async function getOrLoad(policy, input, context, loader) {
  ...
  if (!generations) {
    return joinBypassSingleFlight(policy, input, loader, "generation_unavailable");
  }
...
## Regression Shape and Acceptance
...
stats have no key/value fragments.
