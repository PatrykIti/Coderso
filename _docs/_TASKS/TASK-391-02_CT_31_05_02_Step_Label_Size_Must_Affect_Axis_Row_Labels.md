# TASK-391-02: CT-31-05-02 - `Step label size` must affect axis row labels
# FileName: TASK-391-02_CT_31_05_02_Step_Label_Size_Must_Affect_Axis_Row_Labels.md
...
Execution-ready leaf task for CT-31-05-02 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_COMPARE_TIMELINE_WIDGET.md` and parent `TASK-391`.

Axis row hardcodes `text-xs`, so step label size only affects track cells.
...
**Helper/function shape:** Pass `stepLabelSizeClass` into `CompareAxisRow` and use it instead of hard-coded size.
...
**Regression-test shape:** Renderer test targeting `data-compare-axis=true` and track cells separately.
...
- `git diff --check`
- Focused regression: Renderer test targeting `data-compare-axis=true` and track cells separately.
...
- CT-31-05-02 is fixed or reclassified with fresh evidence in the report and parent task.
