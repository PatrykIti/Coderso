# TASK-397-03: NV-31-05-03 - Drawer active link clones need truthful `aria-current` semantics
# FileName: TASK-397-03_NV_31_05_03_Drawer_Active_Link_Clones_Need_Truthful_Aria_Current.md
...
Execution-ready leaf task for NV-31-05-03 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NAVIGATION_WIDGET.md` and parent `TASK-397`.

Drawer active link has `aria-current` only on first clone.

## Sub-Tasks

- [x] Reproduce NV-31-05-03 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Regression-test shape:** A11y regression for drawer active current markers.
...
- `git diff --check`
- Focused regression: A11y regression for drawer active current markers.
...
2026-06-02: Closed in Navigation scope. Runtime active-link sync now applies
`aria-current="page"` to every duplicate responsive clone for the best active
match. Focused regression:
`navigation runtime marks duplicate drawer active clones with aria-current`.

Validation is recorded in parent TASK-397 closure notes.

## Acceptance Criteria

- NV-31-05-03 is fixed or reclassified with fresh evidence in the report and parent task.
