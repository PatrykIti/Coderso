...
_docs/PAGE_MODEL.md, the relevant Custom SVG/widget guide, this family’s task statuses,
...
placeholder, and browser isolation is verified. Do not publish the private reproduction,
utility token, attack string, or an actionable click-overlay recipe.
...
update SECURITY_SPEC/PAGE_MODEL/guide with defensive invariants only;
run exact targeted and strict scans without allowlists or ignores;
run five fresh post-audit lenses and resolve every HIGH/MEDIUM drift;
