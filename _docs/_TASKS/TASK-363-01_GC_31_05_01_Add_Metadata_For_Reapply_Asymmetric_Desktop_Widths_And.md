# TASK-363-01: GC-31-05-01 - Add metadata for `Reapply asymmetric desktop widths` and shared `Add Column`
# FileName: TASK-363-01_GC_31_05_01_Add_Metadata_For_Reapply_Asymmetric_Desktop_Widths_And.md
...
Execution-ready leaf task for GC-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_GRID_COLUMNS_WIDGET.md` and parent `TASK-363`.

Controls work, but some buttons are not wrapped with stable `data-widget-control` and path metadata.
...
**Helper/function shape:** Extend Grid Columns and shared slot actions so every mutating action has an action id, control path, ownership, and stable slot/column target.
...
**Regression-test shape:** Editor DOM test: strict metadata scan has no unwrapped Grid Columns action controls and preserves existing slot behavior.
...
- `git diff --check`
- Focused regression: Editor DOM test: strict metadata scan has no unwrapped Grid Columns action controls and preserves existing slot behavior.
...
- `_docs/_TASKS/TASK-363_Grid_Columns_Widget_31_05_UI_Audit_Remediation_Family.md` parent status/checklist when this leaf starts or closes.
...
- GC-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
