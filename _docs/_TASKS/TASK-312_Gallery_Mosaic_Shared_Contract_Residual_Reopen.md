# TASK-312: Gallery Mosaic Shared Contract Residual Reopen

# FileName: TASK-312_Gallery_Mosaic_Shared_Contract_Residual_Reopen.md
...
| Finding or residual drift | TASK-312 owner | Notes |
|---|---|---|
| UX-01 duplicated Advanced controls for current shared fields | TASK-312-01 | Shared editor-mode ownership residual from TASK-256-01. |
...
- [x] TASK-312-01: Gallery Mosaic Shared Editor Truthfulness Residuals
- [x] TASK-312-02: Gallery Mosaic Shared Runtime Semantics Residuals
- [x] TASK-312-03: Gallery Mosaic Shared Residual Closure

## Implementation Order

1. Finish `TASK-312-01` first so current editor-mode ownership and current
   media truthfulness are stable before runtime/product follow-ups touch the
   widget again.
2. Finish `TASK-312-02` next so current runtime semantics match the settled
   shared contract before `TASK-270-03` adds new media fields.
3. Finish `TASK-312-03` before reopening `TASK-270` implementation work so the
...
- Run `git status --short --branch` before implementation, staging, commit, and
  merge-back.
- Stage only `TASK-312*` files, Gallery Mosaic shared owner files/tests/docs,
...
- `TASK-270` no longer depends on missing Gallery Mosaic shared prerequisites in
  the live checkout.
- Shared current-contract editor/runtime rows are fixed under `TASK-312`
...
- 2026-05-18: `TASK-312` is fully closed. The reopened Gallery Mosaic shared
  baseline now has:
