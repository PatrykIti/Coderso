...
```text
1. Exercise the Card block through the final targeted validation set.
2. Run the lane-owned tests plus lint/type checks for the card contract.
3. Replay the browser audit steps and confirm the published front still renders a real `card` block with no unresolved dedicated-control drift.
