...
- Saved `asymmetric` blocks with equal or custom desktop spans now show truthful status copy and a bounded reapply action instead of silently drifting, and both the effective state and reapply path now follow live slot order rather than raw `columns[]` alone.
