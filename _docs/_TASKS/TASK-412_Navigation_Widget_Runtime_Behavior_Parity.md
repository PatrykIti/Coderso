# TASK-412: Navigation Widget Runtime Behavior Parity
# FileName: TASK-412_Navigation_Widget_Runtime_Behavior_Parity.md
...
- Merge resolution with the assistant branch added the controlled Advanced
  assistant `collapse-on-scroll` menu behavior so generated Navigation blocks
  can set `behavior.collapseOnScroll` through typed registry-owned options.
...
- Assistant-branch merge validation reran `coderso-dev-core-host` on
  `coderso-b.localhost`, verified the Advanced assistant exposes
  `collapse-on-scroll`, created a temporary public QA page with
  `behavior.collapseOnScroll`, confirmed the public sticky Navigation collapsed
  on window scroll and opened its mobile drawer panel, then deleted the temporary
