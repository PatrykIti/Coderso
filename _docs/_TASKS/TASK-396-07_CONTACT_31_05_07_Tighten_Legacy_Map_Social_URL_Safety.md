# TASK-396-07: CONTACT-31-05-07 - Tighten legacy map/social URL safety
# FileName: TASK-396-07_CONTACT_31_05_07_Tighten_Legacy_Map_Social_URL_Safety.md
...
Execution-ready leaf task for CONTACT-31-05-07 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_CONTACT_WIDGET.md` and parent `TASK-396`.

Legacy arbitrary web URLs are still allowed for map/social paths.

Status log:

- 2026-06-02: Moved to In Progress with TASK-396 Contact remediation.
- 2026-06-02: Done. CONTACT-31-05-07 / CT-31-05-07 is closed with
  allowlisted map/social runtime and bounded CSS color normalization.

## Sub-Tasks

- [x] Reproduce CONTACT-31-05-07 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Use shared safe URL/link normalizers with explicit allowed protocols and reject unsafe/unsupported legacy URLs.
...
**Regression-test shape:** Renderer tests for unsafe map/social URLs fail closed.
...
- `git diff --check`
- Focused regression: Renderer tests for unsafe map/social URLs fail closed.
...
- CONTACT-31-05-07 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Map iframes render only HTTPS Google Maps embed URLs.
- Social links publish only known platform HTTPS hosts; custom/arbitrary
  legacy links stay dormant.
- Contact inline color values now use bounded CSS color normalization.
