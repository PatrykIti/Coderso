# TASK-368-01: SPC-31-05-01 - Lock responsive/fixed/unsafe-length behavior as a regression guard
# FileName: TASK-368-01_SPC_31_05_01_Lock_Responsive_Fixed_Unsafe_Length_Behavior_As_A.md
...
Execution-ready leaf task for SPC-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_SPACER_WIDGET.md` and parent `TASK-368`.

The report confirms responsive heights, fixed lengths, guide gating, unsafe length fallback, and invalid variant fail-closed behavior. Without a guard, later edits can reopen those paths silently.
...
**Helper/function shape:** Add or verify focused tests around responsive desktop/tablet/mobile markers, custom length sanitization, guide hidden state, and invalid variant placeholder.
...
**Regression-test shape:** Targeted Spacer renderer/editor tests plus a Playwright smoke note prove current no-finding status remains intentional.
...
- `git diff --check`
- Focused regression: Targeted Spacer renderer/editor tests plus a Playwright smoke note prove current no-finding status remains intentional.
...
- `_docs/_TASKS/TASK-368_Spacer_Widget_31_05_UI_Audit_Closure_and_Regression_Guard_Family.md` parent status/checklist when this leaf starts or closes.
...
- SPC-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
