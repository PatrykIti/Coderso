# TASK-366-02: ACC-31-05-02 - Sanitize Accordion surface/text/border color fields
# FileName: TASK-366-02_ACC_31_05_02_Sanitize_Accordion_Surface_Text_Border_Color_Fields.md
...
Execution-ready leaf task for ACC-31-05-02 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_ACCORDION_WIDGET.md` and parent `TASK-366`.

Imported style strings flow into container, summary, description, and panel inline styles.

## Sub-Tasks

- [x] Reproduce ACC-31-05-02 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Add Accordion-owned safe color normalizer for surface, border, summary text, and description text fields; omit unsafe strings.
...
**Regression-test shape:** SSR regression: no `javascript:`/`expression(` in Accordion HTML; safe swatches still render.
...
- `git diff --check`
- Focused regression: SSR regression: no `javascript:`/`expression(` in Accordion HTML; safe swatches still render.
...
- ACC-31-05-02 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Reproduced the report drift in code: Accordion color fields were normalized through raw trimming/fallbacks and then emitted into inline styles.
- Added Accordion-local color resolution that uses the shared bounded CSS color helper, preserves safe legacy hyphenated tokens, and rejects unsafe CSS fragments such as `url(`, `expression(`, `javascript:`, `data:`, semicolon injection, braces, and HTML-like delimiters.
- Runtime style assembly now revalidates every Accordion color before writing `backgroundColor`, `borderColor`, or `color`.
- Added SSR regressions proving unsafe imports are absent from HTML and safe colors/legacy tokens still render.
