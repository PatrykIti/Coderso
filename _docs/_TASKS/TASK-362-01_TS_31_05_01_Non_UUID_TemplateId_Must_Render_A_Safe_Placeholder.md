# TASK-362-01: TS-31-05-01 - Non-UUID `templateId` must render a safe placeholder, not HTTP 500
# FileName: TASK-362-01_TS_31_05_01_Non_UUID_TemplateId_Must_Render_A_Safe_Placeholder.md
...
Execution-ready leaf task for TS-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_TEMPLATE_SECTION_WIDGET.md` and parent `TASK-362`.

Runtime trims arbitrary strings and queries a UUID column, so invalid IDs throw Postgres before the widget can return `template_missing`.

## Sub-Tasks

- [x] Reproduce TS-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Validate `templateId` before `getWidgetTemplate`; return `template_invalid` or `template_missing` without querying DB for non-UUID strings. Apply the same validation on admin save/import/assistant mutations.
...
**Regression-test shape:** Runtime service and public renderer tests: non-UUID ID does not query DB, returns HTTP 200 placeholder, and does not expose raw invalid ID.
...
- `git diff --check`
- Focused regression: Runtime service and public renderer tests: non-UUID ID does not query DB, returns HTTP 200 placeholder, and does not expose raw invalid ID.
...
- TS-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
...
- Malformed non-UUID Template Section IDs are rejected by schema-backed page and widget template write paths.
- Public legacy runtime renders `template_missing` safely and does not query `widget_templates.id` with malformed IDs.
- Legacy widget template reads remain tolerant so nested old data can be hydrated into safe placeholders instead of failing before render.
- Added service, route, Vitest, and DB-backed public runtime regressions.
