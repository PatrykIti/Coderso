# TASK-393-03: BC-31-05-03 - Define admin canvas behavior for runtime-only controls
# FileName: TASK-393-03_BC_31_05_03_Define_Admin_Canvas_Behavior_For_Runtime_Only_Controls.md
...
Execution-ready leaf task for BC-31-05-03 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_BOOKING_CALENDAR_WIDGET.md` and parent `TASK-393`.

Admin week picker shell is empty/static while public runtime hydrates days and slots.
...
**Helper/function shape:** Add an explicit noninteractive admin placeholder for runtime-only calendar controls that names the public-only behavior and links diagnostics to the configured service/date state; do not bootstrap public booking submission runtime inside the admin canvas.
...
**Regression-test shape:** Admin preview test: week mode shows days or explicit runtime-preview boundary copy.
...
- `git diff --check`
- Focused regression: Admin preview test: week mode shows days or explicit runtime-preview boundary copy.
...
- BC-31-05-03 is fixed or reclassified with fresh evidence in the report and parent task.
