# TASK-394-02: AF-31-05-02 - Public reservation API must verify server-generated slot match
# FileName: TASK-394-02_AF_31_05_02_Public_Reservation_API_Must_Verify_Server_Generated_Slot.md
...
Execution-ready leaf task for AF-31-05-02 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_APPOINTMENT_FORM_WIDGET.md` and parent `TASK-394`.

Status log:

- 2026-06-02: Moved to In Progress with TASK-394 family implementation.
- 2026-06-02: Done; public reservation submit now verifies exact server-generated slot match.

Direct POST with valid nonce can submit out-of-hours/wrong-duration slot data from client.

## Sub-Tasks

- [x] Reproduce AF-31-05-02 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** After nonce/access check, regenerate availability for selected service/resource/date/timezone and require exact start/end match; reject past/out-of-policy/wrong-duration slots.
...
**Regression-test shape:** Bun public API tests for out-of-hours, wrong end, past, and no public slot match.
...
- `git diff --check`
- Focused regression: Bun public API tests for out-of-hours, wrong end, past, and no public slot match.
...
- AF-31-05-02 is fixed or reclassified with fresh evidence in the report and parent task.
...
- `publicBookingApi.ts` normalizes submitted instants, derives the resource-local
  date/timezone, regenerates availability, and rejects non-matching start/end
  pairs before `createBookingReservation`.
- Bun public API regression covers valid nonce + wrong-duration slot returning
