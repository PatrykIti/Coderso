...
**Dependencies:** TASK-265, TASK-305
...
Adopt the shared color-control contract from TASK-305 for Entry Teaser
surface and border fields.

This leaf owns only the Entry Teaser-specific part of report finding E-14 from
`_docs/PLAYWRIGHT/REPORT_ENTRY_TEASER_WIDGET.md`. TASK-305 owns the generic
...
- Replace Entry Teaser surface/border text-only controls with the shared
  color-control hook/component after TASK-305 lands it.
...
|---|---|
| `core/admin/ui/widgets/editors/EntryTeaserEditors.tsx` | Replace surface/border `ClearableInputField` usage with the shared color-control hook/component from TASK-305 while keeping clear actions. |
...
- Reject-unknown validation: no new persisted keys unless TASK-305 requires a
...
- If TASK-305 has not landed the shared control, keep this leaf blocked
...
- [ ] Consume the shared TASK-305 color-control hook/component.
...
- Shared TASK-305 color-control tests when the shared hook/component is
...
- E-14 has a concrete Entry Teaser owner after TASK-305 defines the generic
