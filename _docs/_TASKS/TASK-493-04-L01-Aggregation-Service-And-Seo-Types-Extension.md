...
- **Owning module(s) to create-or-extend:**
  - `core/services/seo/seoTypes.ts` (**extend** — add `SeoOverview`,
    `SeoSearchPerformance`, `SeoDocumentPerformance`, and a `performance?` field
    on a new `SeoListItemWithPerformance` (do **not** mutate the existing
...
export type SeoDocumentPerformance = {
  indexingState: string;         // SeoIndexingState
  impressions: number;
  clicks: number;
  ctr: number;
  position: number;
...
import { toNumber, normalizeIndexingState } from "./seoSearchPerformanceTypes";

// pure, unit-testable merge over already-fetched rows
export function aggregateOverview(rows: {
  indexed: IndexedRow[]; metrics: MetricRow[]; sitemap?: SitemapRow | null; avgScore: number;
...
export async function getSeoOverview(): Promise<SeoOverview> {
  ...
  const avgScore = await meanSeoScore();           // reuse existing seo_documents.score
  return aggregateOverview({ indexed, metrics, sitemap, avgScore });
}
```

**Data flow:** select rows from the 01 tables (+ existing `seo_documents`) →
coerce numeric columns via `toNumber` → fold into `SeoOverview` /
`SeoSearchPerformance` → return. DB access is thin; the folding math lives in
pure exported helpers (`aggregateOverview`, `aggregateSearchPerformance`) so it
