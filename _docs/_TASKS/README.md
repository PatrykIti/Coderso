...
| ID | Title | Priority | Effort | Notes |
|----|-------|----------|--------|-------|
| TASK-190 | Blueprint Composer Foundation | High | Very Large | In progress: capability manifest/registry, candidate planning, provider/shadow safety layer, deterministic composition graph/conflicts, initial action assembly, schema/listing merge, page-upsert collection-link persistence, and the collection workspace route/read/cache/UI shell are landed; supported mixed-capability and primary-plus-gated setup requests now stay on the composed path before provider drafting can bypass them, while detail-page generic packaging, detail-template editor, assistant workspace context, review metadata, and no-duplicate closure waves remain |
...
| TASK-190-06 | Admin Surface Composer | High | Large | In progress: admin-screen layout, binding/metadata safety, and collection workspace route/read/cache/UI now land through shared owner seams; detail-template editor and assistant workspace context leaves remain |
| TASK-190-06-03 | Collection Workspace and Template Editor | High | Large | In progress: the internal Engine route, bounded server read model, deterministic canonical resolution/read permissions, cache/prefetch, and first UI shell are landed; detail-template editor and assistant context leaves remain |
...
| ID | Title | Priority | Effort | Notes |
|----|-------|----------|--------|-------|
| TASK-190-06-03-01 | Collection Workspace Route, Read Model, and Canonical Resource Linking | High | Medium | Done: route/read model, canonical resolution, cached client helpers, Engine prefetch, and first workspace shell are landed |
