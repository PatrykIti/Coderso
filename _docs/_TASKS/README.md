...
| TASK-502 | Menu Design Fixes V2 — Brand Text, Tablet Cascade, Canvas WYSIWYG & Nested Submenus | High | Large | 7 recon-confirmed Menu Design bugs (2026-07-02) + 2 owner decisions: editable `brand.props.text` with text→siteName fallback on canvas AND front (canvas stops showing the menu name); tablet UN-deferred — `MENU_RESPONSIVE_BREAKPOINT_KEYS += "tablet"` with the Pages cascade (tablet AND mobile each inherit DESKTOP), bounded tablet `@media` + real canvas tablet branch; `mobileMode`/`dropdownDirection` become device-scoped BASE-writing controls (shells removed; 501-era dead overrides pruned non-destructively on read); canvas paints site `--color-*` tokens + site-resolved swatch palettes (PageEditor `useCanvasSiteTokens` pattern) so picked swatches render real colors; divider renders as a vertical separator via doc-scoped context CSS (schema untouched); canvas gates every block through `resolveMenuBlockVisibleForDevice` with a selectable "Hidden" ghost + cta panel gains Size/Open-in-new-tab; recursive `SiteNavItem`/`NavItemsPreview` (delete flatten + parent-dup; doc-scoped nested fly-out CSS — `buildSiteShellCss(null)` byte-identity inviolable). Schema-first, reject-unknown, no new endpoint/RBAC/migration; ≥5-scenario visible-effect smoke mandated. 5 subtasks (01–05). |
| TASK-502-01 | Menu Model — Brand Text & Tablet Breakpoint | High | Medium | `brand.props.text` (trim/120-cap/sparse) + CONSCIOUS `BRAND_PROP_KEYS` extension (fail-closed read trap); `"tablet"` breakpoint with per-breakpoint resolve/patch/clear/prune (both inherit desktop); device-defining carve-out: reject `mobileMode`/`dropdownDirection` in responsive navProps on write, tolerate+prune on stored read. |
| TASK-502-02 | Menu CSS — Tablet Branch, Separators & Nested Sublists | High | Large | Bounded tablet `@media` (`pageResponsiveMediaBounds.tablet`) + canvas tablet branch (tablet⇒desktop mapping removed) + tablet visibility rules; per-divider frame-as-line context rules (inner `<hr>` display:none); doc-scoped nested-sublist hover/fly-out (desktop/tablet) + inline indent (mobile); canvas-only disclosure preview un-hide. No-override + mobile-only docs byte-identical. |
| TASK-502-03 | Front — Recursive Nav & Brand Render | High | Medium | Sole `siteShell.tsx` writer: delete `flattenNavigationDescendants` + the `[item, ...dropdownItems]` parent duplication; recursive `SiteNavItem` (each level owns its `.site-nav-sublist`); resolve the legacy no-doc-header nesting decision EXPLICITLY (base-sheet inheritance proof vs `<details>` fallback) with `buildSiteShellCss(null)` untouched; `BrandRender` text→siteName chain. |
| TASK-502-04 | Design Editor — Canvas WYSIWYG & Device Controls | High | Large | Shared `useCanvasSiteTokens` + `toMenuCanvasColorCssVariableMap` painted on the menu canvas + site palette to every `ColorSwatchControl`; visibility ghost gate (`resolveMenuBlockVisibleForDevice`, dimmed + "Hidden" badge, selectable); brand text Input + real-site-name canvas chain; cta Size/target controls + real leaf preview; `mobileMode` mobile-only / `dropdownDirection` desktop+tablet-only, both base-writing unwrapped; tablet badges/Reset; recursive `NavItemsPreview`; divider preview parity; hover/active copy fix. |
| TASK-502-05 | Menu Fixes Tests, Docs, Closure | Medium | Medium | Full vitest + bun menu matrices, byte-identity guards (`buildSiteShellCss(null)` zero-line; no-override/mobile-only doc CSS), root tsc, the ≥5-scenario real-flow visible-effect playwright smoke (fresh-create e2e; override/reset across 3 devices; 3+-level nesting canvas+front hover; every-panel-control computed-style assertions; publish→front parity at 1280/744/390), changelog (next free after 1210, expected 1211) + 1210 "39/39"→"11/11" correction + docs + board/Statistics. |
