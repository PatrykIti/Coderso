...
| TASK-500 | Screen Builder — Sections, Insertion Targeting & Editor UX | High | Large | Functional follow-up to the shipped TASK-498 look parity: make sections first-class (Add-section CREATES a real named top-level section; select/rename/reorder/delete), give insertion author-directed targeting (into the SELECTED section, before/after any block, INTO a chosen slot of any nested container at arbitrary depth) + drag-to-position across sections/slots, unify the two DISJOINT insert surfaces to ONE canonical kind vocabulary = the 9 chips PLUS the container/composite kinds (`field-group`/`columns` + `record-header`/`rich-text`) from a single constant (drop ONLY the redundant palette FIELDS group; `field-group`/`columns` stay creatable so 500-02 has containers to nest into), dedupe the panel toggle across the two HOST canvases (`ScreenAuthoringCanvas` + `PageEditor`; the shared `CanvasEditor` is controlled read-only and unchanged), and fix the image static-`src` inconsistency. Schema-first, reject-unknown, NO `ScreenDocumentV1` schemaVersion bump; UI/client-state + model only, no route/RBAC change. 5 subtasks (01–05). |
| TASK-500-01 | Sections First-Class & Palette Unification | High | Large | "Add section" creates a real empty named top-level section (select steers insertion, rename, reorder, delete); remove ONLY the redundant command-palette FIELDS group and unify to the single canonical kind vocabulary = the 9 chips PLUS the container/composite kinds (`field-group`/`columns` + `record-header`/`rich-text`) so `field-group`/`columns` stay creatable for 500-02's nesting (`addScreenSection`/`renameScreenSection`/`moveScreenSection`/`removeScreenSection`, `selectedSectionId` steers `handleAddBlock`). |
| TASK-500-02 | Insertion Targeting & Interactivity | High | Large | `addScreenBlockAt`/`moveScreenBlockTo` with a `ScreenInsertTarget` union (section-end/section-index/slot-end/slot-index, index-clamped, fail-soft) + drag-to-position/reorder across sections and slots at arbitrary depth, move-not-clone (id + bindings preserved) with a subtree cycle guard. Replaces the `sections[0]`-only append. |
| TASK-500-03 | Panel-Toggle Dedupe — Shared Shell | Medium | Medium | Remove the redundant in-canvas `PanelRight` close from the two HOST canvases (`ScreenAuthoringCanvas` + `PageEditor`); the shared `CanvasEditor` is controlled read-only and stays UNCHANGED (verify only, no hide affordance added); keep the top toggle + reopen chip as the sole panel-control surface across BOTH Pages + Screens (verify PageEditor otherwise behaviour-identical). |
| TASK-500-04 | Static Block & Image Binding | Medium | Small | Keep unbound heading/text/divider/button rendering on the front; resolve the image inconsistency schema-first (optional static `src` in the image allow-list + scheme-validating normalizer, reject-unknown, non-destructive; renderer src-order bound→static→media→placeholder) — or the justified "requires a bound field" fallback. No schemaVersion bump. |
| TASK-500-05 | Screen-Builder Tests, Docs & Closure | Medium | Medium | Regression matrix (section CRUD, insertion index/slot/target-section, drag reorder + cycle guard, palette unification, toggle dedupe Pages+Screens, static/image render + stored-V4 byte-stability) in the Bun-free Vitest lane, gates, playwright smoke, `CONTENT_TYPES_SPEC.md` + changelog + README/board/Statistics closure. |
| TASK-501 | Menu Per-Device Overrides, Orientation & Block Visibility | High | Large | Follow-up to shipped TASK-499: the Design tab's `DeviceSwitcher` only changes the preview — every writer is flat/global (`setLayoutField`/`setNavField`/`patchBlock`), no `orientation` field exists, and block visibility is all-or-nothing. Owner-chosen UX = the Pages per-breakpoint override pattern: Mobile edits write SPARSE `responsive.mobile` records (section `layout`/`navProps` + block `visibility`), resolved display + Base/Override/Inherited badge + explicit Reset (no auto-remove-on-equality, prune-on-clear), new `orientation: horizontal\|vertical` nav-items enum (default emits nothing), per-block hide-on-mobile / show-only-on-mobile via `data-menu-block-id` CSS gating from the ONE shared `buildMenuRuleSets` (front `@media` + canvas flatten). Tablet deferred (edits write base; canvas keeps tablet⇒desktop). Schema-first reject-unknown + consciously-extended fail-closed read key lists; NO new endpoint/RBAC/migration (`menuUpdateSchema` already carries `document`); NO menu-drawer; `buildSiteShellCss(null)` byte-identity untouched. 4 subtasks (01–04). |
| TASK-501-01 | menuDocumentV2 Responsive Contract | High | Large | Schema keystone: sparse `responsive?: { mobile?: { layout?/navProps? } }` on `MenuSectionV2` + `responsive?: { mobile?: { visibility? } }` on `MenuBlockV2` (ALL block types), `orientation` enum in `normalizeMenuAppearance` + `NAV_ITEMS_PROP_KEYS`, reject-unknown write normalizers (`MenuDocumentError` + path), CONSCIOUS key-list extensions on the fail-closed read (missed list = whole-doc data loss), pure resolve/patch/clear/prune helpers. |
| TASK-501-02 | Menu CSS Responsive Emission | High | Medium | `buildMenuRuleSets` consumes base + mobile-resolved appearance; mobile diff rules emitted AFTER the mobileMode disclosure/inline rules (source-order win); orientation `flex-direction:column;align-items:stretch` (default horizontal emits NOTHING); per-block `data-menu-block-id` visibility gating (mobile branch = hide-on-mobile, desktop branch = show-only-on-mobile) in BOTH front `@media` builder + device-forced canvas flatten (tablet⇒desktop kept); no-override output byte-identical. |
| TASK-501-03 | Design Editor Device-Forked Controls | High | Large | `MenuDesignEditor` writers become device-forked in event handlers (Mobile ⇒ sparse override via `patchMenuSectionForDevice`/`setMenuBlockVisibleForDevice`; desktop/tablet ⇒ base); port Pages `ResponsiveControlShell` (badge + Reset `data-menu-responsive-reset`) around every MenuBarPanel/nav-items control; orientation SegmentedControl; per-block mobile visibility toggle; panel shows resolved values, badges compare against base; no setState-in-effect. |
| TASK-501-04 | Menu Responsive Tests, Docs, Closure | Medium | Medium | Vitest matrix (responsive round-trips, reject-unknown, legacy-doc unchanged + unknown-responsive-degrades-whole-doc asserted consciously, resolve/clear/prune, orientation enum, editor device-fork/badge/reset) + bun lane (menus route persists responsive per-key; CSS emission front+canvas; `siteShellCss` byte-guard ZERO lines) + real-viewport playwright smoke (canvas Mobile + `:3000` ≤639px), changelog (expected 1209) + README/board/Statistics closure. |
