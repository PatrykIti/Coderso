...
| TASK-190-06 | Admin Surface Composer | High | Large | In progress: admin-screen layout and binding/metadata safety now land through shared composer/custom-screen owner seams; the collection workspace route/read-model and canonical-resolution leaves are landed, while cache/UI and detail-template editor leaves remain |
| TASK-190-06-03 | Collection Workspace and Template Editor | High | Large | In progress: the internal Engine route, bounded server read model, and deterministic canonical resolution/read permissions are landed; cache/prefetch/UI shell, detail-template editor, and assistant context leaves remain |
| TASK-190-06-03-01 | Collection Workspace Route, Read Model, and Canonical Resource Linking | High | Medium | In progress: route registration, bounded server summary, deterministic canonical resolution, and owner-read redaction are landed; client cache/UI shell remains |
