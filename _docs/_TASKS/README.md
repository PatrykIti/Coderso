...
| TASK-497 | Posts List & Editor — Prototype-Parity Restyle | Medium | Large | Done (2026-06-30): restyled the Posts list + block editor to the prototype (2nd pass after TASK-479-09 tokens) — quiet `PostsTable` (in place) + single muted editor chrome strip + `EditorRail` "Blocks" rail + flat "Post settings" inspector; kept the three-pane editor (NOT the Pages shell), `PageFilters`/`ListPaginationFooter`, and ALL post functionality + `data-post-editor-*` hooks. Lint/types/Posts vitest (84 files/398 tests)/gates 5/5 green. Changelog 1206. Classic editor out of scope (token-verified). 3 subtasks (01–03). |
...
| TASK-497-02 | Post Editor Restyle | Medium | Large | Done (2026-06-30): single muted chrome strip (2 header rows collapsed), autosave Badge, Save draft + Publish(Rocket), wired undo/redo, optional device toggle, `bg-muted/20` left rail + inserter `EditorRailGroup` look via new shared `EditorRail.tsx`, flat "Post settings" inspector + single SEO sub-card; Block tab + three-pane shell + all `data-post-editor-*`/shortcuts preserved. (changelog 1206) |
| TASK-497-03 | Posts Tests, Docs & Closure | Medium | Medium | Done (2026-06-30): new `posts-editor-chrome-wave` + extended list waves + re-baselined presentation-lock suites (no functional weakening); Posts vitest 84 files/398 tests green, lint/types/gates 5/5; changelog 1206 + README/board/Statistics closure; pure restyle (no contract-doc change). |
