...
| ID | Title | Priority | Effort | Notes |
|----|-------|----------|--------|-------|
| TASK-105 | Real Vitest 100% Coverage Program | High | Large | In progress: fresh 2026-03-15 baseline is `73.28%` lines with `60.83%` branches; `ThemeTemplateDrawer`, `RoleList`, and `UserEditor` all moved materially, and the remaining backlog is increasingly a narrower theme plus low-line admin tail |
| TASK-105-04 | Themes, Booking, Listings, and Forms Wave | High | Large | In progress: booking leaf tabs, `ListingListPage`, and `FormCanvas` are line-closed; `ThemeTemplateDrawer` is now in the mid-80s, so the wave tail is mostly selective theme/page-shell cleanup |
