...
| TASK-499 | Menu: Items Restyle & Design Tab (menuDocumentV2) | Medium | Very Large | PART 1: restyle the menu items/routes editor to the prototype three-pane (preserve live DnD/keyboard/nesting); typed Add-items rail (Pages/Custom-link/Button; Posts/Categories deferred), real `openInNewTab`. PART 2: a "Design" tab = Pages-like canvas on the shared shell over a NEW dedicated `menuDocumentV2` (Option B — no page-schema pollution; thin MenuEditor) with menu blocks (nav-items/brand/cta-button) + a document-driven FRONT renderer that falls back to the byte-identical default menu when Design is empty. 5 subtasks (01–05). |
| TASK-499-01 | Menu Items Editor Restyle | Medium | Large | Re-skin `MenuEditorPage` onto a shared `EditorFrame` (port of EditorPreviewFrame), typed Add-items rail, compact rows, always-on Item-settings inspector, real `openInNewTab` — preserving DnD/keyboard/parent/Save-Publish-Discard verbatim. |
| TASK-499-02 | menuDocumentV2 Contract & Persistence | Medium | Large | New `menuDocumentV2.ts` (own enums + `MENU_DOCUMENT_SCHEMA_VERSION`, leaf-validator reuse only), strict-write/fail-closed-read normalizers + legacy adapter, `resolvePublishedMenuDocument`, per-key envelope merge/publish in `menuService` (no migration, no dropped keys). |
...
| TASK-500 | Screen Builder — Sections, Insertion Targeting & Editor UX | High | Large | Functional follow-up to the shipped TASK-498 look parity: make sections first-class (Add-section CREATES a real named top-level section; select/rename/reorder/delete), give insertion author-directed targeting (into the SELECTED section, before/after any block, INTO a chosen slot of any nested container at arbitrary depth) + drag-to-position across sections/slots, collapse two overlapping insert surfaces to ONE 9-chip vocabulary (drop the palette FIELDS group), dedupe the panel toggle in the shared `CanvasEditor` (Pages+Screens), and fix the image static-`src` inconsistency. Schema-first, reject-unknown, NO `ScreenDocumentV1` schemaVersion bump; UI/client-state + model only, no route/RBAC change. 5 subtasks (01–05). |
| TASK-500-01 | Sections First-Class & Palette Unification | High | Large | "Add section" creates a real empty named top-level section (select steers insertion, rename, reorder, delete); remove the command-palette FIELDS group and collapse to the single canonical 9-chip block vocabulary (`addScreenSection`/`renameScreenSection`/`moveScreenSection`/`removeScreenSection`, `selectedSectionId` steers `handleAddBlock`). |
...
| TASK-500-03 | Panel-Toggle Dedupe — Shared Shell | Medium | Medium | Remove the redundant in-canvas `PanelRight` close from the shared `CanvasEditor`; keep the top toggle + reopen chip as the sole panel-control surface across BOTH Pages + Screens (verify PageEditor otherwise behaviour-identical). |
