# TASK-395-08: FE-31-05-08 - Keep `resolved.submissionNonce` out of persisted widget schema
# FileName: TASK-395-08_FE_31_05_08_Keep_Resolved_SubmissionNonce_Out_Of_Persisted_Widget_Schema.md
...
- [ ] Reproduce FE-31-05-08 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Move nonce to runtime-only projection and reject it from saved widget payloads.
...
**Regression-test shape:** Schema/normalizer test: persisted payload drops/rejects nonce; public render still receives runtime nonce.
...
- `git diff --check`
- Focused regression: Schema/normalizer test: persisted payload drops/rejects nonce; public render still receives runtime nonce.
...
- FE-31-05-08 is fixed or reclassified with fresh evidence in the report and parent task.
