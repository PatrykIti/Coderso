# TASK-392-03: NL-31-05-03 - Public Forms runtime must not be interactive without widget-level nonce
# FileName: TASK-392-03_NL_31_05_03_Public_Forms_Runtime_Must_Not_Be_Interactive_Without.md
...
Execution-ready leaf task for NL-31-05-03 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NEWSLETTER_WIDGET.md` and parent `TASK-392`.

Report found Forms runtime can render interactive publicly without a widget-level nonce.
...
**Helper/function shape:** Require nonce projection for interactive public Newsletter Forms runtime; disconnected/static states may render disabled shells only.
...
**Regression-test shape:** Public renderer/runtime security tests: no nonce => disabled/non-submitting; valid nonce => interactive path.
...
- Newsletter public submissions must use public visibility only for widget runtime, nonce + signature/HMAC, optional reCAPTCHA per botProtection policy, the existing `public_write` bucket keyed by the submission pathname via `resolvePublicWriteIdentifier`, strict reject-unknown validation, and session or API key with explicit newsletter/forms submit scope for internal mode.
...
- `git diff --check`
- Focused regression: Public renderer/runtime security tests: no nonce => disabled/non-submitting; valid nonce => interactive path.
...
- `_docs/_TASKS/TASK-392_Newsletter_Widget_31_05_UI_Audit_Remediation_Family.md` parent status/checklist when this leaf starts or closes.
...
- NL-31-05-03 is fixed or reclassified with fresh evidence in the report and parent task.
