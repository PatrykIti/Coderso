# TASK-397-01: NV-31-05-01 - Empty resolved link list must not make public widget invalid
# FileName: TASK-397-01_NV_31_05_01_Empty_Resolved_Link_List_Must_Not_Make_Public.md
...
Execution-ready leaf task for NV-31-05-01 from `_docs/PLAYWRIGHT/31-05-2026-widgets/REPORT_NAVIGATION_WIDGET.md` and parent `TASK-397`.

Empty links after public resolver cause invalid widget data instead of safe empty/placeholder state.

## Sub-Tasks

- [x] Reproduce NV-31-05-01 with the report fixture before editing and record the observed admin/public state in closure notes.
...
**Helper/function shape:** Normalize empty resolved link lists to a valid empty-navigation state or render an explicit placeholder.
...
**Regression-test shape:** Public renderer test for empty links returns HTTP 200 safe state.
...
- `git diff --check`
- Focused regression: Public renderer test for empty links returns HTTP 200 safe state.
...
2026-06-02: Closed in Navigation scope. `navigationSchema.items` now accepts
`items: []`, matching the public resolver/render contract for cleared links.
Focused regression: `navigation schema accepts resolved empty item lists`.

Validation is recorded in parent TASK-397 closure notes.

## Acceptance Criteria

- NV-31-05-01 is fixed or reclassified with fresh evidence in the report and parent task.
