...
- [x] Reproduce NV-31-05-02 with the report fixture before editing and record the observed admin/public state in closure notes.
...
2026-06-02: Closed in Navigation scope. Navigation now treats bare `#` as a
missing destination while preserving real hash anchors like `#overview`, so
unsafe resolver placeholders cannot render as clickable fake links. Focused
regression: `navigation normalizes unsafe item, child, CTA, and logo hrefs
before render`.
