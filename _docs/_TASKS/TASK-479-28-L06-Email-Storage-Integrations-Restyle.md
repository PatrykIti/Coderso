...
- **Goal:** Give Email/Storage/Integrations the prototype's soft look (grouped
  ...
  `listEmailLogs`, `getStorageSettings`/storage save, and `listIntegrations`/
...
- **Owning module/service:** `core/admin/ui/settings/EmailSettingsPage.tsx`
  ...
  `storageSettingsClient` (`getStorageSettings`, …, `StorageProviderId` =
  `local|s3|azure`), and `integrationsClient` (`listIntegrations`,
  `updateIntegration`, `requestIntegration`, `type IntegrationRecord`).
...
<div className="divide-y divide-border">
  ...
  <SettingsSection title="SMTP" description="Connect your delivery server.">
    <SmtpCard … /> {/* keep host/port/user + PASSWORD (write-only) + encryption Select bindings */}
  </SettingsSection>
...
`emailClient`; assert Sender/SMTP/Test sections, the password field is write-only/
masked, Send test calls `sendTestEmail`, and logs open `EmailLogsDrawer`. Render
