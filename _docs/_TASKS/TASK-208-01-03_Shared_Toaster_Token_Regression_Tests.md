...
- Assert forwarded wrapper props are merged safely: caller `className` is appended
  while `.toaster` remains present, and caller `style` can extend non-token
  layout properties without replacing token-owned variables from the shared
  style map.
- Add a rendered or CSS-selector contract proof for the visible floating toast
  ...
  button, action/cancel controls, loading indicator, shadow, hover/focus states,
  typed success/error/warning state colors, and neutral info variables with a
  custom Admin UI Theme token fixture. If `happy-dom` cannot reliably resolve
  CSS variables for Sonner's generated DOM, assert the wrapper host variables
  plus the scoped `.toaster [data-sonner-toast]` selectors that override Sonner
  hard-coded visible sub-part styles.
...
expect(css).toContain('.toaster [data-sonner-toast][data-styled="true"] [data-button]');
expect(css).toContain('.toaster [data-sonner-toast][data-styled="true"] [data-cancel]');
expect(css).toContain(".toaster .sonner-loading-bar");
```
...
- `tests/vitest/admin/sonner.test.tsx`
  ...
  - assert description, close-button, action button, cancel button, loading bar,
    and shadow selectors are token-backed if a scoped CSS fallback is needed,
  - assert toast focus, action/cancel focus, close-button focus, and
    close-button hover selectors are token-backed instead of keeping Sonner's
    bundled rgba/gray palette,
  ...
  - assert prop forwarding cannot drop `.toaster` or replace token-owned
    variables from the shared style map; caller `style` may extend non-token
...
4. Tests cover the visible toast shell, description, border, close-button,
   action/cancel controls, loading indicator, shadow, hover, and focus token
   ownership.
