...
|---|---|
| `_docs/PLAYWRIGHT/REPORT_NAVIGATION_WIDGET.md` | Add final fixed/deferred/routed status for every report finding. Include textual admin/frontend evidence, DOM excerpts, and validation commands. Do not commit PNG files. Live-preview and sticky frontend rows must cite the exact shared owner task IDs `TASK-317` and `TASK-318` while pending, then be updated to the landed shared-owner state once those tasks are done. |
...
- Do not mark live-preview or sticky frontend rows as closed by TASK-275
  itself unless the report names the exact shared physical owner tasks
  (`TASK-317` and `TASK-318`) and their status; if those shared owners land
  later, update the final docs to the landed shared-owner state.
...
2. TASK-275-06 reads the refreshed report rows and classifies every finding as
   `fixed`, `fixed-via-shared-owner`, `deferred`, `routed`, or
   `routed-pending-owner`.
3. Shared-contract rows are mapped to exact physical owner IDs before closure:
   live preview to `TASK-317`, sticky Section/page-shell overflow to
   `TASK-318`, and broad editor-mode IA rows to `TASK-256-01`. If a shared
   owner later lands, the final report/docs are updated from `routed` to the
   landed shared-owner state instead of staying stale.
...
- Live-preview and sticky frontend findings either cite exact shared physical
  owner task IDs (`TASK-317` and `TASK-318`) while pending or record the
  landed shared-owner resolution once those tasks are done; they are not closed
  by Navigation-only work alone.
