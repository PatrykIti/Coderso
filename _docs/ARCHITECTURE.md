...
- `core/services/assistant/blueprints/blueprintCandidateResolver.ts`, `blueprintCompositionGraph.ts`, `blueprintConflictResolver.ts`, and `blueprintActionAssembler.ts` provide the first `TASK-190` composition foundation: capability candidates, graph fragments, typed route/resource/field conflict detection, blocking gated-domain surfacing, and typed action assembly now power the local setup planner for supported multi-capability and primary-plus-gated setup requests, while single-pack setup/refinement routing plus the broader detail/media/no-duplicate cutover remain deferred to later `TASK-190` rollout leaves.
