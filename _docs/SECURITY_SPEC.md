...
- Hardening:
  ...
  - provider draft output is untrusted and operation-draft-only: provider `actions[]`, arbitrary executor inputs, malformed drafts, unknown fields, and secret-like keys cannot produce executable actions,
  ...
  - legacy CMS resource registry, provider action-array adaptation, and duplicated provider local-first guard lists are removed from active planning; policy helpers are the local source of truth before strict schema validation,
  ...
  - resource catalog includes bounded page summaries but never page data payloads,
