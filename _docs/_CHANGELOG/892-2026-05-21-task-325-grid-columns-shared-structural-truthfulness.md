...
- Visual now shows current desktop/tablet/mobile totals and explains whether each breakpoint fills one row, wraps onto additional rows, or leaves unused width.
...
- Grid Columns color controls now rely on the already-landed shared CSS-variable swatch/text contract, with widget-local regression coverage proving token text stays visible.
