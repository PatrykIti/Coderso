...
Ordinary new entries continue after the highest existing number (`1327` as of
2026-09-02); check the consumption/reservation notes under `## Index` before allocating one.
...
assistant policy coverage fix. 1229/1230 are superseded-pins to the completed TASK-511/517 families (board
rows 1280/1281; not reusable); 1231 is consumed by the completed TASK-518 admin-role migration family; TASK-519 took its pinned 1232; TASK-520 took 1233.
...
TASK-539 and TASK-542, respectively (superseded by the S3 stream pins 1318 for TASK-539 and 1319 for TASK-542; 1251 and 1254 are NOT reusable). Changelog 1257 is consumed by the completed
...
Changelog 1308 is consumed by the completed TASK-467 Admin Bundle Heavy Chunk Hardening family (row above). Changelog 1309 is consumed by the completed TASK-493 SEO: Indexing & Search-Performance Pipeline (sitemap + Google Search Console integration). Changelogs 1310-1316 are reserved (pinned) for the S1 delivery stream. Changelogs 1317, 1318, and 1319 are consumed by the completed TASK-481 (row above), TASK-539 (row above), and TASK-542 (row above) families respectively (S3 stream). Changelogs 1320 and 1321 are consumed by the completed TASK-105-04 and TASK-105-05 families (entries received at their 2026-08-19 closures); 1322 was reserved for TASK-105-06, which is superseded by the completed TASK-580 v1 widget removal family (changelog 1323); 1322 is NOT reusable. Changelog 1323 is consumed by the completed TASK-580 v1 widget removal family. Changelog 1324 is consumed by the completed TASK-105-08-07 assistant coverage leaf. Changelog 1325 is consumed by the TASK-105-09 terminal documentation closure (2026-09-01): fresh canonical whole-lane artifact (lines 99.26%, 291 uncovered lines with the per-line ledger in TASK-105-08-12), the terminal TASK-105-08-05 menus/dashboard/kits family plus its S01 prerequisite, the task-105-l05 and task-105-l08 runtime-smoke acceptance runs, the dep-cache split collateral repairs, and the TASK-105-08-13 draft disposition. Changelog 1326 is consumed by the completed TASK-105-08-11 oversized-test-splits-and-runner-docs family (split of four oversized Vitest families, post-TASK-580 `tests/RUNNER_OWNERSHIP.md` rewrite, bun-lane manifest re-verification, and the four-suite server Bun ownership freeze + schema-validator transfer handoff). Changelog 1327 is consumed by the terminal TASK-105 program closure package (2026-09-02): the 27 verify-then-close TASK-105-08 leaf/parent flips on the TASK-105-08-12 rebaseline standard, the L03 and L02 dead-path source repairs with their authored children 08-08-L03-L01 and 08-08-L02-L01 plus the L02 full-lane posts V8 gate, the TASK-105-11 family closure (11-03-05 four-suite freeze, 11-03-08 schema-validator migration, 11-04 QA/closure), and the TASK-105 program itself closing on the 99.26% lines rebaseline (291 dispositioned lines).

| No. | Date | Title | Type |
|-----|------|-------|------|
| 1327 | 2026-09-02 | TASK-105 Program Closure — 08-family terminal flips (27 verify-then-close leaves/parents on the TASK-105-08-12 rebaseline standard: lines 99.26 (39427/39718), 291 dispositioned lines, canonical 1186 files / 10444 tests), L03 pages/posts dead-path repair (7 files net −43, three new happy-dom suites 31 tests, two owner-boundary audits) with its child L03-L01 (`DetailTemplateInspector` typed list-value seam, `ListItemsControl` narrowed, `:62` fallback + cast deleted), L02 residual suites (+3 tests over 3 committed suites) and the full-lane posts V8 gate (1189 files / 10481 tests / 0 failures; 33/33 include paths at 100% lines; gate total 3810/3810), L02-L01 post-editor-state dead-path repair (23 dead lines deleted plus the non-silent hydrate apply tail — 13 further lines — whose apparent coverage was V8 function-range bleed-through; no `"hydrate"` producer in `core`; production +134/−167), the TASK-105-11 family closure (11-03-05 four-suite Bun freeze receipt, 11-03-08 schema-validator Vitest migration 4 files / 14 tests / 0 failures, commits 5b5ed371 + ae1ca47b, 11-04 QA/closure with changelog follow-through), and the TASK-105 program closing honestly on the 08-12 rebaseline standard (its "100% Coverage" title is historical). `TASK-105-08-05-L03-L02` remains `⏭️ Superseded`; +37 tests over the canonical run (31 L03 suites + 3 L02 extensions + 3 inspector additions), +3 files. | Testing/Docs/Task Board |
| 1326 | 2026-08-26 | TASK-105-08-11 Oversized Test Splits and Runner Docs — Closure (split of four oversized Vitest families (menu-design-editor 2711, users-roles-page-wave 1132, bookingPageFixtures 1123, blueprint-action-assembler 1050) into ≤1000-line independently runnable parts handed downstream to 08-05/08-07/08-08/08-09; post-TASK-580 `tests/RUNNER_OWNERSHIP.md` rewrite; bun-lane manifest re-verification; four-suite server Bun ownership freeze + schema-validator transfer to 11-03-08. All split parts green, line gate held, `git diff --check` clean. | Testing/Docs/Task Board |
| 1325 | 2026-09-01 | TASK-105-09 QA, Docs, Changelog, and Closure — TASK-105-08 terminal documentation sync: fresh canonical whole-lane artifact (1186 files / 10444 tests, stmts 96.23, branch 87.05, funcs 98.86, lines 99.26, 291 uncovered lines with the per-line ledger in TASK-105-08-12); terminal menus/dashboard/kits family TASK-105-08-05 with its ordered source repairs L01-L01 → L01-L02 → L01 → L02-L01 → L02 → S01 four-path browser parity → L03-L01 card-parity diagnostic clearance → L03 → L04 (public `MenuItemDrawer`/`MenuItemInspector`, dashboard permission-catalog, and solution-kit coverage); `task-105-l05` official-r2 and `task-105-l08` r44 five-flow runtime-smoke acceptance with zero console errors; dep-cache split product fix plus task-488/490 `ADMIN_DEP_URL` collateral repair; TASK-105-08-13 17/17 assistant-draft disposition. L03-L02 preserved `⏭️ Superseded`. | Testing/Docs/Task Board |
| 1319 | 2026-08-21 | TASK-542 Menu Determinism, Responsive Cascade, and Runtime Parity — strict deterministic `MenuDocumentV2` (stable ids/topology, fail-closed legacy reads, module split with stable facade), complete responsive neutralizers (per-device × level matrix, reset-every-device semantics, scrolled-header + brand parity), shared `publicNavigationProjection` owner (hidden subtrees dropped, no empty dropdowns, dead-parent linkless groups), front render on the projection + active identity + responsive scroll gate, and draft-safe editor revalidation (force-revalidate, never clobber dirty drafts, no redundant force loop, retryable failures). Seven distinct real-flow smokes `wf542smoke` (legacy stable reads, desktop flyout, L2+ OFF no-inherit, padding-axis independence, icon colors per device, design projection parity, geometry + dirty/no-clobber) PASS with 0 product console errors; 485 targeted Vitest green (12 menu service/site suites 368 + 6 admin menu UI suites 117), core lint/types + root tsc green, admin build + boundary (914 files) + bundle green, `gates:coderso` 5/5; Bun lane green except the documented pre-existing slow-remote-PG `afterEach` hook-timeout flakes. 4 children + 6 leaves terminal. | Menus/Responsive CSS/Public Render/Admin UI/Testing/Docs/Task Board |
| 1318 | 2026-08-20 | TASK-539 Page V2 Post-Audit Remediation II — deep present-key layer merge + strict gallery/effect normalization (stable `pageDocumentV2` facade frozen at 78 types + 133 runtime exports), unitful grid + split background-layer sanitizer, gallery-control gating + z-clamp + shared grid-placement contract + narrow responsive canvas, independent reveal/hover/tilt/magnetic transform channels, renderer behavior/geometry corrections, per-device responsive CSS parity, and the per-root idempotent effects runtime (parser-order rescan + per-event passive policy: keydown non-passive). Nine-flow real-input smoke `wf539smoke` PASS, 0 console errors; post-audit 0 HIGH / 1 MEDIUM (start-gate) / 6 LOW (workflow-leaf inventory deferred to TASK-9999-03); 1131 targeted Vitest green, `precommit:check` EXIT=0, admin/site build + boundary + bundle, DB preflight, routes/parity/pages/site-shell runtime suites. 8 children + 19 leaves terminal. | Pages/Page Editor/Public Render/Responsive CSS/Runtime/Security/Testing/Docs/Task Board |
| 1317 | 2026-08-19 | TASK-481 Page Editor Canvas Brand Token WYSIWYG — content-scope canvas split (chrome vs `data-page-editor-content`), admin brand re-assertion on chrome, brand-only canvas emission (`toPageCanvasBrandColorCssVariableMap`), PageEditor facade split (7 modules <= 1000), live site palette in the inline text-color toolbar, and the cache-bus live-repaint fix (`getSettingsCached({ force: true })` revalidation on mount + `settingsRedacted` event). 65/65 Vitest matrix green, post-audit + smoke-audit 0 HIGH / 0 MEDIUM, 7-scenario real-input smoke `wf481smoke2` PASS incl. the critical live-repaint scenario; HomePage returned to draft after smoke; shared DB settings restored. 4 subtasks + 9 leaves terminal. | Pages/Page Editor/Design Tokens/Admin UI/Testing/Docs/Task Board |
...
| 1293 | 2026-08-18 | TASK-571 Forms Submissions Export Bounded Reads — bounded keyset-cursor export job (`(form_id, created_at DESC, id DESC)`) + advisory lock + scheduler dispatch/retention + migration 0075 index, PII omission + CSV formula guard preserved, token-guarded artifact read; 12 export-job + 21 route tests green, EXPLAIN evidence; smoke: published form → nonce-gated submission → CSV+JSON downloads. | Forms/Admin API/Security/Testing/Docs/Task Board |
| 1292 | 2026-08-18 | TASK-570 Entries Revision Restore Atomicity And Bounded History — one fenced restore tx (fence + `FOR UPDATE` + narrow revision read + snapshot locked data), null-actor restore without new revision row, keyset-cursor revision history (no `data` columns) + narrow detail read, unique `(entryId, version)` migration 0076, 404/409 route mapping; plus TASK-570-01 split of the 2084-line entryService test into 5 suites + support module (29/29 names preserved). Content lane A/B-isolated 141 green. | Engine/Entries/Admin UI/Testing/Docs/Task Board |
| 1291 | 2026-08-18 | TASK-569 Custom Screens Optimistic Concurrency Revision And Conflict — monotonic `revision` + `expectedRevision` conditional PATCH → `custom_screen_conflict` 409, draft-preserving editor on real 409, migration 0073; plus TASK-569-01 split of the 6771-line actionExecutorService into 14 cohesive modules (pure relocation, all < 1000 lines). Assistant lane 103 tests green. | Custom Screens/Admin UI/DB Concurrency/Testing/Docs/Task Board |
| 1286 | 2026-08-18 | TASK-564 Backup Users Section Bounded Memory Restore — staging-table batch upsert (5k-row NDJSON batches into `backup_users_staging`, migration 0078), set-based natural-key/FK/user_roles guards + STAGING→FINAL upsert in one tx, unchanged admin-lockout guard, idempotent run-scoped cleanup, OOM-guard tests; 34 backup + 101 migration-chain tests green. | Backups/Data/Security/Streaming/Testing/Docs/Task Board |
...
| 1283 | 2026-08-18 | TASK-561 Backup Import Native CMS Writer Fence — `importBackupFromUpload` acquires the native writer fence first inside the restore tx, `native_cms_writer_fence_busy` → 409 via `mapBackupError`; DB race regression + runtime smoke (no-maintenance 409, full 22-table import). | Backups/Concurrency/API/Testing/Docs/Task Board |
| 1284 | 2026-08-18 | TASK-562 Backup V2 Import Archive Exact Set — strict per-section manifest schema + member count/byte comparison, any mismatch → `backup_manifest_invalid` 422 before restore work; negative matrix extended; smoke: omitted member rejected, valid archive imports. | Backups/Data/Security/Testing/Docs/Task Board |
| 1285 | 2026-08-18 | TASK-563 Backup Import Cache Invalidation And Post Commit Failure Audit — `clearSiteCache()` after commit, non-`MediaRestoreFailure` rethrow (422 surfaces), redacted `backup.mediaRestoreFailure` audit row + counts; smoke: front 404 after replace (no stale cache). | Backups/Cache/Audit/Testing/Docs/Task Board |
| 1287 | 2026-08-18 | TASK-565 Split mediaDeliveryAccess test (1051 → <1000) — shared `mediaDeliveryTestSupport` harness, split by grant matrix/auth/redirect into independently runnable suites; lane command typo fixed. | Testing/Docs/Task Board |
| 1288 | 2026-08-18 | TASK-566 Split fullSiteManagedOwnershipDb test (1001 → <1000) — named `fullSiteManagedOwnershipSupport` helper module, split ownership-transfer vs setting-compensation; cleanup contract preserved. | Testing/Docs/Task Board |
| 1289 | 2026-08-18 | TASK-567 Outbound Webhook Egress Policy SSRF Hardening — `outboundHttpPolicy` (validate + fetchWithEgressPolicy DNS re-check + https-only Sentry), form-action + login-alert routed through it, config-time Slack/Zapier URL validation, fail-closed resolver. | Network/Security/Integrations/Testing/Docs/Task Board |
| 1290 | 2026-08-18 | TASK-568 Public Analytics Head Log Hygiene — `publicHeadTags` catch logs only fixed `analytics_head_resolution_failed` code (no error internals), fail-closed null; sentinel-secret test; smoke: head clean no-GA / gtag injected / fail-closed removal. | Public Runtime/Security/Logging/Testing/Docs/Task Board |
| 1294 | 2026-08-18 | TASK-572 Entry List Cache Visibility Fence — auth resolved above list cache read, body cache gated on `!isAuthenticated && !routeIsGatedEntry`, restricted transitions fail-closed (no stale cached list); runtime ordering regressions green. | Public Runtime/Security/Cache/Testing/Docs/Task Board |
| 1295 | 2026-08-18 | TASK-573 Entry Gated Probe Cheap Bounded Read — `getEntryVisibilityById/Slug` minimal `(id, visibility)` projection, no joins/SEO/taxonomy; `entryRouteIsGated` wired to narrow reads, slug scoped by typeId; query-shape test. | Content/Performance/Testing/Docs/Task Board |
| 1296 | 2026-08-18 | TASK-574 Popup Runtime Start Stop Generation Guard — generation token + stale check around the arming loop; repeated start/stop cycles cannot duplicate dialogs; session-once reset returns exactly 1 dialog; smoke 4 scenarios. | Popups/Runtime/Testing/Docs/Task Board |
| 1297 | 2026-08-18 | TASK-575 Commerce Rename Variant Attribute Collision Guard — `validateRenameVariantAttributeKey` rejects existing-key rename (trim-aware) before mutation, inline alert light+dark, unique rename preserves values; smoke 4 scenarios. | Commerce/Admin UI/Testing/Docs/Task Board |
| 1298 | 2026-08-18 | TASK-576 Workflow Surface Repair And Gate Re Green — legacy scripts archived, syntax-invalid repaired/archived, forbidden directives stripped, tests split by responsibility, taskGraphIntegrity memoization/timeout fix; workflow gates re-green. | Workflows/Toolchain/Testing/Docs/Task Board |
| 1299 | 2026-08-18 | TASK-577 Smoke Evidence Canonical Boundary — `EVIDENCE_ROOT = _docs/_workflows/_smoke/evidence/<task>` enforced for task-547/540/554 adapters, inventory guard rejects un-reconciled additions; doc note recorded. | Smoke/Toolchain/Testing/Docs/Task Board |
| 1300 | 2026-08-18 | TASK-578 Bun Lane Degraded Path Resilience — classifier omits `weightMs` when timings unknown (never `0`), partitioner semantics unchanged (pinned `0` preserved); degraded-path tests. | Toolchain/Testing/Docs/Task Board |
| 1301 | 2026-08-18 | TASK-579 Smoke Adapter Modularization Browser Actions — task-517 adapter (1519 lines) split into fixtures/public-actions/admin-actions modules < 1000 lines via thin index; suite 6/6 PASS. | Smoke/Toolchain/Testing/Docs/Task Board |
| 1302 | 2026-08-18 | TASK-9999-01-L01 Decouple Actor and Media UUID Domain Naming — neutral `isScreenUuid` + delegating media predicate; actor/`updatedBy` route through neutral grammar, byte-identical acceptance. | Custom Screens/Domain Naming/Testing |
| 1303 | 2026-08-18 | TASK-9999-02-L01 Sync Closed-Family Subtask Tables — parent display tables for 486/487/488/490/491/492/511/518/540/545/547 reconciled to terminal; README Done rows moved to Done section. | Docs/Task Board |
| 1304 | 2026-08-18 | TASK-9999-02-L02 Mark TASK-540 Closed Workflow Fields Historical — "Current" workflow fields relabeled historical, closure wording neutralized, smoke ownership pointed to TASK-552/560. | Docs/Task Board |
| 1305 | 2026-08-18 | TASK-9999-02-L03 Relabel Stale TASK-559 Acceptance Benchmark — 9.98-min/397-file acceptance marked historical; current 440-row manifest recorded; target not re-claimed. | Docs/Benchmark |
| 1306 | 2026-08-18 | TASK-9999-02-L04 Reconcile TASK-547 Closure Notes — checklist flipped to `[x]`, Changelog 1260 note corrected to Final/indexed. | Docs/Task Board |
| 1307 | 2026-08-18 | TASK-478 Page Editor Inline Link And Toolbar Placement UX — closed as already implemented (committed `205c66a5`, tests tagged TASK-478-02/03): token-driven link underline/color on front+canvas, explicit Remove link + seeded URL edit + click-to-select, dockable Top/Right/Left mark toolbar; 235/235 Vitest green. | Pages/Page Editor/Testing/Docs/Task Board |
| 1309 | 2026-08-19 | TASK-493 SEO: Indexing & Search-Performance Pipeline — full sitemap generation/submission/status tracking + Google Search Console integration (indexed-pages/impressions/queries) with 4 new tables, admin SEO Manager real-data rewire, and the shared task-493 smoke suite | SEO/Sitemap/GSC/Admin UI/Testing/Docs/Task Board |
| 1308 | 2026-08-18 | TASK-467 Admin Bundle Heavy Chunk Hardening — lazy-split the admin widget-editor registry (typed `WidgetEditorBundle` lazy loader map, shared `WidgetEditorOutlet` with Suspense fallback + bounded error/retry, lightweight custom-screens cache/client splits), 500 kB dynamic raw chunk budget guard + registry split evidence, documented lazy node-import exemption; new reusable `task-467` runtime-smoke adapter (7 scenarios: lazy fallback, computed-style color control, advanced mount, dark mode, mobile no-overflow, chunk-failure retry, advanced round trip) with per-run worker admin + full absence cleanup. 27/27 unit tests + runtime smoke PASS. | Widgets/Admin UI/Bundle Performance/Smoke/Testing/Docs/Task Board |
| 1320 | 2026-08-19 | TASK-105-04 Themes, Booking, Listings, and Forms Wave — Coverage Closure — test-only Vitest wave closing themes 99.59/92.66, booking 99.82/95.00, listings 92.15/75.85, forms 97.96/81.33 (lines/branches, artifact truncation convention), every rebaseline regression recovered (FormListPage 73.38→99.28, FormDesignPanel 47.06→100.00, ThemeEditorPage →100.00, BookingPage →100.00); oversized splits ≤1000 lines; canonical totals 81.54 stmts / 73.30 branch / 81.18 funcs / 84.59 lines. | Testing/Docs/Task Board |
| 1321 | 2026-08-19 | TASK-105-05 Entries, Pages, and Posts Editor Wave — Coverage Closure — test-only Vitest wave closing entries 93.76/83.56, pages 94.50/82.29, posts 96.72/84.81; PageEditor 84.47→91.06 lines, Canvas →99.27, templates 28-55%→94-100%, posts 8/8 targets grew incl. PostRichTextAdapter 613→644 and PostBlockEditorShell branch regression fix; 8 oversized test files split ≤1000 lines (incl. post-audit blockSettings-wave); post-implementation drift audits 0 HIGH. | Testing/Docs/Task Board |
| 1323 | 2026-08-20 | TASK-580 Remove Old Page Widget System V1 — deleted the v1 widget kernel (`core/widgets/**`), v1 public page runtime (`pageRuntime.tsx`, `renderPublicPageHtml`, `renderPublicPageRuntimeHtml`), per-type hydration, v1 admin widget surface (`admin/ui/widgets/**`, `admin/ui/pages/builder/**`), Widget Library, widget templates, and `modulePackMatrix`; relocated V2-shared contracts to `core/services/renderContracts/*`; migrated stored detail pages to Page V2 (migration 0080) with read-only `legacy-widget` placeholders; retired the `task-467` smoke adapter; rewired tests, added deletion guards, and reduced `_docs/WIDGETS*` + `_docs/_WIDGETS/*` to tombstones. Full Vitest + Bun lanes and admin boundary/bundle guards green. Changelog 1323. 4 children + 7 leaves terminal. | Widgets/Page V2/Public Runtime/Admin UI/DB/Smoke/Testing/Docs/Task Board |
| 1324 | 2026-08-22 | TASK-105-08-07 Assistant Services and UI — Coverage Closure — test-only Vitest wave closing the 34-file assistant scope (`core/services/assistant/**` + `core/admin/ui/assistant/**`); actionPlannerService 453/508→499/508, modelCapabilities 25/26→26/26, providers/index 24/34→34/34, all 9 UI files 100% or residual lines verified unreachable (dead guards: non-rejecting submit catch, throwing intake-session factory, type-closed surface union, defensive sanitizer/no-op checkbox branch); evidence per line in the leaf task file; assistant+UI+admin+pages+ui-integration lanes green (924 files / 7419 tests), tsc + ESLint + diff-check clean. | Testing/Docs/Task Board |
