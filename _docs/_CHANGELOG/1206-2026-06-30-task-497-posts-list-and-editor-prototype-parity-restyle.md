...
editor** — presentation only, no re-architecture and no feature change. Every post
behavior (block model, autosave, revisions, runtime preview, status, bulk actions,
create drawer, keyboard shortcuts, every `data-post-editor-*` hook +
`aria-pressed`/`aria-controls`, and the inspector **Block** tab) is preserved exactly.
The editor keeps its **own** three-pane shell (`PostEditorLayout`/`PostEditorRegions`)
— it does **not** adopt the Pages `shared/CanvasEditor.tsx` — and the list keeps
`PageFilters` + `ListPaginationFooter` and restyles `PostsTable` **in place** (no swap
to `shared/DataTable.tsx`). The classic editor (`PostClassicEditorShell`) is out of
...
### Post editor restyle (TASK-497-02)
- New shared presentational primitive `core/admin/ui/shared/EditorRail.tsx`
  ...
  `bg-primary-soft` active token.
- Collapsed the two-row editor header into a **single muted chrome strip**
  (`bg-muted/40`): autosave pill → `Badge variant="outline"` (dynamic Saving… /
  Unsaved changes / Saved at HH:MM / Synced text kept); wired undo/redo (disabled when
  `canUndo`/`canRedo` are false); optional device/viewport toggle; Preview + Save draft
  + Publish (icon `Send` → `Rocket`, label flips Update when published); the
  Add-block / Outline / Details / Focus / Revisions / Settings toggles demoted to
  `icon` ghost buttons keeping **every** `data-post-editor-*` / `aria-pressed` /
  `aria-expanded` / `aria-controls` / `aria-keyshortcuts` attribute. The
  `data-post-editor-header-row="secondary"` row is removed.
- Left rail region surface → `bg-muted/20`; the block inserter's "Most used"/category
  sections are wrapped in `EditorRailGroup` and each option row carries the rail active
  token `bg-primary-soft` via `className` on its kept `<Button role="option">` (the
  listbox roving-keyboard a11y is preserved — no literal `EditorRailItem` in the
  listbox). `PostListViewSidebar` root → `bg-transparent` so the region shows through.
...
- New `tests/vitest/ui/posts-editor-chrome-wave.test.tsx` renders the real
  `PostBlockEditorShell` and pins the editor chrome restyle (single strip, dynamic
  autosave badge, wired/disabled undo-redo, device toggle, `EditorRailGroup`
  consumption + active rail token, flat "Post settings" inspector with single SEO
  sub-card + Block tab, guarded dotted canvas).
- Extended `posts-list.test.tsx` + `posts-table-wave.test.tsx` with the list restyle
  ...
  lookups left at `=== "New"`), and the five `tests/vitest/ui-integration/post-*`
  suites (document-inspector, shell-restyle, header-workflow, layout-shell,
  writing-canvas-flow, plus the canvas-shared `text-5xl` → `text-3xl` re-baseline).
...
- Full `bun run test:vitest` — 746 files / 4488 tests PASS (exactly **+1 new file**,
  `tests/vitest/ui/posts-editor-chrome-wave.test.tsx`; the re-baselined presentation-lock
  strings moved, not weakened).
...
- The `bun test` full-suite file count moved from the 745 baseline to **746** (+1 new file only).
