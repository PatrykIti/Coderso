...
- capability registry/composer foundation:
  ...
  - catalog capabilities can already describe `detail-page` intent in metadata, and the current `TASK-190` slices now cover persisted detail-page documents, published/runtime detail rendering, shared preview handling, and the first executable `detail-page.upsert` assistant action through the content-domain owner seam; generic detail-page resource packaging, internal admin CRUD, and workspace flows remain deferred to later leaves
