...
export function validManifest(
  ...
  session: string,
  reportSha: string,
  revision: PublicWorkingTreeRevision
  ...
  const scenarios = [
    ...
    variants: [
      {
        id: `${id}-v`,
        surface: id.startsWith("public") ? "public" : "admin",
        theme: id.includes("dark") ? "dark" : "light",
  ...
  return {
    ...
    profile: PROFILE,
    session,
    report: { path: "report.json", sha256: reportSha },
    revision,
...
export async function buildEvidence(root: string): Promise<string> {
  ...
  const reportBytes = `${JSON.stringify({
    ...
    serverUp: true,
    scenarios,
    screenshots: scenarios.map((s) => s.screenshots[0]),
  })}\n`;
...
export function resumeOptions(
  root: string,
  pause: OwnerActionRequiredPayload
): SmokeEvidenceResumeOptions {
  return {
    repoRoot: root,
    expectedTask: TASK,
...
export function writerCheckpoint(gitHead: string): VerifiedTask545Checkpoint {
  return {
    ...
    closureContract: {
      ...
      taskIndex: TASK_INDEX_REL,
      changelogIndex: CHANGELOG_INDEX_REL,
      changelogNumber: CHANGELOG_NUMBER,
      changelogSlug: CHANGELOG_SLUG,
    },
    phase1: { state: "owner_review_required", generatedAt: "2026-08-14T00:00:00.000Z" },
  } as unknown as VerifiedTask545Checkpoint;
}

export function closureIdentity(
  durableState: "none" | "file-only" | "both" = "none"
): Task545ClosureIdentity {
  return {
    taskId: TASK,
    suiteId: SUITE,
    ...
    pinnedChangelogPath: PINNED_CHANGELOG_REL,
    durableState,
  };
}
...
export function taskIndexAfterState(): string {
  return FROZEN_TASK_INDEX.replace(
    "| TASK-545 | Smoke Evidence | High | Medium | ⏳ To Do |",
    "| TASK-545 | Smoke Evidence | High | Medium | ✅ Done |"
