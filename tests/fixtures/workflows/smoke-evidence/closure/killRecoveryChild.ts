...
function requireArgs(argv: string[]): { killPoint: number } {
  ...
  if (!Number.isInteger(killPoint) || killPoint < 0 || killPoint > 6) {
    process.stderr.write(JSON.stringify({ pass: false, code: "fixture_usage", detail: "kill-point 0..6" }));
    process.exit(2);
  }
  return { killPoint };
}
...
async function main(): Promise<void> {
  ...
  if (killPoint >= 3) {
    let handle;
    try {
      handle = await open(changelogPath, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL, 0o600);
