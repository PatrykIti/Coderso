...
const pageEditorState = vi.hoisted(() => {
  const state = {
    ...
    listPageRevisions: vi.fn(async (_id: string) => state.revisions),
    ...
    autosavePage: vi.fn(async (_id: string, _payload: Record<string, unknown>) => ({ ok: true })),
    ...
    discardPageRevision: vi.fn(async (_id: string, _revisionId: string) => ({ ok: true })),
