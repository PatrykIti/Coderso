...
test("renders an invalid revision timestamp as the raw value", async () => {
  state.revisions = [createRevision({ id: "rev-bad", createdAt: "not-a-date" })];
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "History");
    await flush();
    expect(textOf(view.container)).toContain("not-a-date");
  } finally {
    view.cleanup();
  }
});

test("falls back to the cached content types when the force refresh fails", async () => {
  state.failNextListContentTypes = true;
  const view = mount();
  try {
    await flush();
    await flush();
    expect(state.getCachedContentTypes).toHaveBeenCalled();
    expect(state.listContentTypesCached).toHaveBeenCalled();
  } finally {
    view.cleanup();
  }
});

test("applies a remote refresh from a cache event without unsaved changes", async () => {
  const view = mount();
  try {
    await flush();
    state.remoteRecord = createRecord({
      name: "Renamed remotely",
      currentDocument: createDocument({ name: "Renamed remotely" }),
    });
    React.act(() => {
      state.triggerCacheEvent(cacheKeys.detailPageDetail(state.remoteRecord!.id));
    });
    await flush();
    expect(textOf(view.container)).toContain("Renamed remotely");
  } finally {
    view.cleanup();
  }
});

test("shows a pending-update alert for unsaved changes and refreshes on demand", async () => {
  const view = mount();
  try {
    await flush();
    changeInputByLabel(view.container, "Name", "Edited locally");
    state.remoteRecord = createRecord({
      name: "Remote winner",
      currentDocument: createDocument({ name: "Remote winner" }),
    });
    React.act(() => {
      state.triggerCacheEvent(cacheKeys.detailPageDetail(state.remoteRecord!.id));
    });
    await flush();
    expect(textOf(view.container)).toContain("Template changed");
    expect(textOf(view.container)).toContain("New changes are available.");
    clickButton(view.container, "Refresh");
    await flush();
    expect(textOf(view.container)).toContain("Remote winner");
    expect(textOf(view.container)).not.toContain("New changes are available.");
  } finally {
    view.cleanup();
  }
});

test("shows a sample entries error message", async () => {
  state.failNextListEntries = true;
  const view = mount();
  try {
    await flush();
    await flush();
    expect(textOf(view.container)).toContain("Failed to load sample entries.");
  } finally {
    view.cleanup();
  }
});

test("commits a section control change through the inspector", async () => {
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "History");
    ...
    await flush();
    expect(textOf(view.container)).toContain("Block");
    expect(textOf(view.container)).toContain("Section");
  } finally {
    view.cleanup();
  }
});

test("commits a block control change through the inspector", async () => {
  const view = mount();
  try {
    await flush();
    const blockRow = view.container.querySelector<HTMLElement>(
      '[data-detail-template-block="block-heading"]'
    );
    React.act(() => {
      blockRow?.click();
    });
    await flush();
    const primaryInput = Array.from(view.container.querySelectorAll("input")).find(
      (input) => input.value === "Product heading"
    );
    React.act(() => {
      if (!primaryInput) throw new Error("Missing heading text input");
    ...
    await flush();
    const editedInput = Array.from(view.container.querySelectorAll("input")).find(
      (input) => input.value === "Edited heading"
    );
    expect(editedInput).toBeTruthy();
  } finally {
    view.cleanup();
  }
});

test("save is a no-op when the record is missing", async () => {
  state.remoteRecord = null;
  state.cachedRecord = null;
  const view = mount();
  try {
    await flush();
    expect(textOf(view.container)).toContain("Failed to load detail template.");
    clickButton(view.container, "Save draft");
    await flush();
    expect(state.updateDetailPage).not.toHaveBeenCalled();
  } finally {
    view.cleanup();
  }
});

test("shows the revisions load error", async () => {
  state.failNextListRevisions = true;
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "History");
    await flush();
    expect(textOf(view.container)).toContain("Failed to load detail template history.");
  } finally {
    view.cleanup();
  }
});

test("shows the save draft error", async () => {
  ...
  try {
    ...
    clickButton(view.container, "Save draft");
    await flush();
    expect(textOf(view.container)).toContain("Failed to save detail template.");
  } finally {
    view.cleanup();
  }
});

test("shows the autosave error", async () => {
  state.failNextAutosave = true;
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "Autosave");
    await flush();
    expect(textOf(view.container)).toContain("Failed to autosave detail template.");
  } finally {
    view.cleanup();
  }
});

test("publishes dirty templates by saving the draft first", async () => {
  const view = mount();
  try {
    await flush();
    changeInputByLabel(view.container, "Name", "About to publish");
    clickButton(view.container, "Publish");
    await flush();
    expect(state.updateDetailPage).toHaveBeenCalled();
    expect(state.publishDetailPage).toHaveBeenCalled();
    expect(textOf(view.container)).toContain("Published");
  } finally {
    view.cleanup();
  }
});

test("shows the publish error", async () => {
  state.failNextPublish = true;
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "Publish");
    await flush();
    expect(textOf(view.container)).toContain("Failed to publish");
  } finally {
    view.cleanup();
  }
});

test("shows the unpublish error", async () => {
  state.remoteRecord = createRecord({ status: "published" });
  state.cachedRecord = state.remoteRecord;
  state.failNextUnpublish = true;
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "Unpublish");
    await flush();
    expect(textOf(view.container)).toContain("Failed to unpublish");
  } finally {
    view.cleanup();
  }
});

test("previews dirty templates by saving the draft first", async () => {
  const view = mount();
  try {
    await flush();
    changeInputByLabel(view.container, "Name", "Preview me");
    clickButton(view.container, "Preview");
    await flush();
    expect(state.updateDetailPage).toHaveBeenCalled();
    expect(textOf(view.container)).toContain("preview-url:/preview");
  } finally {
    view.cleanup();
  }
});

test("shows the restore revision error", async () => {
  state.failNextRestore = true;
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "History");
    ...
    await flush();
    expect(textOf(view.container)).toContain("Failed to restore detail template revision.");
  } finally {
    view.cleanup();
  }
});

test("shows the discard revision error", async () => {
  state.failNextDiscard = true;
  const view = mount();
  try {
    await flush();
    clickButton(view.container, "History");
    ...
    await flush();
    expect(textOf(view.container)).toContain("Failed to discard detail template autosave.");
  } finally {
    view.cleanup();
  }
});
