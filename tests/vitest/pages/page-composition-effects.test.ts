...
describe("PAGE_COMPOSITION_EFFECTS_CSS", () => {
  ...
  test("TASK-539-04: hover declarations write ONLY hover variables", () => {
    expect(css).toContain("[data-hover=\"lift\"]:hover,[data-hover=\"lift-glow\"]:hover{--cx-hover-y:-6px}");
  ...
  test("TASK-539-04: marquee is exactly viewport > rail > segment; the RAIL animates", () => {
    ...
    expect(css).toContain("[data-marquee][data-marquee-dir=\"right\"] .cx-marquee-rail{animation-direction:reverse}");
...
describe("resolveBlockCompositionAttrs — present-only", () => {
  ...
  test("TASK-539-04: NO host for non-transform effects or no-effect styles", () => {
    ...
    expect(resolveBlockCompositionAttrs({ decoration: { motion: "radiate" } }).dataAttrs).not.toHaveProperty(
      host
    );
    expect(resolveBlockCompositionAttrs({ hoverEffect: "glow-reveal" }).dataAttrs).not.toHaveProperty(
      host
    );
    // surfaces / layers / marquee alone do NOT stamp a transform host
    expect(resolveBlockCompositionAttrs({ surfacePreset: "glass" }).dataAttrs).not.toHaveProperty(host);
    expect(
      resolveBlockCompositionAttrs({ layer: { x: 10, y: 20 } }).dataAttrs
