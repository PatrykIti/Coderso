...
describe("pageRendererReplicaIdentity (TASK-539-05-L01 direct owner)", () => {
  ...
  test("PAGE_MARQUEE_REPLICA_SAFE_BY_BLOCK_TYPE is exhaustive and fail-closed", () => {
    // Every page block type is triaged; exactly the six live/global-binding
    // types are unsafe (a new type fails `satisfies Record<PageBlockType, ...>`).
    // TASK-580 added legacy-widget as migration-only; it never renders from
    // the page runtime, so it is triaged unsafe like the other live types.
    expect(Object.keys(PAGE_MARQUEE_REPLICA_SAFE_BY_BLOCK_TYPE)).toHaveLength(25);
    const unsafe = Object.entries(PAGE_MARQUEE_REPLICA_SAFE_BY_BLOCK_TYPE)
      .filter(([, safe]) => safe === false)
      .map(([type]) => type);
    expect(unsafe).toEqual(["video", "form", "collection", "filters", "embed", "legacy-widget"]);
    expect(
