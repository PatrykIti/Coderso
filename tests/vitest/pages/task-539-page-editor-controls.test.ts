...
import { GalleryCategoryTokensControl, GalleryItemsControl } from "../../../core/admin/ui/pages/editorControls";
...
import { GALLERY_CATEGORY_PATTERN, PAGE_LAYER_Z_CLAMP } from "../../../core/services/pages/pageDocumentV2Types";
...
const mountTokens = (props: {
  ...
  return {
    container,
    rerender: (next: { initialValue?: readonly string[]; onChange?: (value: string[]) => void }) => {
      React.act(() => {
...
describe("TASK-539 dedicated gallery kinds, base-only gates, and z-clamp owner", () => {
  ...
  test("base-only showWhen gates resolve on the base target, never the effective device target", () => {
    ...
    expect(
      isPageEditorControlVisible(filterCategories, { baseTarget: baseTrue, effectiveTarget: effectiveFalse })
    ).toBe(true);
    const baseFalse = galleryBlock(false);
    const effectiveTrue = galleryBlock(true);
    expect(
      isPageEditorControlVisible(filterCategories, { baseTarget: baseFalse, effectiveTarget: effectiveTrue })
    ).toBe(false);

    const baseGradient = dividerBlock(true);
    const effectiveNoGradient = dividerBlock(false);
    expect(
      isPageEditorControlVisible(dividerWidth, { baseTarget: baseGradient, effectiveTarget: effectiveNoGradient })
    ).toBe(true);
    const baseNoGradient = dividerBlock(false);
    const effectiveGradient = dividerBlock(true);
    expect(
      isPageEditorControlVisible(dividerWidth, { baseTarget: baseNoGradient, effectiveTarget: effectiveGradient })
    ).toBe(false);

    const baseParallax = sectionBlock("parallax");
    const effectivePlain = sectionBlock("none");
    expect(
      isPageEditorControlVisible(parallax, { baseTarget: baseParallax, effectiveTarget: effectivePlain })
    ).toBe(true);
    const basePlain = sectionBlock("none");
    const effectiveParallax = sectionBlock("parallax");
    expect(
      isPageEditorControlVisible(parallax, { baseTarget: basePlain, effectiveTarget: effectiveParallax })
    ).toBe(false);
  });
  ...
  test("PAGE_LAYER_Z_CLAMP owner identity: literal, editor control reference, and write-normalizer clamp", () => {
    ...
    const normalize = (block: unknown) =>
      normalizePageDocumentV2ForWrite({
        ...
        sections: [
          createPageSectionV2("content", { id: "sec-z", blocks: [block as PageBlockV2] }),
        ],
      });
    expect(normalize(rawOver).sections[0]!.blocks[0]!.style?.layer?.z).toBe(PAGE_LAYER_Z_CLAMP.max);
    expect(normalize(rawUnder).sections[0]!.blocks[0]!.style?.layer?.z).toBe(PAGE_LAYER_Z_CLAMP.min);
...
describe("TASK-539 gallery row + media control behavior (real GalleryItemsControl)", () => {
  test("renders stored rows with one-based accessible names and data attributes; mount emits nothing", () => {
    ...
    const view = mountGallery({ initialValue: [item(), item({ src: "/media/two.jpg" })], onChange });
    try {
      ...
      expect(
        view.container.querySelector('[data-media-url-value="/media/item.jpg"]')
      ).toBeTruthy();
      expect(
        view.container.querySelector('[data-media-url-value="/media/two.jpg"]')
      ).toBeTruthy();
  ...
  test("alt 500 / caption 2000 commit; 501 / 2001 / source 2049 direct attempts are non-mutating", () => {
    ...
    const onChange = vi.fn();
    const view = mountGallery({ initialValue: [item({ alt: "Old", caption: "Old caption" })], onChange });
    try {
...
describe("TASK-539 category token builder (filterCategories)", () => {
  test("renders stored tokens in order with one-based names; mount emits nothing", () => {
    ...
    try {
      ...
      expect(view.container.querySelector('button[aria-label="Remove category token 1"]')).toBeTruthy();
      expect(onChange).not.toHaveBeenCalled();
    } finally {
      view.cleanup();
    }
