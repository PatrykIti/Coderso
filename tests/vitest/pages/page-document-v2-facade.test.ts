...
// 78 explicit per-specifier type imports from the facade (compile-time fixture).
import {
...
// ── Compile-time fixture: all 78 explicit facade types are imported with
...
export const facadeTypeCount = 78;
...
describe("TASK-539 facade manifest (page-document-v2-facade)", () => {
  ...
  test("exactly 78 explicit type names reach the facade with exact owners", () => {
    const actual = parsed
    ...
    expect(actual).toHaveLength(78);
  });

  test("exactly 133 runtime names reach the facade with exact owners", () => {
    const actual = parsed
      .filter((entry) => !entry.typeOnly)
      .map(({ name, owner }): readonly [string, string] => [name, owner])
      .sort(byName);
    expect(actual).toEqual(sortMapped(facadeRuntimeOwnerMap));
    expect(actual).toHaveLength(133);
  });

  test("facade module evaluates to exactly the 133 runtime names", () => {
    expect(Object.keys(facadeNamespace).sort()).toEqual(
      facadeRuntimeOwnerMap.map(([name]) => name).sort()
    );
  });

  test("all 133 runtime names are the direct owner bindings (toBe identity)", () => {
  ...
  test("the 78-type compile-time tuple is exported and structurally sound", () => {
    ...
    expect(facadeTypeCount).toBe(78);
  });
});
