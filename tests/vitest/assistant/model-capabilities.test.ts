...
test("resolveModelCapabilityProfile detects common model families", () => {
  expect(resolveModelCapabilityProfile({ provider: "openrouter", model: "openai/gpt-5.4-nano" })).toMatchObject({
    family: "gpt",
    structuredOutput: {
  ...
  expect(resolveModelCapabilityProfile({ provider: "openrouter", model: "google/gemini-3.1-flash" })).toMatchObject({
    family: "gemini",
    structuredOutput: {
      preferred: "json_schema_strict",
    },
  });
  expect(resolveModelCapabilityProfile({ provider: "openrouter", model: "qwen/qwen3-7b" })).toMatchObject({
    family: "qwen",
    structuredOutput: {
      preferred: "json_schema_strict",
    },
