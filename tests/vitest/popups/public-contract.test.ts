...
describe("matchPopupRequest", () => {
  test("matches when both targeting and audience apply", () => {
    ...
    expect(matchPopupRequest(popup, { path: "/blog/post-1", isLoggedIn: true })).toBe(true);
  });
  ...
  test("fails when the path does not match", () => {
    ...
    expect(matchPopupRequest(popup, { path: "/contact", isLoggedIn: true })).toBe(false);
  });
  ...
  test("fails when the audience does not match", () => {
    ...
    expect(matchPopupRequest(popup, { path: "/blog/post-1", isLoggedIn: false })).toBe(false);
  });

  test("fails when an exclude path wins", () => {
    const popup = makePopup();
    expect(matchPopupRequest(popup, { path: "/blog/private", isLoggedIn: true })).toBe(false);
  });
});
...
describe("toPublicPopup", () => {
  test("projects exactly the 6 public keys and strips the PII gate fields", () => {
    ...
    const forbidden = ["name", "status", "targeting", "createdAt", "updatedAt", "publishedAt"];
  ...
  test("strips content.templateId and keeps only render fields", () => {
    const result = toPublicPopup(makePopup());
    expect(Object.keys(result.content).sort()).toEqual(["body", "ctaHref", "ctaLabel", "title"]);
...
describe("popupPublicQuerySchema", () => {
  ...
  test("rejects an empty path", () => {
    expect(() => validate(popupPublicQuerySchema, { path: "" })).toThrow("Invalid payload");
  });
});
