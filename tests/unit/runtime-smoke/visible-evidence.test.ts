...
type MutableVariant = DeepMutable<SmokeEvidenceVariantV1> & { assertions: MutableAssertion[]; [key: string]: unknown };
...
describe("requireManifestableScenarioResults", () => {
  ...
  test("legacy scenarios without optional evidence are rejected as manifestable", () => {
    expect(errorCode(() => requireManifestableScenarioResults([legacyScenario("scenario-one")], []))).toBe(
  ...
  test("missing title, variants, assertions, or screenshots fail closed", () => {
    expect(errorCode(() =>
      requireManifestableScenarioResults([strictScenario("scenario-one", { title: undefined })], globalScreenshots())
    )).toBe("smoke_schema_invalid");
    expect(errorCode(() =>
      requireManifestableScenarioResults([strictScenario("scenario-one", { variants: undefined })], globalScreenshots())
    )).toBe("smoke_schema_invalid");
    expect(errorCode(() =>
      requireManifestableScenarioResults([strictScenario("scenario-one", { screenshots: undefined })], globalScreenshots())
    )).toBe("smoke_schema_invalid");
    expect(errorCode(() =>
      requireManifestableScenarioResults([strictScenario("scenario-one", { variants: [] })], globalScreenshots())
    )).toBe("smoke_schema_invalid");
    expect(errorCode(() =>
      requireManifestableScenarioResults(
    ...
    expect(errorCode(() =>
      requireManifestableScenarioResults([strictScenario("scenario-one", { screenshots: [] })], globalScreenshots())
    )).toBe("smoke_schema_invalid");
  });
  ...
  test("unknown nested keys fail closed", () => {
    ...
    expect(errorCode(() => requireManifestableScenarioResults([unknownVariant], globalScreenshots()))).toBe(
      "smoke_schema_invalid"
    );
    const unknownAssertion = clone(strictScenario("scenario-one"));
    unknownAssertion.variants[0].assertions[0].extra = true;
    expect(errorCode(() => requireManifestableScenarioResults([unknownAssertion], globalScreenshots()))).toBe(
      "smoke_schema_invalid"
    );
    const unknownScreenshot = clone(strictScenario("scenario-one"));
    unknownScreenshot.screenshots[0].extra = true;
    expect(errorCode(() => requireManifestableScenarioResults([unknownScreenshot], globalScreenshots()))).toBe(
      "smoke_schema_invalid"
    );
  });
  ...
  test("scalar, array, and dimension caps fail closed", () => {
    ...
    expect(errorCode(() => requireManifestableScenarioResults([tooManyVariants], globalScreenshots()))).toBe(
      "smoke_schema_invalid"
    );
    const tooManyScreenshots = clone(strictScenario("scenario-one"));
    for (let index = 1; index < 129; index += 1) {
      tooManyScreenshots.screenshots.push({ path: `extra-${index}.png`, sha256: sha(`extra-${index}.png`) });
    }
    expect(errorCode(() => requireManifestableScenarioResults([tooManyScreenshots], globalScreenshots()))).toBe(
      "smoke_schema_invalid"
    );
    const oversized = clone(strictScenario("scenario-one"));
    oversized.title = "x".repeat(10_001);
    expect(errorCode(() => requireManifestableScenarioResults([oversized], globalScreenshots()))).toBe(
      "smoke_schema_invalid"
    );
    const badDimensions = clone(strictScenario("scenario-one"));
    badDimensions.variants[0].viewport = { width: -1, height: 844 };
    expect(errorCode(() => requireManifestableScenarioResults([badDimensions], globalScreenshots()))).toBe(
  ...
  test("false assertions and nonempty console errors fail closed", () => {
    ...
    expect(errorCode(() => requireManifestableScenarioResults([falseAssertion], globalScreenshots()))).toBe(
      "smoke_assertion_failed"
    );
    const consoleError = clone(strictScenario("scenario-one"));
    consoleError.variants[0].consoleErrors = ["oops"];
    expect(errorCode(() => requireManifestableScenarioResults([consoleError], globalScreenshots()))).toBe(
...
describe("assertExactUniqueScreenshotUnion", () => {
  ...
  test("extra, missing, reordered, and duplicate-ownership screenshots fail", () => {
    const scenarios = strictScenarios();
    expect(errorCode(() =>
      assertExactUniqueScreenshotUnion(scenarios, [...globalScreenshots(), { path: "extra.png", sha256: sha("extra.png") }])
    )).toBe("smoke_manifest_report_screenshot_mismatch");
    expect(errorCode(() => assertExactUniqueScreenshotUnion(scenarios, globalScreenshots().slice(1)))).toBe(
      "smoke_manifest_report_screenshot_mismatch"
    );
    expect(errorCode(() => assertExactUniqueScreenshotUnion(scenarios, [...globalScreenshots()].reverse()))).toBe(
      "smoke_manifest_report_screenshot_mismatch"
    );
    ...
    expect(errorCode(() =>
      assertExactUniqueScreenshotUnion([...scenarios.slice(0, 1), duplicateOwnership, ...scenarios.slice(2)], globalScreenshots())
...
describe("normalizeStrictManifestableScenario", () => {
  test("passes strict scenarios and rejects legacy", () => {
    ...
    expect(errorCode(() => normalizeStrictManifestableScenario(legacyScenario("scenario-one")))).toBe(
      "smoke_schema_invalid"
    );
    expect(errorCode(() => normalizeStrictManifestableScenario(strictScenario("scenario-one", { pass: false })))).toBe(
...
describe("requireManifestEqualsRunnerReport", () => {
  ...
  test("mutated report evidence fails with the exact mismatch code", () => {
    const cases: [Record<string, unknown>, string][] = [
      [report({ scenarios: strictScenarios().map((s, index) => (index === 0 ? { ...s, id: "drifted-id" } : s)) }), "smoke_manifest_report_evidence_mismatch"],
      [report({ scenarios: strictScenarios().map((s, index) => (index === 0 ? { ...s, title: "Drift" } : s)) }), "smoke_manifest_report_evidence_mismatch"],
      [
        report({
      ...
      [
        report({
          scenarios: strictScenarios().map((s, index) =>
            index === 0 ? { ...s, screenshots: [{ path: "drift.png", sha256: sha("drift.png") }] } : s
          ),
        }),
        "smoke_manifest_report_evidence_mismatch",
      ],
      [report({ screenshots: [...globalScreenshots(), { path: "extra.png", sha256: sha("extra.png") }] }), "smoke_manifest_report_screenshot_mismatch"],
    ];
    for (const [mutated, expectedCode] of cases) {
      expect(errorCode(() => requireManifestEqualsRunnerReport(validManifest(), mutated))).toBe(expectedCode);
    }
  });
  ...
  test("a failed report scenario fails", () => {
    expect(errorCode(() =>
      requireManifestEqualsRunnerReport(
        validManifest(),
        report({ scenarios: strictScenarios().map((s, index) => (index === 0 ? { ...s, pass: false } : s)) })
      )
  ...
  test("a legacy report without strict evidence fails", () => {
    expect(errorCode(() =>
      requireManifestEqualsRunnerReport(validManifest(), report({ scenarios: [legacyScenario("scenario-one")] }))
    )).toBe("smoke_schema_invalid");
  });
});
...
describe("cross-suite manifest fixtures", () => {
  ...
  async function loadSuite(suite: string): Promise<{ manifest: SmokeEvidenceManifestV1; report: MutableReport }> {
    const manifest = JSON.parse(await readFile(join(FIXTURES, `${suite}-manifest.json`), "utf8")) as SmokeEvidenceManifestV1;
    const report = JSON.parse(await readFile(join(FIXTURES, `${suite}-report.json`), "utf8")) as MutableReport;
  ...
  function mutationCases(): MutationCase[] {
    return [
      ["report scenario id", (_m, r) => { r.scenarios[0].id = "drifted-id"; }, "smoke_manifest_report_evidence_mismatch"],
      ["report scenario pass bit", (_m, r) => { r.scenarios[0].pass = false; }, "smoke_scenario_not_passed"],
      ["report scenario title", (_m, r) => { r.scenarios[0].title = "Drifted title"; }, "smoke_manifest_report_evidence_mismatch"],
      ["report variant", (_m, r) => { r.scenarios[0].variants[0].theme = "dark"; }, "smoke_manifest_report_evidence_mismatch"],
      ["report assertion expected", (_m, r) => { r.scenarios[0].variants[0].assertions[0].expected = "block"; }, "smoke_manifest_report_evidence_mismatch"],
      ["report assertion actual", (_m, r) => { r.scenarios[0].variants[0].assertions[0].actual = "block"; }, "smoke_manifest_report_evidence_mismatch"],
      ["report assertion pass bit", (_m, r) => { r.scenarios[0].variants[0].assertions[0].pass = false; }, "smoke_assertion_failed"],
      ["report console error", (_m, r) => { r.scenarios[0].variants[0].consoleErrors = ["ops"]; }, "smoke_console_errors"],
      ["report screenshot path", (_m, r) => { r.scenarios[0].screenshots[0].path = "drift.png"; }, "smoke_manifest_report_evidence_mismatch"],
      ["report screenshot hash", (_m, r) => { r.scenarios[0].screenshots[0].sha256 = sha("drift"); }, "smoke_manifest_report_evidence_mismatch"],
      ["report global screenshot order", (_m, r) => { r.screenshots.reverse(); }, "smoke_manifest_report_screenshot_mismatch"],
      ["report screenshot ownership", (_m, r) => {
      ...
      ["report scenario order", (_m, r) => { [r.scenarios[0], r.scenarios[1]] = [r.scenarios[1], r.scenarios[0]]; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest scenario id", (m, _r) => { m.scenarios[0].id = "drifted-id"; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest scenario title", (m, _r) => { m.scenarios[0].title = "Drifted title"; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest variant", (m, _r) => { m.scenarios[0].variants[0].viewport.width = 999; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest assertion expected", (m, _r) => { m.scenarios[0].variants[0].assertions[0].expected = "block"; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest assertion actual", (m, _r) => { m.scenarios[0].variants[0].assertions[0].actual = "block"; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest assertion pass bit", (m, _r) => { m.scenarios[0].variants[0].assertions[0].pass = false; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest console error", (m, _r) => { m.scenarios[0].variants[0].consoleErrors = ["ops"]; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest screenshot path", (m, _r) => { m.scenarios[0].screenshots[0].path = "drift.png"; }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest screenshot hash", (m, _r) => { m.scenarios[0].screenshots[0].sha256 = sha("drift"); }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest screenshot order", (m, _r) => {
        [m.scenarios[0].screenshots[0], m.scenarios[0].screenshots[1]] = [m.scenarios[0].screenshots[1], m.scenarios[0].screenshots[0]];
      }, "smoke_manifest_report_evidence_mismatch"],
      ["manifest screenshot ownership", (m, _r) => {
    ...
    test(`${suite} every report/manifest evidence mutation requires mismatch failure`, async () => {
      ...
      for (const [label, mutate, expectedCode] of mutationCases()) {
        ...
        const actual = errorCode(() =>
          requireManifestEqualsRunnerReport(manifestClone as unknown as SmokeEvidenceManifestV1, reportClone)
        );
