...
testIfDb(
  ...
  const [taxonomyState, storedFields, storedSeo, storedMenuItems] = await Promise.all([
    ...
    db
      .select()
      .from(formFields)
      .where(eq(formFields.formId, formRows[0]!.id)),
    db
      .select()
      .from(seoDocuments)
      .where(
        and(
          eq(seoDocuments.targetType, "page"),
          eq(seoDocuments.targetId, pageRows[0]!.id)
        )
      ),
...
testIfDb(
  ...
  const [existingType] = await db.insert(contentTypes).values({
  ...
  const [existingPage] = await db.insert(pages).values({
  ...
  const [appliedType, appliedForm, appliedPage, appliedMenu] = await Promise.all([
    db
      ...
      .then((rows) => rows[0]),
    db.select().from(forms).where(eq(forms.slug, createdFormSlug)).then((rows) => rows[0]),
    db.select().from(pages).where(eq(pages.slug, existingPageSlug)).then((rows) => rows[0]),
    db
      .select()
      .from(menus)
  ...
  const [appliedTaxonomy, appliedFields, appliedSeo, appliedItems] = await Promise.all([
    ...
    db
      .select()
      .from(seoDocuments)
      .where(and(eq(seoDocuments.targetType, "page"), eq(seoDocuments.targetId, appliedPage!.id))),
