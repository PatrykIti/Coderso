...
class SyntheticRepo {
  ...
  async fingerprintEveryScope(
    groups: readonly CanonicalAuditGroup[]
  ): Promise<Record<string, string>> {
    const out: Record<string, string> = {};
    for (const group of groups)
      out[group.repoRelativePath] = fileFingerprint(
        group.repoRelativePath,
        this.bytes(group.repoRelativePath)
      );
    return out;
  }
}
...
function makeHarness(options: HarnessOptions = {}): Harness {
  ...
  const auditFile = async (group: CanonicalAuditGroup, round: number) => {
    ...
    return options.audit ? options.audit(group, round, harness) : cleanPayload();
  };
  const reconcile = async (context: {
    ...
    return options.reconcile ? options.reconcile(context, harness) : cleanPayload();
  };
  ...
  harness.run = (overrides: Partial<CanonicalAuditRoundsOptions> = {}) =>
    runCanonicalAuditRounds({
      ...
      fingerprintEveryScope: (all: readonly CanonicalAuditGroup[]) =>
        repo.fingerprintEveryScope(all),
...
describe("result completeness guards", async () => {
  ...
  test("wrong, duplicate, and reordered file identities are rejected by the wrapper contract", async () => {
    ...
    await expectWorkflowError(
      () =>
        requireAllResults(
          envelopes([`file:${A}`, `file:unknown.md`, "reconcile"]),
          expected,
          "wrap"
        ),
  ...
  test("a genuine LOW stays visible and does not block HIGH/MEDIUM convergence", async () => {
    const harness = makeHarness({
      ...
      audit: (group, round) => {
        ...
        const findings: WorkflowStructuredFinding[] = [
          finding("LOW", "docs polish", { area: "style" }),
        ];
...
describe("exported driver helpers", async () => {
  test("requireDeclaredAffectedScopeIds validates and normalizes the declared claim", async () => {
    ...
    expect(requireDeclaredAffectedScopeIds({ affectedScopeIds: [B, A] }, groups, "h")).toEqual([
      A,
      B,
    ]);
    await expectWorkflowError(
  ...
  test("selectVerifiedAffectedGroups preserves declared group order", async () => {
    ...
    expect(
      selectVerifiedAffectedGroups(groups, [A, B]).map((group) => group.repoRelativePath)
    ).toEqual([B, A]);
    expect(
      selectVerifiedAffectedGroups(groups, [A]).map((group) => group.repoRelativePath)
    ).toEqual([A]);
  });
