...
const CHILD_PATH = join(import.meta.dir, "../../fixtures/workflows/smoke-evidence/closure/killRecoveryChild.ts");
...
async function frozenResume(): Promise<{ root: string; checkpoint: VerifiedTask545Checkpoint; identity: Task545ClosureIdentity }> {
...
function applyOpsLocal(text: string, ops: readonly ClosureMetadataOperationV1[]): string {
  ...
  for (const op of ops) {
    if (op.kind === "create_file") out = op.bytes as string;
    else if (op.kind === "replace_once") out = out.replace(op.search as string, op.replacement as string);
      ...
      if (found !== -1) lines[found] = `${prefix} ${op.value}`;
        ...
        const at = lines.map((line, i) => (line.startsWith(anchor) ? i : -1)).filter((i) => i !== -1);
...
function indexMutationFromPlan(plan: ClosureMetadataMutationPlanV1): { anchor: string; row: string; pointerFrom: string; pointerTo: string } {
  ...
  return { anchor: insert.anchor as string, row: insert.line as string, pointerFrom: replace.search as string, pointerTo: replace.replacement as string };
}
...
async function applyFullClosure(
  ...
  await writeFile(join(taskDir, "TASK-545-01-All-Results-Guard-And-Static-Workflow-Contract.md"), taskFileAfterState(FROZEN_TASK_FILE_PARENT_BYTES.toString("utf8"), identity.closureUtcDate));
  await writeFile(join(taskDir, "TASK-545-03-L01-Add-Require-All-Results-Helper.md"), taskFileAfterState(TASK_FILE_L01, identity.closureUtcDate));
  ...
  return writeOrResumeOrderedDurableChangelogFileThenIndexV1({
    ...
    changelogBytes: (changelogRecord.operations as readonly ClosureMetadataOperationV1[])[0].bytes as string,
...
function runCli(root: string, args: string[]): { status: number; stdout: string; stderr: string } {
  const result = spawnSync(process.execPath, [CLI_PATH, "closure-delta", "--repo-root", root, ...args], { encoding: "utf8" });
...
describe("buildExactClosureMetadataAllowlist", () => {
  test("returns exactly the task files, indexes, and pinned changelog path", async () => {
    ...
    expect(result.has("_docs/_TASKS/TASK-545-01-All-Results-Guard-And-Static-Workflow-Contract.md")).toBe(true);
  ...
  test("rejects traversal, unsorted, duplicate, and overlapping inputs", async () => {
    ...
    const call = (options: { readonly frozenContract: SmokeEvidenceClosureContractV1; readonly pinnedChangelogPath: `_docs/_CHANGELOG/${string}.md`; readonly closureUtcDate: string }) => () => buildExactClosureMetadataAllowlist(options);
    const evil = { frozenContract, pinnedChangelogPath: "_docs/_CHANGELOG/../README.md" as `_docs/_CHANGELOG/${string}.md`, closureUtcDate: identity.closureUtcDate };
    expect(errorCodeSync(call(evil))).toBe("smoke_path_invalid");
    const duplicate = { frozenContract, pinnedChangelogPath: "_docs/_CHANGELOG/1257-2026-08-14-other-slug.md" as `_docs/_CHANGELOG/${string}.md`, closureUtcDate: identity.closureUtcDate };
    expect(errorCodeSync(call(duplicate))).toBe("smoke_changelog_path_invalid");
    const badDate = { frozenContract, pinnedChangelogPath: "_docs/_CHANGELOG/1257-2026-02-31-task-545-smoke-evidence-checkpoint.md" as `_docs/_CHANGELOG/${string}.md`, closureUtcDate: "2026-02-31" };
    ...
    expect(errorCodeSync(call({ frozenContract: unsorted, pinnedChangelogPath: identity.pinnedChangelogPath, closureUtcDate: identity.closureUtcDate }))).toBe("smoke_closure_contract_invalid");
    const duplicated = { ...frozenContract, taskFiles: [frozenContract.taskFiles[0], frozenContract.taskFiles[0]] };
    expect(errorCodeSync(call({ frozenContract: duplicated, pinnedChangelogPath: identity.pinnedChangelogPath, closureUtcDate: identity.closureUtcDate }))).toBe("smoke_closure_contract_invalid");
    const overlap = { ...frozenContract, supplementalTaskFiles: [frozenContract.taskFiles[0]] };
    expect(errorCodeSync(call({ frozenContract: overlap, pinnedChangelogPath: identity.pinnedChangelogPath, closureUtcDate: identity.closureUtcDate }))).toBe("smoke_closure_contract_invalid");
  });
});
...
describe("buildClosureMetadataMutationPlanV1", () => {
  test("builds the exact five-record frozen plan with byte-exact after hashes", async () => {
    ...
    expect(changelog!.operations).toEqual([{ kind: "create_file", bytes: CHANGELOG_TEMPLATE, label: "changelog_file" }]);
    ...
    expect(parent!.operations.map((op) => op.kind)).toEqual(["replace_once", "upsert_field", "upsert_field"]);
    expect(parent!.operations[0].replacement as string).toContain("✅ Done");
    for (const record of plan) {
      const frozen = record.path === PINNED_CHANGELOG_REL ? "" : revBytes(root, record.path).toString("utf8");
  ...
  test("detects declared after-hash drift in the recomputation guard", async () => {
    ...
    const drifted = structuredClone(plan) as Array<{ path: string; beforeSha256: string; expectedAfterSha256: string; operations: readonly ClosureMetadataOperationV1[] }>;
    const target = drifted[0];
    target.expectedAfterSha256 = target.expectedAfterSha256.replace(/^./u, "0");
    const frozen = target.path === PINNED_CHANGELOG_REL ? "" : revBytes(root, target.path).toString("utf8");
  ...
  test("rejects tampered evidence, HEAD change, and stale base", async () => {
    ...
    await expect(errorCodeAsync(buildClosureMetadataMutationPlanV1(resume.checkpoint, resume.closureIdentity, { repoRoot: root }))).resolves.toBe("smoke_hash_mismatch");
    ...
    await expect(errorCodeAsync(buildClosureMetadataMutationPlanV1(second.checkpoint, second.identity, { repoRoot: second.root }))).resolves.toBe("smoke_head_changed");

    const third = await frozenResume();
    await writeFile(join(third.root, "_docs", "_CHANGELOG", "README.md"), FROZEN_CHANGELOG_INDEX + "dirty\n");
    await expect(errorCodeAsync(buildClosureMetadataMutationPlanV1(third.checkpoint, third.identity, { repoRoot: third.root }))).resolves.toBe("smoke_changelog_index_base_changed");
  });

  test("rejects ambiguous or missing board/statistics rows", async () => {
    async function resumeOver(taskIndexOverride: string): Promise<{ root: string; resume: Task545ClosureResume }> {
      const root = await makeRepo();
    ...
    const duplicate = await resumeOver(FROZEN_TASK_INDEX + "| TASK-545 | Duplicate | High | Medium | ⏳ To Do |\n");
    await expect(errorCodeAsync(buildClosureMetadataMutationPlanV1(duplicate.resume.checkpoint, duplicate.resume.closureIdentity, { repoRoot: duplicate.root }))).resolves.toBe("duplicateBoardStatisticRejected");

    const missingStatistics = FROZEN_TASK_INDEX.split("\n").filter((line) => !line.startsWith("| TASK-545 | ⏳ To Do | 4 | 13 |")).join("\n");
    const missing = await resumeOver(missingStatistics);
    await expect(errorCodeAsync(buildClosureMetadataMutationPlanV1(missing.resume.checkpoint, missing.resume.closureIdentity, { repoRoot: missing.root }))).resolves.toBe("boardStatisticMissing");
  });
});
...
describe("writeOrResumeOrderedDurableChangelogFileThenIndexV1", () => {
  test("completes the frozen file-then-index protocol with both identity and no residue", async () => {
    ...
    const result = await writeOrResumeOrderedDurableChangelogFileThenIndexV1({
      repoRoot: root, checkpoint, runId: identity.runId, closureIdentity: identity,
      changelogBytes: changelogRecord!.operations[0].bytes as string,
      changelogIndexMutation: indexMutationFromPlan(plan), protocol: WRITER_PROTOCOL,
    });
    expect(result.durableState).toBe("both");
    expect(await readFile(join(root, ...PINNED_CHANGELOG_REL.split("/")), "utf8")).toBe(CHANGELOG_TEMPLATE);
    expect(await readFile(join(root, "_docs", "_CHANGELOG", "README.md"), "utf8")).toBe(CHANGELOG_INDEX_AFTER);
  ...
  test("recovers every crash boundary through the child fixture", async () => {
    for (const killPoint of [0, 1, 2, 3, 4, 5, 6]) {
      const spawned = spawnSync(process.execPath, [CHILD_PATH, `--kill-point=${killPoint}`], { encoding: "utf8" });
      expect(spawned.status, `kill point ${killPoint}`).toBe(0);
      const handoff = JSON.parse(spawned.stdout);
      const result = await writeOrResumeOrderedDurableChangelogFileThenIndexV1({
        repoRoot: handoff.repoRoot, checkpoint: handoff.checkpoint, runId: RUN_ID,
        closureIdentity: handoff.closureIdentity, changelogBytes: handoff.changelogBytes,
        changelogIndexMutation: handoff.changelogIndexMutation, protocol: handoff.protocol as typeof WRITER_PROTOCOL,
      });
      expect(result.durableState, `kill point ${killPoint}`).toBe("both");
      expect(await readFile(join(handoff.repoRoot, ...PINNED_CHANGELOG_REL.split("/")), "utf8")).toBe(CHANGELOG_TEMPLATE);
      expect(await readFile(join(handoff.repoRoot, "_docs", "_CHANGELOG", "README.md"), "utf8")).toBe(CHANGELOG_INDEX_AFTER);
  ...
  test("fails closed on index-only, corrupt residue, and protocol drift", async () => {
    ...
    const base: OrderedDurableWriterOptionsV1 = {
      repoRoot: root, checkpoint, runId: RUN_ID, closureIdentity: identity,
      changelogBytes: CHANGELOG_TEMPLATE, changelogIndexMutation: mutation, protocol: WRITER_PROTOCOL,
    };
    await writeFile(join(changelogDir, "README.md"), FROZEN_CHANGELOG_INDEX + CHANGELOG_ROW + "\n");
    await expect(errorCodeAsync(writeOrResumeOrderedDurableChangelogFileThenIndexV1(base))).resolves.toBe("smoke_changelog_index_only");
    await writeFile(join(changelogDir, "README.md"), FROZEN_CHANGELOG_INDEX);

    await writeFile(join(changelogDir, `.smoke-closure.${RUN_ID}.journal`), "corrupt");
    await expect(errorCodeAsync(writeOrResumeOrderedDurableChangelogFileThenIndexV1(base))).resolves.toBe("smoke_transaction_artifact_mismatch");

    await writeFile(join(changelogDir, `.smoke-closure.${RUN_ID}.journal`), journalBytes(RUN_ID, PINNED_CHANGELOG_REL));
    ...
    await expect(errorCodeAsync(writeOrResumeOrderedDurableChangelogFileThenIndexV1({ ...base, protocol: "ordered-durable-changelog-file-then-index@v9" as unknown as typeof WRITER_PROTOCOL }))).resolves.toBe("smoke_changelog_protocol_invalid");
  });
});
...
describe("validateMetadataOnlyClosureDelta", () => {
  test("validates the exact allowlisted metadata delta and is idempotent", async () => {
    ...
    expect([...delta.changedPaths]).toEqual([
      PINNED_CHANGELOG_REL, CHANGELOG_INDEX_REL, TASK_INDEX_REL,
      "_docs/_TASKS/TASK-545-01-All-Results-Guard-And-Static-Workflow-Contract.md",
      "_docs/_TASKS/TASK-545-03-L01-Add-Require-All-Results-Helper.md",
    ]);
  ...
  test("fails closed on any non-allowlisted drift", async () => {
    async function driftCase(): Promise<{ root: string; checkpoint: VerifiedTask545Checkpoint; identityBoth: Task545ClosureIdentity & { durableState: "both" } }> {
      const { root, checkpoint, identity } = await frozenResume();
      const plan = await buildClosureMetadataMutationPlanV1(checkpoint, identity, { repoRoot: root });
      return { root, checkpoint, identityBoth: await applyFullClosure(root, checkpoint, identity, plan) };
    }
    const cases: Array<[string, (root: string) => Promise<void>, string]> = [
      ...
      ["test", (root) => mkdir(join(root, "tests/unit"), { recursive: true }).then(() => writeFile(join(root, "tests/unit/extra.test.ts"), "test\n")), "smoke_non_metadata_delta"],
      ["config", (root) => writeFile(join(root, "coderso.config.ts"), "export {}\n"), "smoke_non_metadata_delta"],
      ["runtime doc", (root) => writeFile(join(root, "_docs", "SECURITY_SPEC.md"), "# security\n"), "smoke_non_metadata_delta"],
      ["workflow", (root) => writeFile(join(root, "_docs", "_workflows", "task-545-implement.mjs"), "// changed\n"), "smoke_non_metadata_delta"],
      ["same-family file", (root) => writeFile(join(root, "_docs", "_TASKS", "TASK-545-05-New.md"), "# New\n"), "smoke_non_metadata_delta"],
    ];
    for (const [name, mutate, code] of cases) {
      const current = await driftCase();
      await mutate(current.root);
      await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(current.checkpoint, current.identityBoth, current.root)), name).resolves.toBe(code);
    }
  });

  test("fails closed on wrong number, slug, date, and prefix lookalikes", async () => {
    async function bothCorpus(): Promise<{ root: string; checkpoint: VerifiedTask545Checkpoint; identityBoth: Task545ClosureIdentity & { durableState: "both" } }> {
      const { root, checkpoint, identity } = await frozenResume();
      const plan = await buildClosureMetadataMutationPlanV1(checkpoint, identity, { repoRoot: root });
      return { root, checkpoint, identityBoth: await applyFullClosure(root, checkpoint, identity, plan) };
    }
    ...
    await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(wrongSlug.checkpoint, wrongSlug.identityBoth, wrongSlug.root))).resolves.toBe("smoke_changelog_path_invalid");
    ...
    await writeFile(join(dateDir, "1257-2026-08-15-task-545-smoke-evidence-checkpoint.md"), CHANGELOG_TEMPLATE.replace("2026-08-14", "2026-08-15"));
    await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(wrongDate.checkpoint, wrongDate.identityBoth, wrongDate.root))).resolves.toBe("smoke_changelog_date_invalid");

    const lookalike = await bothCorpus();
    const lookDir = join(lookalike.root, "_docs", "_CHANGELOG");
    await writeFile(join(lookDir, "1257-2026-08-14-task-545-smoke-evidence-checkpoint-extra.md"), CHANGELOG_TEMPLATE);
    await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(lookalike.checkpoint, lookalike.identityBoth, lookalike.root))).resolves.toBe("smoke_changelog_multiple");
    ...
    await writeFile(join(numDir, "1258-2026-08-14-task-545-smoke-evidence-checkpoint.md"), CHANGELOG_TEMPLATE);
    await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(wrongNumber.checkpoint, wrongNumber.identityBoth, wrongNumber.root))).resolves.toBe("smoke_changelog_index_only");
  });
  ...
  test("fails closed on metadata content drift inside allowlisted files", async () => {
    ...
    const plan = await buildClosureMetadataMutationPlanV1(checkpoint, identity, { repoRoot: root });
    const identityBoth = await applyFullClosure(root, checkpoint, identity, plan);
    const taskPath = join(root, "_docs", "_TASKS", "TASK-545-01-All-Results-Guard-And-Static-Workflow-Contract.md");
    await writeFile(taskPath, (await readFile(taskPath, "utf8")) + "arbitrary prose\n");
    await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(checkpoint, identityBoth, root))).resolves.toBe("smoke_metadata_delta_mismatch");
    await git(root, ["checkout", "-q", "--", "_docs/_TASKS/TASK-545-01-All-Results-Guard-And-Static-Workflow-Contract.md"]);

    const second = await frozenResume();
    const plan2 = await buildClosureMetadataMutationPlanV1(second.checkpoint, second.identity, { repoRoot: second.root });
    const identityBoth2 = await applyFullClosure(second.root, second.checkpoint, second.identity, plan2);
    const readmePath = join(second.root, "_docs", "_TASKS", "README.md");
    await writeFile(readmePath, (await readFile(readmePath, "utf8")).replace("| TASK-540 | Other Board Row | Low | Small | ✅ Done |", "| TASK-540 | Other Board Row | Low | Small | ⏳ To Do |"));
    await expect(errorCodeAsync(validateMetadataOnlyClosureDelta(second.checkpoint, identityBoth2, second.root))).resolves.toBe("smoke_metadata_delta_mismatch");
...
describe("runClosureDeltaCli", () => {
  test("applies the full closure and emits the exact JSON receipt", async () => {
    ...
    const plan = await buildClosureMetadataMutationPlanV1(checkpoint, closureIdentity, { repoRoot: root });
    ...
    const args = ["--task", TASK, "--suite", SUITE, "--profile", PROFILE, "--checkpoint", pause.checkpointPath, "--checkpoint-sha256", pause.checkpointSha256, "--run-id", closureIdentity.runId, "--session", SESSION];
    ...
    expect(receipt.changedPaths).toEqual([pinned, CHANGELOG_INDEX_REL, TASK_INDEX_REL, "_docs/_TASKS/TASK-545-01-All-Results-Guard-And-Static-Workflow-Contract.md", "_docs/_TASKS/TASK-545-03-L01-Add-Require-All-Results-Helper.md"]);
  });
  ...
  test("emits usage errors with exit 2 and fails closed on identity mismatch", async () => {
    ...
    const partialFlags = runCli(corpusRoot, ["--task", TASK, "--suite", SUITE, "--profile", PROFILE, "--checkpoint-sha256", "0".repeat(64), "--run-id", RUN_ID, "--session", SESSION]);
  ...
  test("fails closed on wrong checkpoint hash and non-canonical paths", async () => {
    ...
    const wrongHash = runCli(root, [...base, "--checkpoint", pause.checkpointPath, "--checkpoint-sha256", "f".repeat(64), "--run-id", resume.closureIdentity.runId, "--session", SESSION]);
    ...
    const wrongPath = runCli(root, [...base, "--checkpoint", relPath, "--checkpoint-sha256", pause.checkpointSha256, "--run-id", resume.closureIdentity.runId, "--session", SESSION]);
    expect(wrongPath.status).toBe(1);
    expect(JSON.parse(wrongPath.stderr).code).toBe("smoke_checkpoint_path_mismatch");
    const wrongTask = runCli(root, [...base, "--suite", "wrong-suite", "--checkpoint", pause.checkpointPath, "--checkpoint-sha256", pause.checkpointSha256, "--run-id", resume.closureIdentity.runId, "--session", SESSION]);
...
describe("re-export surface and type gates", () => {
  ...
  test("type fixtures pin the writer option and delta shapes", () => {
    ...
    const options = {
      repoRoot: "/tmp", checkpoint, runId: RUN_ID, closureIdentity: identity,
      changelogBytes: CHANGELOG_TEMPLATE, changelogIndexMutation: changelogIndexMutation(),
      protocol: WRITER_PROTOCOL,
    };
