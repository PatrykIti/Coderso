...
describe("validateSmokeEvidenceManifest", () => {
  ...
  test("wrong schema version rejected", () => {
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ schemaVersion: 2 })))).toBe(
      "smoke_manifest_version_unknown"
    );
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ schemaVersion: "1" })))).toBe(
      "smoke_manifest_version_unknown"
    );
  });
  ...
  test("empty scenarios and fewer than five scenarios fail", () => {
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ scenarios: baseScenarios().slice(0, 4) })))).toBe(
      "smoke_schema_invalid"
    );
  });

  test("hash grammar failures fail closed", () => {
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ report: { path: "report.json", sha256: "x".repeat(64) } })))).toBe(
      "smoke_hash_invalid"
    );
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ revision: revision({ gitHead: "z".repeat(40) }) })))).toBe(
      "smoke_revision_invalid"
    );
  });
  ...
  test("serverUp false and non-boolean fail", () => {
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ serverUp: false })))).toBe("smoke_server_down");
  });
  ...
  test("nested screenshot entries are validated", () => {
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(emptyScreenshots))).toBe("smoke_schema_invalid");
  });
  ...
  test("duplicate scenario and variant ids are rejected", () => {
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(dupScenario))).toBe("smoke_scenario_duplicate");
    const dupVariant = clone(validManifest());
    dupVariant.scenarios[0].variants.push(variant(dupVariant.scenarios[0].variants[0].id));
    expect(errorCode(() => validateSmokeEvidenceManifest(dupVariant))).toBe("smoke_variant_duplicate");
  });
  ...
  test("console errors, failed assertions, and bad variant fields fail", () => {
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(consoleError))).toBe("smoke_console_errors");
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(badSurface))).toBe("smoke_variant_invalid");
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(badViewport))).toBe("smoke_variant_invalid");
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(emptyVariants))).toBe("smoke_schema_invalid");
    const emptyAssertions = clone(validManifest());
    emptyAssertions.scenarios[0].variants[0].assertions = [];
    expect(errorCode(() => validateSmokeEvidenceManifest(emptyAssertions))).toBe("smoke_schema_invalid");
  });
  ...
  test("admin light+dark coverage is required when any admin surface exists", () => {
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(clone1))).toBe("smoke_admin_theme_coverage_missing");
    ...
    expect(errorCode(() => validateSmokeEvidenceManifest(clone2))).toBe("smoke_admin_theme_coverage_missing");
  });

  test("task id, session, profile, and generatedAt grammar fail closed", () => {
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ taskId: "TASK-12" })))).toBe("smoke_task_id_invalid");
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ taskId: "task-540" })))).toBe("smoke_task_id_invalid");
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ session: "BAD/SESSION" })))).toBe("smoke_session_invalid");
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ profile: "full" })))).toBe("smoke_schema_invalid");
    expect(errorCode(() => validateSmokeEvidenceManifest(validManifest({ generatedAt: "not-a-date" })))).toBe(
      "smoke_schema_invalid"
    );
  });
...
describe("projectSmokeEvidenceManifest", () => {
  ...
  test("failed or non-manifestable report scenarios are rejected", () => {
    const failed = validReport({
      scenarios: [{ id: "admin-light-wide", pass: false, elapsedMs: 1, title: "T", variants: [], screenshots: [] }],
    });
    expect(errorCode(() =>
      projectSmokeEvidenceManifest({
        taskId: TASK, suiteId: SUITE, profile: PROFILE, session: SESSION,
        reportPath: "report.json", reportSha256: "0".repeat(64), revision: revision(),
        generatedAt: "2026-08-13T18:00:00.000Z", report: failed,
      })
    )).toBe("smoke_scenario_not_passed");
    const legacy = validReport({ scenarios: [{ id: "admin-light-wide", pass: true, elapsedMs: 1 }] });
    expect(errorCode(() =>
      projectSmokeEvidenceManifest({
        taskId: TASK, suiteId: SUITE, profile: PROFILE, session: SESSION,
        reportPath: "report.json", reportSha256: "0".repeat(64), revision: revision(),
        generatedAt: "2026-08-13T18:00:00.000Z", report: legacy,
      })
    )).toBe("smoke_schema_invalid");
  });

  test("global screenshot union drift is rejected", () => {
    const extra = validReport({ screenshots: [...validReport().screenshots, { path: "extra.png", sha256: sha("extra.png") }] });
    expect(errorCode(() =>
      projectSmokeEvidenceManifest({
        taskId: TASK, suiteId: SUITE, profile: PROFILE, session: SESSION,
        reportPath: "report.json", reportSha256: "0".repeat(64), revision: revision(),
        generatedAt: "2026-08-13T18:00:00.000Z", report: extra,
      })
...
describe("canonical evidence directory", () => {
  ...
  test("rejects invalid task ids, sessions, and non-repo roots", async () => {
    const root = await makeRepo();
    await expect(errorCodeAsync(() => resolveCanonicalEvidenceDirectory(root, "TASK-1", SESSION))).resolves.toBe(
      "smoke_task_id_invalid"
    );
    await expect(errorCodeAsync(() => resolveCanonicalEvidenceDirectory(root, TASK, "bad/session"))).resolves.toBe(
      "smoke_session_invalid"
    );
    await expect(errorCodeAsync(() => resolveCanonicalEvidenceDirectory(join(tmpdir(), "missing-root-xyz"), TASK, SESSION))).rejects.toThrow();
  });
  ...
  test("rejects a symlinked evidence component", async () => {
    ...
    await expect(errorCodeAsync(() => resolveCanonicalEvidenceDirectory(root, TASK, SESSION))).resolves.toBe(
      "smoke_path_symlink"
    );
  });
  ...
  test("rejects a same-basename evidence root under a different repository root", async () => {
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({
        repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE,
        expectedSession: SESSION, expectedRevision: publicRevision(revision),
      })
...
describe("validateSmokeEvidence driver", () => {
  async function buildEvidence(overrides: {
  ...
  test("valid evidence passes the full driver", async () => {
    ...
    expect(result.referencedFiles).toEqual(
      ["admin-dark-narrow.png", "admin-form-focus.png", "admin-light-wide.png", "admin-menu-open.png", "manifest.json", "public-mobile-nav.png", "report.json"].sort()
    );
  });
  ...
  test("task, suite, and session mismatches fail", async () => {
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: "TASK-540", expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision })
    )).resolves.toBe("smoke_evidence_file_missing");
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: TASK, expectedSuite: "task-540", expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision })
    )).resolves.toBe("smoke_suite_mismatch");
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: "other-session", expectedRevision })
    )).resolves.toBe("smoke_evidence_file_missing");
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: wrongTask.root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(wrongTask.revision) })
    )).resolves.toBe("smoke_task_manifest_mismatch");
    const wrongSession = await buildEvidence({ manifest: { ...validManifest(), session: "other-session" } });
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: wrongSession.root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(wrongSession.revision) })
  ...
  test("revision mismatch fails", async () => {
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: revision() })
  ...
  test("report hash mismatch fails", async () => {
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(revision) })
  ...
  test("screenshot byte hash mismatch and missing files fail", async () => {
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: wrongBytes.root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(wrongBytes.revision) })
    )).resolves.toBe("smoke_hash_mismatch");

    const { root, revision } = await buildEvidence();
    await rm(join(root, "_docs/_workflows/_smoke/evidence", TASK.toLowerCase(), SESSION, "public-mobile-nav.png"));
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(revision) })
    )).resolves.toBe("smoke_evidence_file_missing");
  });
  ...
  test("screenshot bytes above MAX_SCREENSHOT_BYTES fail before hashing", async () => {
    ...
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: oversized.root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(oversized.revision) })
  ...
  test("screenshots at exactly MAX_SCREENSHOT_BYTES pass", async () => {
    ...
    const report = validReport() as unknown as { scenarios: { screenshots: MutableScreenshot[] }[]; screenshots: MutableScreenshot[] };
    report.scenarios[0].screenshots[0].sha256 = sha256(bytes);
    report.screenshots[0].sha256 = sha256(bytes);
    const { root, revision } = await buildEvidence({ manifest, report, screenshots: { "admin-light-wide": bytes } });
    const result = await validateSmokeEvidence({
      repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE,
      expectedSession: SESSION, expectedRevision: publicRevision(revision),
    });
  ...
  test("oversized manifest fails before parsing", async () => {
    ...
    await writeFile(join(evidence, "manifest.json"), JSON.stringify({ pad: "x".repeat(1_100_000) }));
    await expect(errorCodeAsync(() =>
      validateSmokeEvidence({ repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE, expectedSession: SESSION, expectedRevision: publicRevision(revision) })
...
describe("auditSmokeEvidenceDirectory", () => {
  test("exact file set passes and unreferenced files fail", async () => {
    ...
    const pass = await auditSmokeEvidenceDirectory({
      repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE,
      expectedSession: SESSION, expectedRevision, requireCheckpoint: false, requireTracked: false,
    });
  ...
  test("requireTracked passes only after staging the exact set", async () => {
    ...
    await expect(errorCodeAsync(() =>
      auditSmokeEvidenceDirectory({
        repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE,
        expectedSession: SESSION, expectedRevision, requireCheckpoint: false, requireTracked: true,
      })
  ...
  test("requireCheckpoint expects the checkpoint control file", async () => {
    ...
    await expect(errorCodeAsync(() =>
      auditSmokeEvidenceDirectory({
        repoRoot: root, expectedTask: TASK, expectedSuite: SUITE, expectedProfile: PROFILE,
        expectedSession: SESSION, expectedRevision: publicRevision(revision),
        requireCheckpoint: true, requireTracked: false,
      })
    )).resolves.toBe("smoke_evidence_file_set_mismatch");
  });
});
...
describe("writeSmokeEvidenceManifest", () => {
  test("writes canonical bytes and refuses overwrite", async () => {
    const root = await makeRepo();
    const result = await writeSmokeEvidenceManifest({ repoRoot: root, expectedTask: TASK, expectedSession: SESSION, manifest: validManifest() });
    ...
    await expect(errorCodeAsync(() =>
      writeSmokeEvidenceManifest({ repoRoot: root, expectedTask: TASK, expectedSession: SESSION, manifest: validManifest() })
...
describe("grammar helpers", () => {
  test("task id, session, and path helpers fail closed", () => {
    ...
    expect(errorCode(() => requireRuntimeSmokeSessionName("task-545/cert"))).toBe("smoke_session_invalid");
