...
interface WorkflowEntry { readonly path: string; readonly role: WorkflowRole; readonly taskId: string | null; }
interface ParsedModule { readonly displayPath: string; readonly source: string; readonly sourceFile: ts.SourceFile; }
...
const CANONICAL_FUTURE_ENTRY = /^task-(?<task>[0-9]{3}|9999)-(?<suffix>author-audit|implement|fix)\.mjs$/u;
...
function classifyTrackedEntryOrThrow(relativePath: string): WorkflowEntry {
  ...
  if (initialRole) return { path: relativePath, role: initialRole, taskId: ENTRY_TASK_NUMBER.exec(basename)?.groups?.task ?? null };
  const future = CANONICAL_FUTURE_ENTRY.exec(basename);
  if (future?.groups) return { path: relativePath, role: future.groups.suffix as WorkflowRole, taskId: future.groups.task ?? null };
  throw new Error(`tracked workflow entry is neither an initial TASK-545 entry nor a canonical future entry: ${relativePath}`);
}
function trackedWorkflowEntries(root: string = ROOT): WorkflowEntry[] {
  return trackedWorkflowFiles(root).filter((relativePath) => !relativePath.includes("/lib/")).map(classifyTrackedEntryOrThrow);
}
function trackedWorkflowEntriesByRole(...roles: WorkflowRole[]): string[] {
  return trackedWorkflowEntries().filter((entry) => roles.includes(entry.role)).map((entry) => entry.path);
}
function trackedUiClosureWorkflowFiles(): string[] {
  // UI-capable entries run real smoke and pause for owner evidence staging.
  return trackedWorkflowEntries().map((entry) => entry.path).filter((file) =>
...
function assertTrackedRegularFileNoSymlink(relativePath: string, root: string = ROOT): void {
  ...
  let stats;
  try { stats = lstatSync(path.join(root, relativePath)); } catch {
...
function expectTrackedInitialEntriesOrCanonicalExtensions(
  entries: readonly WorkflowEntry[], initial: ReadonlyArray<readonly [string, WorkflowRole]>
): void {
  ...
  for (const [filename, role] of initial) {
    ...
    if (!entry) throw new Error(`initial TASK-545 migration entry missing from inventory: ${filename}`);
    if (entry.role !== role) throw new Error(`initial TASK-545 migration entry role mismatch for ${filename}: ${entry.role}`);
  ...
  for (const entry of entries) {
    ...
    if (!future?.groups) throw new Error(`tracked entry does not match the canonical future pattern: ${entry.path}`);
    if (future.groups.suffix !== entry.role) throw new Error(`tracked future entry role mismatch for ${entry.path}`);
    assertTrackedRegularFileNoSymlink(entry.path);
    assertBytesEqualGitShowHead(entry.path);
  }
}
...
function parseModuleSource(source: string, displayPath: string): ParsedModule {
  return { displayPath, source, sourceFile: ts.createSourceFile(displayPath, source, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS) };
}
...
interface ImportBinding { readonly importedName: string | null; readonly localName: string; }
function staticImportBindings(sourceFile: ts.SourceFile): ReadonlyArray<{ readonly module: string; readonly bindings: readonly ImportBinding[] }> {
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (clause) {
      ...
      if (clause.namedBindings) {
        if (ts.isNamespaceImport(clause.namedBindings)) bindings.push({ importedName: null, localName: clause.namedBindings.name.text });
        else for (const specifier of clause.namedBindings.elements) {
          bindings.push({ importedName: specifier.propertyName ? specifier.propertyName.text : specifier.name.text, localName: specifier.name.text });
        }
      }
    }
...
function dynamicImportBindings(sourceFile: ts.SourceFile): ReadonlyArray<{ readonly module: string; readonly localNames: readonly string[] }> {
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isVariableDeclaration(node) && node.initializer && ts.isAwaitExpression(node.initializer) &&
      ts.isCallExpression(node.initializer.expression) && node.initializer.expression.expression.kind === ts.SyntaxKind.ImportKeyword &&
      ...
      if (ts.isObjectBindingPattern(node.name)) {
        for (const element of node.name.elements) {
          if (ts.isBindingElement(element) && ts.isIdentifier(element.name)) localNames.push(element.name.text);
        }
...
function importedSymbolFromLib(sourceFile: ts.SourceFile, libBasename: string, symbolName: string): boolean {
  const names = new Set<string>();
  for (const entry of staticImportBindings(sourceFile)) {
    if (entry.module.split("/").pop() === libBasename) for (const binding of entry.bindings) names.add(binding.localName);
  }
  for (const entry of dynamicImportBindings(sourceFile)) {
    if (entry.module.split("/").pop() === libBasename) for (const name of entry.localNames) names.add(name);
  }
...
function callsOfIdentifier(sourceFile: ts.SourceFile, name: string): ts.CallExpression[] {
  return collectCalls(sourceFile).filter((call) => ts.isIdentifier(call.expression) && call.expression.text === name);
}
function hasFunctionDeclaration(sourceFile: ts.SourceFile, name: string): boolean {
  let found = false;
  const visit = (node: ts.Node): void => {
    if (found) return;
    if (ts.isFunctionDeclaration(node) && node.name?.text === name) { found = true; return; }
    if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && node.name.text === name &&
      node.initializer && (ts.isArrowFunction(node.initializer) || ts.isFunctionExpression(node.initializer))) { found = true; return; }
    ts.forEachChild(node, visit);
  };
  visit(sourceFile);
  return found;
...
function propertyNameOf(property: ts.ObjectLiteralElementLike): string | null {
  ...
  if (ts.isPropertyAssignment(property) && (ts.isIdentifier(property.name) || ts.isStringLiteral(property.name))) return property.name.text;
  return null;
}
...
function propertyValueOf(call: ts.CallExpression, key: string): ts.Expression | null {
  ...
  for (const property of first.properties) {
    ...
    if (ts.isPropertyAssignment(property) && propertyNameOf(property) === key) return property.initializer;
  }
  return null;
}
const isFunctionLike = (node: ts.Node): boolean => ts.isArrowFunction(node) || ts.isFunctionExpression(node) || ts.isIdentifier(node);

// ---- Agent-result source analysis and unguarded-consumer detection ----

interface SourceAnalysis { readonly agentVars: ReadonlySet<string>; readonly collectionVars: ReadonlySet<string>; }
function callCalleeIdentifier(node: ts.Node): string | null {
  if (!ts.isCallExpression(node)) return null;
  if (ts.isIdentifier(node.expression)) return node.expression.text;
  if (ts.isPropertyAccessExpression(node.expression) && ts.isIdentifier(node.expression.expression) &&
    node.expression.name.text === "all") return `${node.expression.expression.text}.all`;
  return null;
}
function addBindingNames(pattern: ts.BindingName, target: Set<string>): void {
  if (ts.isIdentifier(pattern)) { target.add(pattern.text); return; }
  for (const element of pattern.elements) {
    if (ts.isBindingElement(element) && ts.isIdentifier(element.name)) target.add(element.name.text);
  }
}
function analyzeSources(sourceFile: ts.SourceFile): SourceAnalysis {
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isVariableDeclaration(node) && node.initializer) {
      ...
      if (callee === "parallel" || callee === "Promise.all") addBindingNames(node.name, collectionVars);
    }
...
function rootIdentifier(expression: ts.Expression): string | null {
  ...
  for (;;) {
    ...
    if (ts.isParenthesizedExpression(node) || ts.isNonNullExpression(node) || ts.isAsExpression(node) ||
      ts.isTypeAssertionExpression(node) || ts.isAwaitExpression(node)) { node = node.expression; continue; }
    return null;
  }
}
const isSplitCallChain = (expression: ts.Expression): boolean =>
  ts.isCallExpression(expression) && ts.isPropertyAccessExpression(expression.expression) && expression.expression.name.text === "split";

const IDENTITY_GUARD_CALL_NAMES = Object.freeze([
  "requireAllResults", "requirePass", "requireCleanAudit", "requireResult",
  "normalizeAuditFindings", "normalizeAuthorAuditResult",
]);
const IDENTITY_GUARD_FUNCTION_NAMES = Object.freeze([
  "requirePass", "requireCleanAudit", "requireResult", "normalizeAuditFindings", "normalizeAuthorAuditResult",
]);
...
function unguardedAgentResultConsumers(sourceFile: ts.SourceFile): string[] {
  ...
  const position = (node: ts.Node): string => `${sourceFile.fileName}:${sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile)).line + 1}`;
  for (const call of collectCalls(sourceFile)) {
    ...
    const isCollectionRoot = root !== null && (analysis.agentVars.has(root) || analysis.collectionVars.has(root));
    if (property === "filter" && call.arguments.length === 1 && ts.isIdentifier(call.arguments[0]) && call.arguments[0].text === "Boolean") {
      ...
      if (isCollectionRoot) violations.push(`filter(Boolean) applied to an agent-result collection before identity validation (${position(call)})`);
      continue;
    }
    if (["flatten", "flatMap", "map", "reduce", "reduceRight", "count"].includes(property) && root !== null && analysis.collectionVars.has(root)) {
      violations.push(`${property} applied to an agent-result collection before identity validation (${position(call)})`);
    }
  }
  const visit = (node: ts.Node): void => {
    if (ts.isPropertyAccessExpression(node) && (node.name.text === "length" || node.name.text === "size") &&
      ...
      if (root !== null && analysis.collectionVars.has(root) && ["filter", "flatMap", "map", "reduce"].includes(inner.name.text)) {
        violations.push(`clean classification counts a filtered/mapped agent-result collection before identity validation (${position(node)})`);
      }
    }
    ts.forEachChild(node, visit);
  };
...
function assertIdentityGuardCallSites(sourceFile: ts.SourceFile, displayPath: string): void {
  for (const call of collectCalls(sourceFile)) {
    ...
    if (name === "requireAllResults" && call.arguments.length !== 3) {
      throw new Error(`${displayPath}: requireAllResults must receive (results, expectedIdentities, label): ${call.arguments.length} argument(s)`);
    }
    if (IDENTITY_GUARD_CALL_NAMES.includes(name) && name !== "requireAllResults" && call.arguments.length < 2) {
...
const FULL_UNGUARDED_CONSUMER_OPTIONS: UnguardedConsumerOptions = Object.freeze({
  rejectFilterBooleanBeforeValidation: true, rejectFlattenCountOrCleanBeforeValidation: true,
  requireTrustedOrderedIdentityEnvelopes: true, allowUnrelatedDomainCollectionFiltering: true,
});
function assertNoUnguardedAgentResultConsumer(parsed: ParsedModule, displayPath: string, options: UnguardedConsumerOptions): void {
  if (options.rejectFilterBooleanBeforeValidation || options.rejectFlattenCountOrCleanBeforeValidation) {
    const violations = unguardedAgentResultConsumers(parsed.sourceFile);
    if (violations.length > 0) throw new Error(`unguarded agent-result consumption in ${displayPath}:\n${violations.join("\n")}`);
  }
  if (options.requireTrustedOrderedIdentityEnvelopes) assertIdentityGuardCallSites(parsed.sourceFile, displayPath);
}

// ---- Canonical driver import/call contracts ----

interface CanonicalDriver { readonly lib: string; readonly symbol: string; }
const AUDIT_ROUNDS_DRIVER: CanonicalDriver = Object.freeze({ lib: "audit-rounds.mjs", symbol: "runCanonicalAuditRounds" });
const POST_AUDIT_DRIVER: CanonicalDriver = Object.freeze({ lib: "post-audit.mjs", symbol: "runCanonicalPostAudit" });
const hasDriverImportAndCall = (parsed: ParsedModule, driver: CanonicalDriver): boolean =>
  importedSymbolFromLib(parsed.sourceFile, driver.lib, driver.symbol) && callsOfIdentifier(parsed.sourceFile, driver.symbol).length > 0;
function assertDriverImportAndCall(parsed: ParsedModule, driver: CanonicalDriver, displayPath: string): void {
  if (!importedSymbolFromLib(parsed.sourceFile, driver.lib, driver.symbol)) {
    throw new Error(`${displayPath}: missing canonical driver import ${driver.symbol} from lib/${driver.lib}`);
  }
...
function agentCallNodes(sourceFile: ts.SourceFile): ts.CallExpression[] {
  return collectCalls(sourceFile).filter((call) => ts.isIdentifier(call.expression) && call.expression.text === "agent");
}
function everyAgentCallFeedsGuard(sourceFile: ts.SourceFile, displayPath: string): void {
  ...
  for (const agentCall of agentCallNodes(sourceFile)) {
    ...
    if (ts.isVariableDeclaration(node.parent) && ts.isIdentifier(node.parent.name)) {
      ...
      if (guardCalls.some((call) => call.arguments.some((argument) => ts.isIdentifier(argument) && argument.text === variableName))) continue;
    }
...
function assertLegacyExactIdentityGuardedDispatch(parsed: ParsedModule, displayPath: string): void {
  ...
  const guardsDefined = IDENTITY_GUARD_FUNCTION_NAMES.filter((name) => hasFunctionDeclaration(sourceFile, name));
  if (guardsDefined.length === 0 && collectCalls(sourceFile).filter(isIdentityGuardCall).length === 0) {
    throw new Error(`${displayPath}: no exact-identity result guard (requireAllResults/requirePass/requireCleanAudit/requireResult/normalizeAuditFindings)`);
  }
  everyAgentCallFeedsGuard(sourceFile, displayPath);
}
function assertCanonicalDriverImportAndCall(parsed: ParsedModule, role: WorkflowRole, displayPath: string): void {
  if (role === "author-audit") { assertDriverImportAndCall(parsed, AUDIT_ROUNDS_DRIVER, displayPath); return; }
...
const TASK_554_CLOSEOUT_GUARDS = Object.freeze([
  "captureTask554CloseoutSnapshot", "normalizeTask554CloseoutSnapshot",
  "assertTask554BoardClosureDelta", "assertTask554ChangelogClosureDelta",
  "assertTask554TerminalStatusDelta", "validateTask554MetadataCloseout", "validateTask554TerminalCloseout",
]);
function assertTask554CloseoutGuardContract(parsed: ParsedModule, displayPath: string): void {
  if (agentCallNodes(parsed.sourceFile).length > 0) throw new Error(`${displayPath}: closeout guard must never dispatch agents`);
  if (importedSymbolFromLib(parsed.sourceFile, "audit-rounds.mjs", "runCanonicalAuditRounds") ||
  ...
  for (const guard of TASK_554_CLOSEOUT_GUARDS) {
    if (!hasFunctionDeclaration(parsed.sourceFile, guard)) throw new Error(`${displayPath}: closeout guard missing ${guard}`);
  }
  if (!/function selfTest\(|selfTest\(\)/u.test(parsed.source)) throw new Error(`${displayPath}: closeout guard missing its self-test`);
  if (!/TASK_554_WORKFLOW_IMPORT/u.test(parsed.source)) throw new Error(`${displayPath}: closeout guard missing the direct-invocation guard`);
}
...
const FULL_AUDIT_DRIVER_OPTIONS: CanonicalAuditDriverOptions = Object.freeze({
  requireInitialCompletePass: true, reconcilePerPass: 1, affectedScopesOnlyAfterVerifiedFix: true,
  deriveActualChangedScopesFromPerScopeFingerprints: true, requireDeclaredActualIdentitySetEquality: true,
  invalidateAllReceiptsOnUnmappableChange: true, requireAllResults: true, requireExactOrderedIdentities: true,
...
function assertDriverLibRequiresAllResults(libBasename: string): void {
  ...
  if (!importedSymbolFromLib(parsed.sourceFile, "workflow-contracts.mjs", "requireAllResults")) {
    throw new Error(`lib/${libBasename}: missing requireAllResults import from workflow-contracts.mjs`);
  }
  ...
  for (const call of calls) {
    if (call.arguments.length !== 3) throw new Error(`lib/${libBasename}: requireAllResults must receive (results, expectedIdentities, label)`);
  }
  if (!/identity:/u.test(parsed.source)) throw new Error(`lib/${libBasename}: missing trusted ordered identity envelopes`);
}
function assertCanonicalAuditDriver(source: string, displayPath: string, options: CanonicalAuditDriverOptions): void {
  ...
  for (const call of calls) {
    ...
    if (options.deriveActualChangedScopesFromPerScopeFingerprints) required.push("fingerprintEveryScope");
    if (options.requireDeclaredActualIdentitySetEquality || options.invalidateAllReceiptsOnUnmappableChange) required.push("fingerprintUniverse", "maximumFixPasses");
    if (options.requireRevisionFingerprint) required.push("fingerprint");
    for (const key of required) if (!keys.has(key)) throw new Error(`${displayPath}: runCanonicalAuditRounds missing required option ${key}`);
    const reconcile = propertyValueOf(call, "reconcile");
    if (!reconcile || !isFunctionLike(reconcile)) throw new Error(`${displayPath}: runCanonicalAuditRounds reconcile must be a single function (one per pass)`);
    if (options.affectedScopesOnlyAfterVerifiedFix) {
      const fix = propertyValueOf(call, "fix");
      if (!fix || !isFunctionLike(fix)) throw new Error(`${displayPath}: runCanonicalAuditRounds fix must be a function`);
    }
  }
  if (options.requireInitialCompletePass && calls.length !== 1) throw new Error(`${displayPath}: expected exactly one complete audit-round pass, found ${calls.length}`);
  if (options.requireAllResults || options.requireExactOrderedIdentities) assertDriverLibRequiresAllResults("audit-rounds.mjs");
}
...
const FULL_POST_AUDIT_DRIVER_OPTIONS: CanonicalPostAuditDriverOptions = Object.freeze({
  requireDeclaredIndependentLensIds: true, requireAllResults: true, requireExactOrderedIdentities: true,
  fingerprintBeforeAndAfterPass: true, fingerprintFixAndValidation: true,
  deriveActualChangedLensesFromInputFingerprints: true, requireDeclaredActualIdentitySetEquality: true,
  ...
  for (const statement of sourceFile.statements) {
    ...
    for (const declaration of statement.declarationList.declarations) {
      ...
      const isObjectFreeze =
        (ts.isIdentifier(initializer.expression) && initializer.expression.text === "Object.freeze") ||
        (ts.isPropertyAccessExpression(initializer.expression) && ts.isIdentifier(initializer.expression.expression) &&
          initializer.expression.expression.text === "Object" && initializer.expression.name.text === "freeze");
      if (!isObjectFreeze || !ts.isArrayLiteralExpression(initializer.arguments[0])) continue;
      const ids = initializer.arguments[0].elements.map((element) => (ts.isStringLiteral(element) ? element.text : null));
      if (ids.length > 0 && ids.every((id) => id !== null)) return ids as string[];
    }
  }
  return null;
}
function assertLegacyPostAuditContract(parsed: ParsedModule, displayPath: string, options: CanonicalPostAuditDriverOptions): void {
  const sourceFile = parsed.sourceFile;
  if (options.requireDeclaredIndependentLensIds) {
    const lensIds = declaredLensIds(sourceFile);
    if (!lensIds || lensIds.length === 0 || new Set(lensIds).size !== lensIds.length) throw new Error(`${displayPath}: legacy post-audit must declare independent lens ids`);
  }
  if (options.requireAllResults || options.requireExactOrderedIdentities) {
    if (IDENTITY_GUARD_FUNCTION_NAMES.filter((name) => hasFunctionDeclaration(sourceFile, name)).length === 0) {
      throw new Error(`${displayPath}: legacy post-audit must define an exact-identity result guard`);
    }
    for (const call of collectCalls(sourceFile).filter(isIdentityGuardCall)) {
      if (call.arguments.length < 2) throw new Error(`${displayPath}: identity guard call missing its identity argument`);
    }
  }
  if (options.fingerprintBeforeAndAfterPass || options.fingerprintFixAndValidation) {
    if (!/Fingerprint|fingerprint/u.test(parsed.source)) throw new Error(`${displayPath}: legacy post-audit must fingerprint the audited state`);
    if (!/(assertScopedRepositoryMutation|assertNoRepositoryMutation|assertFixScope|sameFingerprint)/u.test(parsed.source)) {
      throw new Error(`${displayPath}: legacy post-audit must validate repository mutation against fingerprints`);
    }
  }
  if (options.deriveActualChangedLensesFromInputFingerprints &&
    !/ownersForChangedPaths|deriveChanged|assertScopedRepositoryMutation|assertFixScope/u.test(parsed.source)) {
    throw new Error(`${displayPath}: legacy post-audit must derive actual changed scopes from fingerprints`);
  }
  if ((options.requireDeclaredActualIdentitySetEquality || options.invalidateAllReceiptsOnUnmappableChange) &&
    !/receipt_stale|sameFingerprint\(|assertResumePreflight|assertImplementationPreflight/u.test(parsed.source)) {
    throw new Error(`${displayPath}: legacy post-audit must invalidate receipts on unmappable change`);
  }
}
function assertCanonicalPostAuditDriver(source: string, displayPath: string, options: CanonicalPostAuditDriverOptions): void {
  const parsed = parseModuleSource(source, displayPath);
  ...
  if (driverCalls.length > 0) {
    for (const call of driverCalls) {
      ...
      if (options.fingerprintFixAndValidation || options.deriveActualChangedLensesFromInputFingerprints) required.push("fingerprintEveryLensInput");
      if (options.requireDeclaredActualIdentitySetEquality || options.invalidateAllReceiptsOnUnmappableChange || options.fingerprintFixAndValidation) {
        required.push("fingerprintUniverse", "maximumFixPasses");
      }
      for (const key of required) if (!keys.has(key)) throw new Error(`${displayPath}: runCanonicalPostAudit missing required option ${key}`);
    }
    if (options.requireAllResults || options.requireExactOrderedIdentities) assertDriverLibRequiresAllResults("post-audit.mjs");
    return;
  }
...
function assertPromptRequiresImmediateEvidenceValidation(source: string, file: string): void {
  if (!/(validateSmoke\s*\(|assertExactTask554SmokeEvidence|sameFingerprint\s*\(|normalizeAuditFindings\s*\()/u.test(source)) {
    throw new Error(`${file}: closure prompts must validate evidence immediately`);
  }
}
function assertPromptReturnsOwnerActionRequiredBeforeTrackedAudit(source: string, file: string): void {
  if (!/(ownerActionRequired|createTask543ResumeCheckpoint|openTask543ClosureResume)/u.test(source)) {
...
function assertPromptUsesCanonicalResumeCheckpoint(source: string, file: string): void {
  if (!/(createResumeCheckpoint|openWorkflowClosureResume|openTask543ClosureResume|RESUME_AFTER_FIX_ARG|resume_full_validation_post_audit_smoke)/u.test(source)) {
...
function assertPromptReturnsExactOwningWorkflowResumeArgv(source: string, file: string): void {
  if (!/(checkpointPath|checkpointSha256|runId|RESUME_AFTER_FIX_ARG|resume_full_validation_post_audit_smoke)/u.test(source)) {
...
function assertResumeEntryMatchesExecutingWorkflow(source: string, file: string): void {
  if (!/import\.meta\.url/u.test(source)) throw new Error(`${file}: resume identity must derive from the executing import.meta.url`);
}
function assertPromptResumesWithRequireTracked(source: string, file: string): void {
  if (!/(checkpointSha256|assertResumePreflight|assertImplementationPreflight|resume_full_validation_post_audit_smoke)/u.test(source)) {
...
function assertPromptValidatesMetadataOnlyClosureDelta(source: string, file: string): void {
  if (!/(finalMetadataGatePrompt|assertTask554BoardClosureDelta|assertTask554ChangelogClosureDelta|assertTask554TerminalStatusDelta|validateTask554MetadataCloseout|terminal_phase_receipt_required)/u.test(source)) {
    throw new Error(`${file}: closure must validate a metadata-only delta against the frozen snapshot`);
  }
}
function functionDeclarationBodyContains(
  ...
  const check = (node: ts.Node): void => {
    ...
    if (ts.isStringLiteral(node) && stringLiterals.includes(node.text)) { hit = true; return; }
...
function assertResumeCannotDispatchImplementationStages(source: string, file: string): void {
  ...
  if (functionDeclarationBodyContains(parsed.sourceFile, "runResumeAfterFixWorkflow", ["runOwner"], ["Sequential owners"])) {
...
function assertPromptNeverStagesAsAgent(source: string, file: string): void {
  if (/\bgit\s+(?:add|commit|push)\b/iu.test(source) || /\bstage\s+(?:the|your|all|these)\s+(?:files|changes)\b/iu.test(source) ||
    /\b(?:then|now)\s+stage\b/iu.test(source)) {
    throw new Error(`${file}: a prompt must never instruct an agent to stage or commit`);
  }
  if (!/Never\s+(?:stage|commit|push)|stage, or commit|do not\s+(?:stage|commit|push)/iu.test(source)) {
...
function assertNoForbiddenPatterns(source: string, displayPath: string, patterns: ReadonlyArray<RegExp>): void {
  for (const pattern of patterns) {
    const match = pattern.exec(source);
    if (match) throw new Error(`${displayPath}: forbidden action directive matched ${pattern}: ${JSON.stringify(match[0])}`);
  }
}

// ---- Owning workflow entries: tracked, clean, task-bound, import.meta.url-only ----

interface OwningWorkflowRegistration { readonly importMetaUrl: string; readonly taskId: string; readonly role: WorkflowRole; readonly path: string; }
...
function deriveOnlyFromExecutingImportMetaUrl(importMetaUrl: string): string {
  if (!importMetaUrl.startsWith("file:")) throw new Error(`workflow identity must derive from a file: import.meta.url: ${importMetaUrl}`);
  const relativePath = path.relative(ROOT, fileURLToPath(importMetaUrl)).split(path.sep).join("/");
  if (relativePath.startsWith("../") || path.posix.isAbsolute(relativePath) || !relativePath.startsWith("_docs/_workflows/")) {
...
function assertExactBuiltinTaskAndRoleBinding(entry: string, registeredTaskId: string): void {
  ...
  if (taskId !== registeredTaskId) throw new Error(`TASK-545 builtin task binding mismatch for ${entry}: registered ${registeredTaskId}`);
}
function assertCanonicalFutureEntry(
  entry: string, registeredTaskId: string,
  options: { readonly pattern: RegExp; readonly requireTaskIdAndSuffixBinding: boolean; readonly role: WorkflowRole }
): void {
  if (!options.pattern.test(entry)) throw new Error(`entry does not match the canonical future workflow pattern: ${entry}`);
  ...
  if (taskId !== registeredTaskId) throw new Error(`future entry task binding mismatch for ${entry}: registered ${registeredTaskId}`);
  if (basename === "task-554-closeout.mjs") {
    if (registeredTaskId !== "554") throw new Error(`future closeout entry must bind task 554: ${entry}`);
    return;
  }
  ...
  if (options.role !== "closeout" && suffix !== options.role) throw new Error(`future entry suffix/role binding mismatch for ${entry}: role ${options.role}`);
}
function assertNoCallerWorkflowEntryOverride(owner: OwningWorkflowRegistration, derivedEntry: string): void {
  if (owner.path !== derivedEntry) {
    throw new Error(`caller workflow entry override: registered path ${owner.path} differs from the import.meta.url-derived ${derivedEntry}`);
  }
  const source = readFileSync(path.join(ROOT, derivedEntry), "utf8");
  if (/(ownerActionRequired|createResumeCheckpoint|openWorkflowClosureResume|RESUME_AFTER_FIX_ARG)/u.test(source) && !/import\.meta\.url/u.test(source)) {
    throw new Error(`${derivedEntry}: a resume-capable workflow must derive its identity from import.meta.url`);
  }
}
function assertCanonicalTask545StaticContractsAndImports(entry: string, role: WorkflowRole, root: string = ROOT): void {
  ...
  if (role === "closeout") {
    ...
    if (role === "author-audit") assertCanonicalAuditDriver(parsed.source, entry, FULL_AUDIT_DRIVER_OPTIONS);
...
function createTrackedWorkflowRepo(): string {
  ...
  execFileSync("git", ["config", "user.email", "task-545-static@example.invalid"], { cwd: tempRoot, stdio: "ignore" });
  execFileSync("git", ["config", "user.name", "TASK-545 static gate"], { cwd: tempRoot, stdio: "ignore" });
  ...
  execFileSync("git", ["commit", "-qm", "tracked workflow corpus"], { cwd: tempRoot, stdio: "ignore" });
  return tempRoot;
}
...
describe("TASK-545-01-L02 static workflow contract", () => {
  ...
  test("initial migration entries and all future owners are tracked", () => {
    expectTrackedInitialEntriesOrCanonicalExtensions(trackedWorkflowEntries(), INITIAL_MIGRATION_SET);
  });
  ...
  test("canonical author/audit workflows run one complete pass and affected reruns", () => {
    for (const file of trackedWorkflowEntriesByRole("author-audit")) {
      assertCanonicalAuditDriver(readFileSync(path.join(ROOT, file), "utf8"), file, FULL_AUDIT_DRIVER_OPTIONS);
    }
  });
  ...
  test("implementation workflows declare and complete their independent post lenses", () => {
    for (const file of trackedWorkflowEntriesByRole("implement", "fix")) {
      assertCanonicalPostAuditDriver(readFileSync(path.join(ROOT, file), "utf8"), file, FULL_POST_AUDIT_DRIVER_OPTIONS);
    }
  });
    ...
    for (const owner of trackedOwningWorkflowRegistrations()) {
      ...
      if (isExactTask545BuiltinEntry(entry)) {
        ...
        assertCanonicalFutureEntry(entry, owner.taskId, {
          pattern: CANONICAL_FUTURE_PATH_PATTERN, requireTaskIdAndSuffixBinding: true, role: owner.role,
        });
      }
  ...
  test("tracked prompts do not commit, allocate dynamically, or defer smoke", () => {
    for (const file of trackedWorkflowFiles()) {
      assertNoForbiddenPatterns(readFileSync(path.join(ROOT, file), "utf8"), file, FORBIDDEN_ACTION_PATTERNS);
    }
  });
  ...
  test("negative source fixtures fail exactly where the static contract rejects them", () => {
    ...
    for (const [filename, syntheticPath, role] of allowed) {
      ...
      if (role === "author-audit") assertCanonicalAuditDriver(parsed.source, syntheticPath, FULL_AUDIT_DRIVER_OPTIONS);
      else assertCanonicalPostAuditDriver(parsed.source, syntheticPath, FULL_POST_AUDIT_DRIVER_OPTIONS);
    }
    const domainParsed = parseFixture("legal-domain-filter-boolean.mjs", "_docs/_workflows/task-777-implement.mjs");
    expect(() =>
    ...
    expect(() =>
      assertNoUnguardedAgentResultConsumer(
        parseFixture("unguarded-result-filter-false-clean.mjs", "_docs/_workflows/task-777-implement.mjs"),
        "unguarded-result-filter-false-clean.mjs", FULL_UNGUARDED_CONSUMER_OPTIONS
      )
    ).toThrow(/filter\(Boolean\)/);
    expect(() =>
      assertNoUnguardedAgentResultConsumer(
        parseFixture("flatten-count-clean-before-validation.mjs", "_docs/_workflows/task-777-implement.mjs"),
        "flatten-count-clean-before-validation.mjs", FULL_UNGUARDED_CONSUMER_OPTIONS
      )
    ).toThrow(/flatMap|counts a filtered/);
    expect(() =>
      assertCanonicalDriverImportAndCall(
        parseFixture("comment-only-static-contract-fake.mjs", "_docs/_workflows/task-777-author-audit.mjs"),
        "author-audit", "comment-only-static-contract-fake.mjs"
      )
    ).toThrow(/missing canonical driver/);
    expect(() =>
      assertCanonicalDriverImportAndCall(
        parseFixture("missing-task545-imports.mjs", "_docs/_workflows/task-777-implement.mjs"),
        "implement", "missing-task545-imports.mjs"
      )
    ).toThrow(/no exact-identity result guard/);
    expect(() =>
      assertNoForbiddenPatterns(
        readFileSync(path.join(fixtures, "forbidden-prompt-actions.mjs"), "utf8"),
        "forbidden-prompt-actions.mjs", FORBIDDEN_ACTION_PATTERNS
      )
    ).toThrow(/forbidden action directive/);
    expect(() => assertCanonicalFutureEntry("_docs/_workflows/task-556-implement.mjs", "555", {
      pattern: CANONICAL_FUTURE_PATH_PATTERN, requireTaskIdAndSuffixBinding: true, role: "implement",
    })).toThrow(/task binding mismatch/);
    expect(() => assertCanonicalFutureEntry("_docs/_workflows/task-555-fix.mjs", "555", {
      pattern: CANONICAL_FUTURE_PATH_PATTERN, requireTaskIdAndSuffixBinding: true, role: "implement",
    })).toThrow(/suffix\/role binding mismatch/);
    expect(() => classifyTrackedEntryOrThrow("_docs/_workflows/task-1234-implement.mjs")).toThrow(/neither an initial TASK-545 entry nor a canonical future entry/);
    expect(() => classifyTrackedEntryOrThrow("_docs/_workflows/task-555-migrate.mjs")).toThrow(/neither an initial TASK-545 entry nor a canonical future entry/);
    const overrideOwner: OwningWorkflowRegistration = {
      ...
      taskId: "543", role: "implement", path: "_docs/_workflows/task-554-implement.mjs",
    };
    expect(() =>
      assertNoCallerWorkflowEntryOverride(overrideOwner, deriveOnlyFromExecutingImportMetaUrl(overrideOwner.importMetaUrl))
  ...
  test("ignored local files cannot alter the tracked inventory and a missing canonical driver call is rejected", () => {
    ...
    try {
      ...
      writeFileSync(path.join(tempRoot, "_docs/_workflows/task-999-local-only.mjs"), "export const x = 1;\n");
      writeFileSync(path.join(tempRoot, "_docs/_workflows/lib/ignored-local-helper.mjs"), "export const x = 1;\n");
      expect(trackedWorkflowFiles(tempRoot)).toEqual(trackedWorkflowFiles(ROOT));
      expect(() => assertTrackedRegularFileNoSymlink("_docs/_workflows/lib/ignored-local-helper.mjs", tempRoot)).toThrow(/not tracked by git ls-files/);
      ...
      expect(() => assertBytesEqualGitShowHead("_docs/_workflows/task-522-author.mjs", tempRoot)).toThrow(/bytes differ|unstaged changes/);
      execFileSync("git", ["checkout", "--", "_docs/_workflows/task-522-author.mjs"], { cwd: tempRoot, stdio: "ignore" });
      rmSync(path.join(tempRoot, "_docs/_workflows/task-554-closeout.mjs"));
      symlinkSync("task-554-fix.mjs", path.join(tempRoot, "_docs/_workflows/task-554-closeout.mjs"));
      execFileSync("git", ["add", "-f", "_docs/_workflows/task-554-closeout.mjs"], { cwd: tempRoot, stdio: "ignore" });
      execFileSync("git", ["commit", "-qm", "symlink closeout"], { cwd: tempRoot, stdio: "ignore" });
      expect(() => assertTrackedRegularFileNoSymlink("_docs/_workflows/task-554-closeout.mjs", tempRoot)).toThrow(/not a regular non-symlink file/);
      rmSync(path.join(tempRoot, "_docs/_workflows/task-554-closeout.mjs"));
      execFileSync("git", ["checkout", "HEAD^", "--", "_docs/_workflows/task-554-closeout.mjs"], { cwd: tempRoot, stdio: "ignore" });
      const missingDriver = readFileSync(path.join(fixturePath, "missing-task545-imports.mjs"), "utf8");
      ...
      execFileSync("git", ["commit", "-qm", "add missing-driver future entry"], { cwd: tempRoot, stdio: "ignore" });
      const future = trackedWorkflowEntries(tempRoot).find((entry) => entry.path === futurePath);
      expect(future).toBeDefined();
      expect(() => assertCanonicalTask545StaticContractsAndImports(future!.path, future!.role, tempRoot)).toThrow(/no exact-identity result guard/);
    } finally {
