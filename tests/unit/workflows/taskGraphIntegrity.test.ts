...
const STATUS_RE = /^\*\*Status:\*\*\s*(?:(⏳|✅|🚧|⏭️|❌)\s*\*{0,2}\s*)?(?<raw>To Do|In Progress|Done|Superseded|Cancelled)/mu;
...
function parseTaskFile(file: string): TaskFile {
  ...
  if (fileName !== path.basename(file)) throw new Error(`${file}: FileName ${JSON.stringify(fileName)} != basename ${path.basename(file)}`);
...
describe("task graph integrity", () => {
  ...
  test("closed parents have no open physical descendants (by Parent field)", () => {
    ...
    for (const task of tasks) {
      ...
      for (const child of tasks) {
        if (child.parent === task.id && child.status !== "done" && child.status !== "superseded" && child.status !== "cancelled") {
  ...
  test("TASK-533 board changelog resolves to 1247 and TASK-511 stays To Do", () => {
    ...
    const task511 = gitLsTaskFiles().map(parseTaskFile).find((t) => t.id === "TASK-511");
