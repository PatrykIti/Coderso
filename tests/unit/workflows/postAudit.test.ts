...
import { WorkflowResultError } from "../../../_docs/_workflows/lib/workflow-contracts.mjs";
import type {
...
interface HarnessOptions {
  ...
  fix?: (blocking: readonly WorkflowStructuredFinding[], pass: number, harness: Harness) => unknown;
...
function makeHarness(options: HarnessOptions = {}): Harness {
  ...
  harness.run = (overrides: Partial<CanonicalPostAuditOptions> = {}) =>
    runCanonicalPostAudit({
      ...
      fingerprintUniverse: (all: readonly CanonicalPostAuditLens[]) =>
        repo.fingerprintUniverse(all),
...
describe("lens identity validation", () => {
  test("a single lens is a valid non-empty exact identity set", async () => {
    ...
    expect(harness.runLensCalls).toEqual([
      { key: "scope-fidelity", label: "Scope fidelity lens", pass: 0 },
    ]);
  ...
  test("duplicate or unsafe lens keys reject", async () => {
    ...
    const missing = makeHarness({
      lenses: [{ label: "no key" } as unknown as CanonicalPostAuditLens],
    });
    await expectWorkflowError(
      () => missing.run(),
      "workflow_lenses_invalid",
...
describe("exported driver helpers", () => {
  test("requireNonEmptyAffectedLensSubset validates and normalizes the declared claim", () => {
    ...
    expect(
      requireNonEmptyAffectedLensSubset(
        { affectedLensKeys: ["test-integrity", "scope-fidelity"] },
        lenses,
        "t"
      )
    ).toEqual(["scope-fidelity", "test-integrity"]);
    expect(() => requireNonEmptyAffectedLensSubset({ affectedLensKeys: [] }, lenses, "t")).toThrow(
      "workflow_post_declared_scope_invalid:t:empty"
    );
    expect(() =>
    ...
    expect(() =>
      requireNonEmptyAffectedLensSubset(
        { affectedLensKeys: ["scope-fidelity", "scope-fidelity"] },
        lenses,
        "t"
      )
