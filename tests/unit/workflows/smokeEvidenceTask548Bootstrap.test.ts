...
const CONTENT: Record<string, string> = {
  ...
  "_docs/_workflows/task-548-author-audit.mjs":
    "export async function authorAudit() { return true; }\n",
...
async function loadFixture(): Promise<Task548CommittedSixPathBootstrapReceiptV1> {
  if (fixture === undefined) {
    fixture = JSON.parse(
      await readFile(join(FIXTURES, "receipt-v1.json"), "utf8")
    ) as Task548CommittedSixPathBootstrapReceiptV1;
  }
  return fixture;
}
...
async function commitBootstrap(
  ...
  priorHead: string;
  head: string;
  receipt: Task548CommittedSixPathBootstrapReceiptV1;
}> {
...
describe("normalizeTask548CommittedSixPathBootstrapReceiptV1", () => {
  test("round-trips the committed-bootstrap receipt fixture", async () => {
    ...
    const normalized = normalizeTask548CommittedSixPathBootstrapReceiptV1(
  ...
  test("rejects unknown or missing root fields and non-record input", async () => {
    ...
    expect(errorCode(() => normalizeTask548CommittedSixPathBootstrapReceiptV1(withExtra))).toBe(
      "smoke_schema_invalid"
    );
    const missing = JSON.parse(JSON.stringify(base)) as Record<string, unknown>;
    delete missing.files;
    expect(errorCode(() => normalizeTask548CommittedSixPathBootstrapReceiptV1(missing))).toBe(
      "smoke_schema_invalid"
    );
    expect(
      errorCode(() => normalizeTask548CommittedSixPathBootstrapReceiptV1("not a record"))
    ).toBe("smoke_schema_invalid");
    expect(errorCode(() => normalizeTask548CommittedSixPathBootstrapReceiptV1(null))).toBe(
      "smoke_schema_invalid"
    );
  });
  ...
  test("rejects every root key mutation", async () => {
    ...
    for (const [key, mutate, code] of cases) {
      ...
      expect(
        errorCode(() => normalizeTask548CommittedSixPathBootstrapReceiptV1(copy)),
        `root key ${key}`
      ).toBe(code);
    }
  });
  ...
  test("rejects every nested files mutation", async () => {
    ...
    const cases: Array<[string, (value: Task548CommittedBootstrapFileV1[]) => unknown, string]> = [
      ...
      [
        "entry not a record",
        (value) => [value[0], "x", value[2], value[3], value[4], value[5]],
        "smoke_schema_invalid",
      ],
      [
        "entry unknown key",
        (value) => [value[0], { ...value[1], extra: 1 }, value[2], value[3], value[4], value[5]],
        "smoke_schema_invalid",
      ],
      [
        "entry missing key",
        (value) => [value[0], { path: value[1].path }, value[2], value[3], value[4], value[5]],
        "smoke_schema_invalid",
      ],
      [
        "hash grammar",
        (value) => [
          value[0],
          { ...value[1], sha256: "0".repeat(63) },
          value[2],
          value[3],
          value[4],
          value[5],
        ],
        "smoke_task548_hash_invalid",
      ],
      [
        "reordered",
        (value) => [value[1], value[0], value[2], value[3], value[4], value[5]],
        "smoke_task548_path_invalid",
      ],
      [
        "substituted path",
        (value) => [
          value[0],
          { ...value[1], path: CLOSEOUT_ENTRY },
          value[2],
          value[3],
          value[4],
          value[5],
      ...
      [
        "duplicate path",
        (value) => [value[0], value[0], value[2], value[3], value[4], value[5]],
        "smoke_task548_path_invalid",
      ],
      [
        "valid-hash mutation",
        (value) => [
          value[0],
          { ...value[1], sha256: sha("other content") },
          value[2],
          value[3],
          value[4],
          value[5],
        ],
        "smoke_task548_aggregate_mismatch",
      ],
    ];
  ...
  test("a grammar-valid priorHead mutation breaks the recomputed aggregate", async () => {
    ...
    expect(errorCode(() => normalizeTask548CommittedSixPathBootstrapReceiptV1(copy))).toBe(
      "smoke_task548_aggregate_mismatch"
    );
  });
  ...
  test("receipt type is a strict literal contract", async () => {
    ...
    void first;
    const badSchema: Task548CommittedSixPathBootstrapReceiptV1 = {
      ...base,
      // @ts-expect-error schema is the literal v1 value
      schema: "coderso.task548-committed-bootstrap@v2",
    };
    void badSchema;
    ...
    void badTask;
    const badEntry: Task548CommittedSixPathBootstrapReceiptV1 = {
      ...base,
      // @ts-expect-error workflowEntry is the literal implement path
      workflowEntry: CLOSEOUT_ENTRY,
    };
    void badEntry;
  });
});
...
describe("requireTask548CommittedSixPathBootstrapAuthorizationV1", () => {
  ...
  test("rejects a missing receipt option and a non-record receipt", async () => {
    ...
    expect(await errorCodeAsync(() => authorize(root, "not a receipt"))).toBe(
      "smoke_schema_invalid"
    );
  ...
  test("rejects when priorHead is not the single direct parent", async () => {
    ...
    expect(await errorCodeAsync(() => authorize(root, bogus))).toBe(
      "smoke_task548_parent_mismatch"
    );
  });
  ...
  test("rejects when the committed diff is not exactly the six paths", async () => {
    ...
    expect(await errorCodeAsync(() => authorize(root, receipt))).toBe(
      "smoke_task548_diff_mismatch"
    );
  });
  ...
  test("rejects a workflow entry that fails the static/import gates", async () => {
    ...
    const { receipt: badImportReceipt } = await commitBootstrap(root, {
      implementSource: noImport,
    });
    expect(await errorCodeAsync(() => authorize(root, badImportReceipt))).toBe(
      "smoke_task548_static_contract"
    );
    ...
    expect(await errorCodeAsync(() => authorize(root2, stagedReceipt))).toBe(
      "smoke_task548_static_contract"
    );
  });
  ...
  test("rejects a wrong-entry receipt", async () => {
    ...
    expect(await errorCodeAsync(() => authorize(root, wrongEntry))).toBe(
      "smoke_task548_schema_invalid"
    );
  });
  ...
  test("rejects reordered six paths", async () => {
    ...
    expect(await errorCodeAsync(() => authorize(root, reordered))).toBe(
      "smoke_task548_path_invalid"
    );
  });
  ...
  test("rejects an intervening action between the gate and the phase-1 call", async () => {
    ...
    expect(await errorCodeAsync(() => authorize(root2, verified2))).toBe(
      "smoke_task548_head_stale"
    );
  });
  ...
  test("rejects passing the receipt into createResumeCheckpoint", async () => {
    const root = await makeRepo();
    const { receipt } = await commitBootstrap(root);
    expect(await errorCodeAsync(() => createResumeCheckpoint(receipt as never))).toBe(
      "smoke_schema_invalid"
    );
    const verified = await authorize(root, receipt);
    expect(await errorCodeAsync(() => createResumeCheckpoint(verified as never))).toBe(
      "smoke_schema_invalid"
    );
  });
});
