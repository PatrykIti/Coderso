...
export class Task490PrivateWorkspace implements LifecycleResource {
  readonly name: string;
  readonly path: string;
  #closed = false;

  private constructor(session: string, path: string) {
    this.name = `task490-workspace-${session}`;
    this.path = path;
  }

  static async create(context: RuntimeSmokeContext): Promise<Task490PrivateWorkspace> {
    context.lifecycle.assertAccepting();
    const root = await realpath(context.root).catch((error: unknown) => {
      throw new SmokeError("smoke_repository_invalid", "TASK-490 root is unavailable", {
        cause: error,
      });
    });
    const parentInput = resolveInsideRoot(root, ".tmp/runtime-smoke", "TASK-490 workspace root");
    ...
    if (parent !== parentInput || !isWithin(root, parent) || !(await stat(parent)).isDirectory()) {
      throw new SmokeError("smoke_repository_invalid", "TASK-490 workspace parent is redirected");
    }

    let candidate: string | null = null;
    try {
      candidate = await mkdtemp(join(parent, `${context.input.session}-task490-`));
      ...
      if (
        ...
        throw new SmokeError(
          ...
          "TASK-490 private workspace ownership is invalid"
        );
      }
      const workspace = new Task490PrivateWorkspace(context.input.session, canonical);
...
export function createTask490PrivateWorkspace(
  context: RuntimeSmokeContext
): Promise<Task490PrivateWorkspace> {
  return Task490PrivateWorkspace.create(context);
}
