...
import { TASK493_SCENARIO_IDS } from "./browser-actions";

const PNG_SIGNATURE = Buffer.from("89504e470d0a1a0a", "hex");
const MAX_PNG_BYTES = 16 * 1024 * 1024;
const SHA256 = /^[a-f0-9]{64}$/u;
export const EVIDENCE_ROOT = "_docs/_workflows/_smoke/evidence/task-493";
const REPORT_FILE = "report.json";

export interface Task493ScreenshotManifestEntry {
  readonly scenarioId: (typeof TASK493_SCENARIO_IDS)[number];
  readonly path: string;
}

export interface Task493ScreenshotManifest {
  readonly entries: readonly Task493ScreenshotManifestEntry[];
...
function sessionFor(profile: "fast" | "certification"): string {
  return `task-493-${profile}`;
}

export function assertExactTask493Invocation(value: unknown): asserts value is SmokeInput {
  if (!isPlainObject(value)) invalid("TASK-493 invocation is invalid", "smoke_argument_invalid");
  assertExactKeys(value, ["command", "suite", "profile", "session"], "TASK-493 invocation");
  if (
    value.command !== "run" ||
    value.suite !== "task-493" ||
    (value.profile !== "fast" && value.profile !== "certification") ||
    value.session !== sessionFor(value.profile)
  ) {
    invalid("TASK-493 invocation is invalid", "smoke_argument_invalid");
  }
}
...
export function buildExactTask493ScreenshotManifest(input: SmokeInput): Task493ScreenshotManifest {
  assertExactTask493Invocation(input);
  const entries = TASK493_SCENARIO_IDS.map((scenarioId, index) =>
...
export function assertExactTask493ScreenshotManifest(
  input: SmokeInput,
  manifest: Task493ScreenshotManifest
): void {
  assertExactTask493Invocation(input);
  if (
    ...
    manifest.entries.length !== TASK493_SCENARIO_IDS.length ||
    manifest.paths.length !== TASK493_SCENARIO_IDS.length ||
    new Set(manifest.paths).size !== TASK493_SCENARIO_IDS.length
  ) {
    invalid("TASK-493 screenshot manifest cardinality is invalid");
  }
  for (const [index, scenarioId] of TASK493_SCENARIO_IDS.entries()) {
    ...
    if (
      ...
      invalid("TASK-493 screenshot manifest row drifted");
    }
  }
}
...
function channels(colorType: number): number {
  ...
  return invalid("TASK-493 PNG color type is invalid");
}
...
export function decodeTask493Png(
  ...
  if (!image.subarray(0, PNG_SIGNATURE.length).equals(PNG_SIGNATURE))
    invalid("TASK-493 PNG signature is invalid");
  ...
  while (offset < image.byteLength) {
    if (offset + 12 > image.byteLength) invalid("TASK-493 PNG chunk is truncated");
    ...
    if (length > MAX_PNG_BYTES || next > image.byteLength)
      invalid("TASK-493 PNG chunk bounds are invalid");
    ...
    if (
      ...
      invalid("TASK-493 PNG chunk checksum is invalid");
    }
    if (type === "IHDR") {
      if (header || offset !== PNG_SIGNATURE.length || length !== 13)
        invalid("TASK-493 PNG header is invalid");
      ...
      if (
        ...
        invalid("TASK-493 PNG header fields are invalid");
      header = true;
    } else if (type === "PLTE") {
      if (!header || data || length === 0 || length % 3 !== 0 || length > 768)
        invalid("TASK-493 PNG palette is invalid");
      palette = true;
    } else if (type === "IDAT") {
      if (!header || ended || length === 0 || (colorType === 3 && !palette))
        invalid("TASK-493 PNG data is invalid");
      data = true;
      compressed.push(content);
    } else if (type === "IEND") {
      if (!header || !data || ended || length !== 0 || next !== image.byteLength)
        invalid("TASK-493 PNG end marker is invalid");
      ended = true;
    } else if ((type.charCodeAt(0) & 0x20) === 0) {
      invalid("TASK-493 PNG critical chunk is unknown");
    }
    offset = next;
  }
  if (!header || !data || !ended) invalid("TASK-493 PNG is incomplete");
  let inflated: Buffer;
  try {
    ...
    throw new SmokeError("smoke_output_invalid", "TASK-493 PNG image data cannot be decoded", {
      cause: error,
    });
  }
  const rowBytes = Math.ceil((width * channels(colorType) * bitDepth) / 8);
  if (inflated.byteLength !== (rowBytes + 1) * height)
    invalid("TASK-493 PNG decoded bytes are invalid");
  for (let row = 0; row < height; row += 1) {
    if ((inflated[row * (rowBytes + 1)] ?? 255) > 4) invalid("TASK-493 PNG filter is invalid");
  }
...
async function assertNoFollowDirectories(root: string, session: string): Promise<string> {
  const canonicalRoot = await realpath(root).catch((error: unknown) => {
    throw new SmokeError("smoke_output_invalid", "TASK-493 repository root is unavailable", {
      cause: error,
    });
  });
  ...
  for (const component of [...EVIDENCE_ROOT.split("/"), session]) {
    ...
    if (!inside(canonicalRoot, current)) invalid("TASK-493 evidence escapes repository root");
    const metadata = await lstat(current).catch((error: unknown) => {
      throw new SmokeError("smoke_output_invalid", "TASK-493 evidence directory is unavailable", {
        cause: error,
      });
    });
    if (metadata.isSymbolicLink() || !metadata.isDirectory())
      invalid("TASK-493 evidence directory is invalid");
  }
  return current;
}
...
async function readExactPng(root: string, path: string): Promise<Buffer> {
  ...
  if (!inside(root, candidate)) invalid("TASK-493 evidence path escapes repository root");
  ...
  try {
    ...
    if (
      ...
      invalid("TASK-493 PNG ownership is invalid");
    }
    ...
    if (
      ...
      invalid("TASK-493 PNG changed while reading");
    return bytes;
  } catch (error) {
    if (error instanceof SmokeError) throw error;
    throw new SmokeError("smoke_output_invalid", "TASK-493 PNG is unavailable", { cause: error });
  } finally {
    await handle?.close();
  }
}
...
async function assertPrivateReportFile(directory: string): Promise<void> {
  const report = await lstat(resolve(directory, REPORT_FILE)).catch((error: unknown) => {
    throw new SmokeError("smoke_output_invalid", "TASK-493 report receipt is unavailable", {
      cause: error,
    });
  });
  if (report.isSymbolicLink() || !report.isFile() || report.nlink !== 1) {
    invalid("TASK-493 report receipt is invalid");
  }
}

export async function validateTask493ScreenshotOutputs(
  root: string,
  input: SmokeInput,
  manifest: Task493ScreenshotManifest
  ...
  if (
    ...
    invalid("TASK-493 screenshot evidence set is invalid");
  }
  ...
  for (const entry of manifest.entries) {
    ...
    decodeTask493Png(bytes);
    const sha256 = createHash("sha256").update(bytes).digest("hex");
    if (!SHA256.test(sha256)) invalid("TASK-493 PNG hash is invalid");
    results.push(Object.freeze({ path: entry.path, sha256 }));
  }
  if (results.length !== TASK493_SCENARIO_IDS.length)
    invalid("TASK-493 screenshot output cardinality is invalid");
