...
import { TASK493_WORKER_PROFILE_ID, createTask493WorkerRegistry } from "./worker-operations";

function parseProfile(args: readonly string[]): string {
  if (args.length !== 2 || args[0] !== "--profile" || args[1] !== TASK493_WORKER_PROFILE_ID) {
    throw new SmokeError("smoke_argument_invalid", "TASK-493 worker profile is invalid");
  }
  return args[1];
}
...
export async function runTask493WorkerEntry(args: readonly string[]): Promise<void> {
  const profileId = parseProfile(args);
  await realpath(resolve(import.meta.dir, "../../../.."));
  await runWorkerEntry({
    profileId,
    registry: createTask493WorkerRegistry(),
...
if (import.meta.main) {
  void runTask493WorkerEntry(Bun.argv.slice(2)).catch(() => {
    process.stderr.write('{"code":"task493_worker_failed"}\n');
    process.exitCode = 1;
  });
}
