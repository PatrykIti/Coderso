...
import { TASK517_WORKER_PROFILE_ID, createTask517WorkerRegistry } from "./worker-operations";

function parseProfile(args: readonly string[]): string {
  if (args.length !== 2 || args[0] !== "--profile" || args[1] !== TASK517_WORKER_PROFILE_ID) {
    throw new SmokeError("smoke_argument_invalid", "TASK-517 worker profile is invalid");
  }
  return args[1];
}
...
export async function runTask517WorkerEntry(args: readonly string[]): Promise<void> {
  const profileId = parseProfile(args);
  await realpath(resolve(import.meta.dir, "../../../.."));
  await runWorkerEntry({
    profileId,
    registry: createTask517WorkerRegistry(),
...
if (import.meta.main) {
  void runTask517WorkerEntry(Bun.argv.slice(2)).catch(() => {
    process.stderr.write('{"code":"task517_worker_failed"}\n');
    process.exitCode = 1;
  });
}
