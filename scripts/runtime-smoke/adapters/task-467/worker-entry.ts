...
import { TASK467_WORKER_PROFILE_ID, createTask467WorkerRegistry } from "./worker-operations";

function parseProfile(args: readonly string[]): string {
  if (args.length !== 2 || args[0] !== "--profile" || args[1] !== TASK467_WORKER_PROFILE_ID) {
    throw new SmokeError("smoke_argument_invalid", "TASK-467 worker profile is invalid");
  }
  return args[1];
}
...
export async function runTask467WorkerEntry(args: readonly string[]): Promise<void> {
  const profileId = parseProfile(args);
  await realpath(resolve(import.meta.dir, "../../../.."));
  await runWorkerEntry({
    profileId,
    registry: createTask467WorkerRegistry(),
...
if (import.meta.main) {
  void runTask467WorkerEntry(Bun.argv.slice(2)).catch(() => {
    process.stderr.write('{"code":"task467_worker_failed"}\n');
    process.exitCode = 1;
  });
}
