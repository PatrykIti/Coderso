...
import { TASK554_WORKER_PROFILE_ID, createTask554WorkerRegistry } from "./worker-operations";

function parseProfile(args: readonly string[]): string {
  if (args.length !== 2 || args[0] !== "--profile" || args[1] !== TASK554_WORKER_PROFILE_ID) {
    throw new SmokeError("smoke_argument_invalid", "TASK-554 worker profile is invalid");
  }
  return args[1];
}
...
export async function runTask554WorkerEntry(args: readonly string[]): Promise<void> {
  const profileId = parseProfile(args);
  await realpath(resolve(import.meta.dir, "../../../.."));
  await runWorkerEntry({
    profileId,
    registry: createTask554WorkerRegistry(),
...
if (import.meta.main) {
  void runTask554WorkerEntry(Bun.argv.slice(2)).catch(() => {
    process.stderr.write('{"code":"task554_worker_failed"}\n');
    process.exitCode = 1;
  });
}
