...
import { TASK511_SCENARIO_IDS, type Task511ScenarioId } from "./browser-actions";
...
export interface Task511ScreenshotManifest {
  readonly entries: readonly Task511ScreenshotManifestEntry[];
...
function sessionFor(profile: "fast" | "certification"): string {
  return `task-511-${profile}`;
}

export function assertExactTask511Invocation(value: unknown): asserts value is SmokeInput {
  if (!isPlainObject(value)) invalid("TASK-511 invocation is invalid", "smoke_argument_invalid");
  assertExactKeys(value, ["command", "suite", "profile", "session"], "TASK-511 invocation");
  if (
    value.command !== "run" ||
...
export function buildExactTask511ScreenshotManifest(input: SmokeInput): Task511ScreenshotManifest {
  assertExactTask511Invocation(input);
  const entries = TASK511_SCENARIO_IDS.map((scenarioId, index) =>
...
export function assertExactTask511ScreenshotManifest(
  input: SmokeInput,
  manifest: Task511ScreenshotManifest
): void {
  assertExactTask511Invocation(input);
  if (
    ...
    manifest.entries.length !== TASK511_SCENARIO_IDS.length ||
    manifest.paths.length !== TASK511_SCENARIO_IDS.length ||
    new Set(manifest.paths).size !== TASK511_SCENARIO_IDS.length
  ) {
    invalid("TASK-511 screenshot manifest cardinality is invalid");
  }
  for (const [index, scenarioId] of TASK511_SCENARIO_IDS.entries()) {
    ...
    if (
      ...
      invalid("TASK-511 screenshot manifest row drifted");
    }
  }
}
...
function channels(colorType: number): number {
  ...
  return invalid("TASK-511 PNG color type is invalid");
}
...
export function decodeTask511Png(
  ...
  if (!image.subarray(0, PNG_SIGNATURE.length).equals(PNG_SIGNATURE))
    invalid("TASK-511 PNG signature is invalid");
  ...
  while (offset < image.byteLength) {
    if (offset + 12 > image.byteLength) invalid("TASK-511 PNG chunk is truncated");
    ...
    if (length > MAX_PNG_BYTES || next > image.byteLength)
      invalid("TASK-511 PNG chunk bounds are invalid");
    ...
    if (
      ...
      invalid("TASK-511 PNG chunk checksum is invalid");
    }
    if (type === "IHDR") {
      if (header || offset !== PNG_SIGNATURE.length || length !== 13)
        invalid("TASK-511 PNG header is invalid");
      ...
      if (
        ...
        invalid("TASK-511 PNG header fields are invalid");
      header = true;
    } else if (type === "PLTE") {
      if (!header || data || length === 0 || length % 3 !== 0 || length > 768)
        invalid("TASK-511 PNG palette is invalid");
      palette = true;
    } else if (type === "IDAT") {
      if (!header || ended || length === 0 || (colorType === 3 && !palette))
        invalid("TASK-511 PNG data is invalid");
      data = true;
      compressed.push(content);
    } else if (type === "IEND") {
      if (!header || !data || ended || length !== 0 || next !== image.byteLength)
        invalid("TASK-511 PNG end marker is invalid");
      ended = true;
    } else if ((type.charCodeAt(0) & 0x20) === 0) {
      invalid("TASK-511 PNG critical chunk is unknown");
    }
    offset = next;
  }
  if (!header || !data || !ended) invalid("TASK-511 PNG is incomplete");
  let inflated: Buffer;
  try {
    ...
    throw new SmokeError("smoke_output_invalid", "TASK-511 PNG image data cannot be decoded", {
      cause: error,
    });
  }
  const rowBytes = Math.ceil((width * channels(colorType) * bitDepth) / 8);
  if (inflated.byteLength !== (rowBytes + 1) * height)
    invalid("TASK-511 PNG decoded bytes are invalid");
  for (let row = 0; row < height; row += 1) {
    if ((inflated[row * (rowBytes + 1)] ?? 255) > 4) invalid("TASK-511 PNG filter is invalid");
  }
...
async function assertNoFollowDirectories(root: string, session: string): Promise<string> {
  const canonicalRoot = await realpath(root).catch((error: unknown) => {
    throw new SmokeError("smoke_output_invalid", "TASK-511 repository root is unavailable", {
      cause: error,
    });
  });
  ...
  for (const component of [...EVIDENCE_ROOT.split("/"), session]) {
    ...
    if (!inside(canonicalRoot, current)) invalid("TASK-511 evidence escapes repository root");
    const metadata = await lstat(current).catch((error: unknown) => {
      throw new SmokeError("smoke_output_invalid", "TASK-511 evidence directory is unavailable", {
        cause: error,
      });
    });
    if (metadata.isSymbolicLink() || !metadata.isDirectory())
      invalid("TASK-511 evidence directory is invalid");
  }
  return current;
}
...
async function readExactPng(root: string, path: string): Promise<Buffer> {
  ...
  if (!inside(root, candidate)) invalid("TASK-511 evidence path escapes repository root");
  ...
  try {
    ...
    if (
      ...
      invalid("TASK-511 PNG ownership is invalid");
    }
    ...
    if (
      ...
      invalid("TASK-511 PNG changed while reading");
    return bytes;
  } catch (error) {
    if (error instanceof SmokeError) throw error;
    throw new SmokeError("smoke_output_invalid", "TASK-511 PNG is unavailable", { cause: error });
  } finally {
    await handle?.close();
  }
}
...
async function assertPrivateReportFile(directory: string): Promise<void> {
  const report = await lstat(resolve(directory, REPORT_FILE)).catch((error: unknown) => {
    throw new SmokeError("smoke_output_invalid", "TASK-511 report receipt is unavailable", {
      cause: error,
    });
  });
  if (report.isSymbolicLink() || !report.isFile() || report.nlink !== 1) {
    invalid("TASK-511 report receipt is invalid");
  }
}

export async function validateTask511ScreenshotOutputs(
  root: string,
  input: SmokeInput,
  manifest: Task511ScreenshotManifest
): Promise<readonly SmokeScreenshotResult[]> {
  assertExactTask511ScreenshotManifest(input, manifest);
  ...
  if (
    ...
    invalid("TASK-511 screenshot evidence set is invalid");
  }
  ...
  for (const entry of manifest.entries) {
    ...
    decodeTask511Png(bytes);
    const sha256 = createHash("sha256").update(bytes).digest("hex");
    if (!SHA256.test(sha256)) invalid("TASK-511 PNG hash is invalid");
    results.push(Object.freeze({ path: entry.path, sha256 }));
  }
  if (results.length !== TASK511_SCENARIO_IDS.length)
    invalid("TASK-511 screenshot output cardinality is invalid");
