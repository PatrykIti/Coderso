...
export interface Task491TimingPolicy {
  readonly healthTimeoutMs: number;
  readonly authTimeoutMs: number;
  readonly browserDispatchTimeoutMs: number;
}

export function task491TimingPolicy(profile: SmokeProfileId): Task491TimingPolicy {
  if (profile === "fast") {
  ...
  throw new SmokeError("smoke_argument_invalid", "TASK-491 timing profile is unsupported");
}
...
export function task491Readiness(
  fetchImpl: typeof globalThis.fetch = globalThis.fetch
): readonly SupervisedServerReadinessProbe[] {
  return Object.freeze([
    Object.freeze({
      id: "task491-front-ready",
      // The public root exercises the complete SSR read model including the
      // analytics head resolution; the first render can take a few seconds.
      check: () => exactHttpReady("http://127.0.0.1:3000/", fetchImpl, 15_000),
    }),
    Object.freeze({
      id: "task491-admin-ready",
      check: () => exactHttpReady("http://127.0.0.1:5173/admin/", fetchImpl),
    }),
    Object.freeze({
      id: "task491-site-vite-ready",
      check: () => exactHttpReady("http://127.0.0.1:5174/site/main.ts", fetchImpl),
    }),
  ]);
}

export function createTask491HostSpec(input: {
  readonly context: RuntimeSmokeContext;
  readonly environment: NodeJS.ProcessEnv;
  readonly timing: Task491TimingPolicy;
  ...
  return Object.freeze({
    ...
    readiness: task491Readiness(input.fetch),
    readinessTimeoutMs: input.timing.healthTimeoutMs,
    family: "task491-dev-host",
  });
}

export function startTask491DevHost(
  ...
  const timing = input.timing ?? task491TimingPolicy(context.input.profile);
  return (input.start ?? startSupervisedServer)(
    context,
    createTask491HostSpec({
      context,
