...
export class Task491PrivateWorkspace implements LifecycleResource {
  ...
  private constructor(session: string, path: string) {
    this.name = `task491-workspace-${session}`;
    this.path = path;
    this.authStatePath = join(path, "admin-auth-state.json");
  }

  static async create(context: RuntimeSmokeContext): Promise<Task491PrivateWorkspace> {
    context.lifecycle.assertAccepting();
    const root = await realpath(context.root).catch((error: unknown) => {
      throw new SmokeError("smoke_repository_invalid", "TASK-491 root is unavailable", {
        cause: error,
      });
    });
    const parentInput = resolveInsideRoot(root, ".tmp/runtime-smoke", "TASK-491 workspace root");
    ...
    if (parent !== parentInput || !isWithin(root, parent) || !(await stat(parent)).isDirectory()) {
      throw new SmokeError("smoke_repository_invalid", "TASK-491 workspace parent is redirected");
    }

    let candidate: string | null = null;
    try {
      candidate = await mkdtemp(join(parent, `${context.input.session}-task491-`));
      ...
      if (
        ...
        throw new SmokeError(
          ...
          "TASK-491 private workspace ownership is invalid"
        );
      }
      const workspace = new Task491PrivateWorkspace(context.input.session, canonical);
...
export function createTask491PrivateWorkspace(
  context: RuntimeSmokeContext
): Promise<Task491PrivateWorkspace> {
  return Task491PrivateWorkspace.create(context);
}
