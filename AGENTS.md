...
- Preserve readability-critical multiline formatting with `// prettier-ignore` where prettier's canonical output would collapse clarity (for example short union type literals or aligned multi-line conditions). Use it sparingly, only where readability genuinely suffers, and never to bypass formatting on new authored code that prettier already formats readably. Do not fight prettier's default output elsewhere; the default canonical style is the house style.
